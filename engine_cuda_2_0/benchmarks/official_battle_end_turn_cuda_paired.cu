...
BattleReplayStats run_seed_cuda(
    ...
    while (!official.state.isFinish()) {
        if (stats.decisions >= decision_limit) {
            // A legal coverage policy can revisit reversible actions forever
            // (for example, retreat/switch loops). Every decision up to this
            // cap has already been compared across official CPU, host POD and
            // CUDA POD. Leave terminal_result as None so aggregate matrix
            // reports count this rollout explicitly as unfinished instead of
            // aborting all remaining deck pairs.
            break;
        }
