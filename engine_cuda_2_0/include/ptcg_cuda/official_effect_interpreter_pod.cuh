...
official_record_auto_empty_selection_boundary(OfficialStatePod* state) {
    ...
    constexpr std::uint8_t attack_post_effect_stage = 3;
    constexpr std::uint8_t attack_to_bench_context = 6;
    constexpr std::uint8_t attack_to_hand_context = 20;
    constexpr std::uint8_t attack_attached_discard_context = 22;
    constexpr std::uint8_t attack_discard_context = 23;
    const bool attack_post_effect_empty_boundary =
        state->attack_flow_stage == attack_post_effect_stage
        && (state->select_context == attack_to_bench_context
            || state->select_context == attack_to_hand_context
            || state->select_context == attack_attached_discard_context
            || state->select_context == attack_discard_context);
    // Attack effects run inside the ApiSelect call that chose the attack;
    // their zero-cardinality selections are auto-advanced without exposing a
    // separate State::step boundary. Trigger-return-to-Main paths above are
    // the cases where the hidden boundary remains observable.
    if (trigger_hidden_boundary || attack_post_effect_empty_boundary) {
...
official_advance_effect_interpreter(
    ...
    while (interpreter.active != 0) {
        ...
        if (effect_type == OfficialEffectTypeId::kSwitch) {
            // ActivateEffect performs this check before exposing the Bench
            // choice.  For an opponent-selected switch, the effect is treated
            // as affecting that player's Active Pokemon, so Mist Energy must
            // suppress the selection itself (not merely make the eventual
            // switch primitive a no-op).
            const std::uint8_t player_mask = official_effect_player_mask(
                state, rules, effect);
            if (player_mask == 1 || player_mask == 2) {
                const std::int32_t target_player = official_effect_target_player(
                    *state, player_mask, 0);
                const std::int32_t select_player = official_effect_select_player(
                    *state, effect);
                const bool affects_active =
                    official_interpreter_has_flag(
                        effect, kOfficialEffectTargetActive)
                    || (state->effect_state.ability.use_player != target_player
                        && select_player != target_player);
                if (affects_active
                    && !official_interpreter_has_flag(
                        effect, kOfficialEffectTargetBench)
                    && official_effect_blocks_active_effect(
                        state, rules, target_player)) {
                    if (!official_pod_ok(state)) {
                        return OfficialEffectInterpreterResult::kError;
                    }
                    official_finish_effect_iteration(state);
...
official_apply_effect_action(
    ...
    if (resume == OfficialEffectResumeKind::kSkillChooseEffect
        ...
        const std::int32_t own_second =
            skill->values[kSkillSecondEffectStart];
        const std::int32_t enemy_second =
        ...
        const std::uint16_t start_index = selected_yes
            ...
            : static_cast<std::uint16_t>(
                own_second > 0 ? own_second : enemy_second);
