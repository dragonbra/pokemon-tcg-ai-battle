    ...
    switch (type) {
        ...
        case OfficialEffectTypeId::kKo:
            for (std::uint16_t index = 0; index < state->targets.count; ++index) {
                const OfficialAreaRefPod target = state->targets.values[index];
                if (!official_pod_area_ref_valid(state, target)) continue;
                // EffectInstant::Ko uses checkGetCardNotPreventEffect.  This
                // matters for copied attacks too: Mist Energy on the opposing
                // Active Pokemon must block Destined Fight's effect KO.
                if (official_effect_blocks_target_effect(
                        state, rules, target.card)) continue;
                OfficialCardStatePod* card = official_pod_card(state, target.card);
        ...
        case OfficialEffectTypeId::kPrizeToHand:
            // Official SelectedPrizeToHand calls MoveCard(..., Hand, 1)
            // without setting State::changed.  Do not use the generic target
            // mover here: it marks changed. The `1` is the move-log openType
            // (hidden from the opponent), not the destination card's reverse
            // flag.
            for (std::uint16_t index = 0;
                 index < state->targets.count && official_pod_ok(state);
                 ++index) {
                ...
                if (!official_pod_area_ref_valid(state, target)) continue;
                official_pod_move_ref_complete(
                    state, target.card, OfficialArea::kHand, false, false, 1);
            }
            break;
