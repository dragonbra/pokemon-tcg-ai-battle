...
PTCG_OFFICIAL_CORE_HD inline std::int32_t official_pod_prize_count(
    ...
    if (card->area == static_cast<std::uint8_t>(OfficialArea::kActive)) {
        const auto* attacker_turn_state = reinterpret_cast<const std::int8_t*>(
            &state->players[1 - card->player].turn_state);
        if ((card->runtime_flags & kCardKoEnemyTerastalAttackDamage) != 0) {
            count += attacker_turn_state[6];
        }
        if ((card->runtime_flags & kCardKoEnemyNAttackDamage) != 0) {
            count += attacker_turn_state[7];
        }
    }
