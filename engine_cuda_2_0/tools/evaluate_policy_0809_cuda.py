...
from evaluation.frozen_0812_runtime import load_frozen_0812_runtime_catalog
...
from engine_cuda_2_0.tools.evaluate_policy_0806_cuda import (
    build_cuda_game_records,
    resolve_candidate_deck_path,
)
policy_identity = importlib.import_module(
    "train.0040_dragapult_0809_action_boundary_rl.policy_identity"
)


POLICY_ID = "Policy-0809-CPU-checkpoint-2.0"
POLICY_CHECKPOINT = (
    ROOT / "archive/pretrained/0031_friend_0809_gsb_v5_value_v9/model.pt"
)
POLICY_SHA256 = "11ac5c9a40ab46536a1bc651c388617edac4d7c5c4e1e2de1476d3317e7ad7f3"
POLICY_REGISTRY = ROOT / "engine_cuda_2_0/configs/policy_0809_cpu_checkpoint_registry.json"
POLICY_PORTABLE_CHECKPOINT = ROOT / (
    "archive/submission/0031_mega_lopunny_froslass_dd632_0809/strategy/model.bin"
)
POLICY_PORTABLE_SHA256 = "3f6683b0d916c72a31f3f487edb597c0cec4aedf529a236a4effacb177de89db"
POOL_ROOT = ROOT / (
    "evaluation/arena/frozen_pools/"
    "0812_top100_top500_stadiums_mill_plus_limitless_dragapult_v3"
)
BENCHMARK = ROOT / "engine_cuda_2_0/tools/benchmark_0037_cuda_resident_refill.py"
CUDA_EXTENSION = ROOT / "engine_cuda_2_0/build/torch_official"
CUDA_RULES = ROOT / "engine_cuda_2_0/generated/private/official_3aaeaa92/official_rules.bin"
TEMP_ROOT = ROOT / (
...
OUTPUT_ROOT = ROOT / (
    ...
    "0812_top100_top500_stadiums_mill_v3_cuda_seeded_2048_agent_choice_v3_"
    "kaggle_fp16_storage_fp32_runtime_v1"
)
EXPECTED_GAMES = 2048
LANE_COUNT = 256
...
def _catalog() -> tuple[Any, tuple[Any, ...]]:
    ...
    if len(candidates) != 65 or sum(item.games for item in catalog.pool.schedule) != 256:
        raise RuntimeError("FATAL: Frozen catalog is not the immutable 65-deck/256-slot pool")
...
def preflight_identity() -> dict[str, Any]:
    ...
    import torch
    from engine_cuda_2_0.tools.benchmark_0037_update32_cuda_rollout import (
        expanded_portable_state,
    )

    payload = torch.load(POLICY_CHECKPOINT, map_location="cpu", weights_only=True)
    state = payload.get("state_dict") if isinstance(payload, dict) else None
    if not isinstance(state, dict):
        raise RuntimeError("FATAL: compact 0809 checkpoint has no state_dict")
    materialized = expanded_portable_state(state)
    focal = policy_identity.audit_materialized_state_dict(
        POLICY_ID, materialized,
        purpose="formal_frozen_cuda2048_focal_source_preflight",
        registry_path=POLICY_REGISTRY,
    ).to_manifest()
    opponent = policy_identity.audit_materialized_state_dict(
        POLICY_ID, materialized,
...
def _package(candidate: Any) -> Path:
    ...
    deck_path = resolve_candidate_deck_path(candidate, POOL_ROOT / "decks")
    if not package.exists():
        # The target branch supplies a compact 0809 CPU checkpoint plus its
        # matching FP16-storage/FP32-runtime portable artifact.  Keep the
        # legacy exporter fail-closed and assemble only these audited assets.
        template = POOL_ROOT / "policies/policy_0806"
        temporary = package.with_name(f".{package.name}.{uuid.uuid4().hex}.tmp")
        shutil.copytree(template, temporary)
        try:
            shutil.copy2(deck_path, temporary / "deck.csv")
            shutil.copy2(POLICY_PORTABLE_CHECKPOINT, temporary / "strategy/model.bin")
            manifest = json.loads(
                (template / "manifest.json").read_text(encoding="utf-8")
            )
            deck_manifest = json.loads(
                (deck_path.parent / "manifest.json").read_text(encoding="utf-8")
            )
            manifest.update({
                "candidate": candidate.name,
                "checkpoint": str(POLICY_CHECKPOINT),
                "checkpoint_schema_version": "0031_compact_shared_prototype_fp32_v1",
                "checkpoint_sha256": POLICY_SHA256,
                "deck_id": candidate.name,
                "deck_sha256": deck_manifest["exact_deck_sha256"],
                "portable_checkpoint_schema_version": (
                    "0031_shared_prototype_fp16_storage_fp32_runtime_candidate_checkpoint_v1"
                ),
                "portable_checkpoint_sha256": POLICY_PORTABLE_SHA256,
                "runtime_dtype": "fp32",
                "storage_dtype": "fp16",
                "version": "Policy-0809-CPU-checkpoint-2.0",
            })
...
def _run_one(catalog: Any, candidate: Any, *, games: int = EXPECTED_GAMES) -> dict[str, Any]:
    ...
    if not result_path.is_file():
        ...
        command = [
            ...
            "--policy-registry", str(POLICY_REGISTRY),
            "--extension-dir", str(CUDA_EXTENSION),
            "--rules", str(CUDA_RULES),
...
def _render_deck(
    ...
    *,
    ...
    pool_id: str = "0812_top100_top500_stadiums_mill_plus_limitless_dragapult_v3",
) -> str:
    del catalog
    candidate = next(
...
def _publish(
    ...
    *,
    ...
    validate_combat_mat_index(manifest, expected_catalog_decks=65)
...
def main(argv: list[str] | None = None) -> int:
    ...
    valid_numbers = {f"{number:03d}" for number in range(1, 66)}
    if not requested_numbers or any(number not in valid_numbers for number in requested_numbers):
        parser.error("choose --top-ten or deck numbers 001 through 065")
