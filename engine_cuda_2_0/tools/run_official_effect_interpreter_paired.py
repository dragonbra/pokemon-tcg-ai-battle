...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run the official effect-range CPU POD/CUDA paired smoke."
    )
    ...
    parser.add_argument(
        ...
        default=(
            CUDA_ENGINE_ROOT
            / "artifacts"
            / "official_effect_interpreter_paired.json"
        ),
    )
...
def main() -> None:
    ...
    build_dir = CUDA_ENGINE_ROOT / "build" / "official_effect_interpreter_paired"
    build_dir.mkdir(parents=True, exist_ok=True)
    executable = build_dir / "official_effect_interpreter_smoke"
    if args.skip_build:
        if not executable.is_file():
            raise SystemExit(f"benchmark executable does not exist: {executable}")
        command = f"/build/official_effect_interpreter_smoke /workspace/{rules_relative}"
    else:
        command = (
            ...
            "/workspace/engine_cuda_2_0/benchmarks/official_effect_interpreter_smoke.cu "
            "-o /build/official_effect_interpreter_smoke; "
            f"/build/official_effect_interpreter_smoke /workspace/{rules_relative}"
        )
    ...
    if (
        ...
        or result.get("selected_card") != 11
        or result.get("selection_error") != 0
        or result.get("condition_error") != 0
    ):
        raise RuntimeError("official effect interpreter paired smoke did not pass")
    evidence = {
        **result,
        "contract": "official_effect_interpreter_paired_v1",
        "scope": [
            "condition_true_false",
            "card_option_build",
            "selection_suspend_resume",
            "selected_target_primitive",
            "remaining_effect_chain",
        ],
        "official_fixture": {"skill_id": 24, "card_id": 72},
        "docker_image": args.image,
        "docker_image_id": docker_image_id(args.image),
        "benchmark_sha256": sha256_file(
            CUDA_ENGINE_ROOT
            / "benchmarks"
            / "official_effect_interpreter_smoke.cu"
        ),
        "interpreter_sha256": sha256_file(
            CUDA_ENGINE_ROOT
            / "include"
            / "ptcg_cuda"
            / "official_effect_interpreter_pod.cuh"
        ),
