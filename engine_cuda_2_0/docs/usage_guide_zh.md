...
本文说明 `dev/cyd_main` 分支下 `engine_cuda_2_0/` 的构建、Docker CUDA 运行、
...
python engine_cuda_2_0/tools/compile_rule_pack.py `
  engine_cuda_2_0/rules/smoke_rules.json `
  --check
```
...
或公开 artifact。仓库的 `.gitignore` 已屏蔽 `engine_cuda_2_0/generated/private/`。
...
```powershell
python engine_cuda_2_0/tools/extract_official_rules.py `
  ...
  --output engine_cuda_2_0/generated/private/official_3aaeaa92

python engine_cuda_2_0/tools/compile_official_rule_pack.py `
  engine_cuda_2_0/generated/private/official_3aaeaa92/official_rules.json `
  engine_cuda_2_0/generated/private/official_3aaeaa92/official_rules.bin

python engine_cuda_2_0/tools/verify_rule_pack.py `
  --manifest engine_cuda_2_0/generated/private/official_3aaeaa92/official_rules.bin.manifest.json `
  --source "engine/source/ptcgProgram 22" `
  --oracle "<path-to-frozen-libcg_seeded.so>" `
  --card-data data/simulation/EN_Card_Data.csv data/simulation/JP_Card_Data.csv
```
...
cmake -S engine_cuda_2_0 -B engine_cuda_2_0/build -G Ninja \
  -DCMAKE_CUDA_ARCHITECTURES=86
cmake --build engine_cuda_2_0/build --parallel

./engine_cuda_2_0/build/ptcg_cuda_smoke \
...
python engine_cuda_2_0/tools/run_official_semantic_micro_fixture.py
```
...
```text
engine_cuda_2_0/artifacts/0022_deck40_official_parity.json
```
...
python engine_cuda_2_0/tools/run_0022_deck40_official_parity.py `
  ...
  --skip-setup `
  --skip-battle `
  --output engine_cuda_2_0/artifacts/frozen51_manifest_summary.json
```

然后在一个 persistent Docker GPU 容器中编译并运行全部 51 x 50 个非镜像有序 matchup：

```powershell
python engine_cuda_2_0/tools/run_official_battle_ordered_matrix_cuda.py `
  --manifest engine_cuda_2_0/generated/private/official_3aaeaa92/frozen51_all_s1_1/manifest.json `
  --seed-start 1 `
  --seed-count 1 `
  --decision-limit 512 `
  --policy coverage-first-legal `
  ...
  --nvcc-threads 1 `
  --output engine_cuda_2_0/artifacts/frozen51_all_ordered_s1.json
```
...
```powershell
python engine_cuda_2_0/tools/run_official_battle_ordered_matrix_cuda.py `
  --manifest engine_cuda_2_0/generated/private/official_3aaeaa92/frozen51_mirrors_s2026080701_3/manifest.json `
  --seed-start 2026080701 `
  --seed-count 3 `
  --decision-limit 512 `
  --policy coverage-random-legal `
...
```powershell
python engine_cuda_2_0/tools/audit_frozen51_support.py `
  --evidence engine_cuda_2_0/artifacts/frozen51_all_ordered_s1.json
```
...
docker run --rm --gpus all `
  ...
  python engine_cuda_2_0/tools/run_0031_semantic_cuda_smoke.py `
...
`engine_cuda_2_0/generated/private/official_3aaeaa92/`；不要把该文件 `git add -f`。

### Docker 中找不到源码

命令必须从仓库根目录运行。目标目录名是 `engine_cuda_2_0`，工具中的容器路径也是
`/workspace/engine_cuda_2_0`；不要用早期原型分支的目录名。
