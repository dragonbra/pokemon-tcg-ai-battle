...
from engine_cuda_2_0.tools.evaluate_policy_0809_cuda import (
    CANDIDATE_CONTRACT,
    DIAGNOSTIC_SCHEMA,
    EXPECTED_GAMES,
    POLICY_ID,
...
class Policy0809CudaEvaluationTest(unittest.TestCase):
    ...
    def test_full_catalog_has_65_decks_and_14_explicit_meta_classes(self) -> None:
    ...
    def test_detail_contract_requires_60_cards_65_opponents_and_14_meta_rows(self) -> None:
        ...
        self.assertEqual(len(by_opponent), 65)
        self.assertEqual(len(by_meta), 14)
        self.assertEqual(sum(item["games"] for item in by_meta) + other["games"], 130)
    ...
    def test_index_and_detail_renderers_expose_the_fixed_ui_contract(self) -> None:
        ...
        validate_combat_mat_index(index_payload, expected_catalog_decks=65)
        candidate = candidates[6]
        games = []
        for opponent in candidates:
            games.append({
        ...
        self.assertEqual(page.count('class="chart-row opponent-row"'), 65)
