...
def rollout_turn(sid, cur_obs, your_index):
    # 2-Ply Minimax Rollout! Simulates both our turn AND the opponent's counter-attack.
    steps = 0
    ...
    while steps < 40:
        ...
        # Break when 2 full turns have passed and it's our turn to move again
        if cur_obs.current.turn >= start_turn + 2 and cur_obs.select.context == SelectContext.MAIN:
            break
...
def SEARCH_ALGO(obs_dict, obs):
    ...
    candidates = base_order[:6] # Top 6
    
    try:
        my_p = obs.current.players[obs.current.yourIndex]
        
        candidate_totals = {c: 0.0 for c in candidates}
        candidate_counts = {c: 0 for c in candidates}
        
        def evaluate_candidate(cand, count):
            for _ in range(count):
                yd = random.sample(my_deck, getattr(my_p, "deckCount", 60))
                sbi = search_begin(obs, your_deck=yd)
                if getattr(sbi, "error", 0) != 0 or sbi.state is None: continue
                ar = search_step(sbi.state.searchId, [cand])
                if getattr(ar, "error", 0) != 0 or ar.state is None: continue
                
                # This rollout now plays out the opponent's turn too!
                cur = rollout_turn(ar.state.searchId, ar.state.observation, obs.current.yourIndex)
                candidate_totals[cand] += evaluate_state(cur)
                candidate_counts[cand] += 1
                
        # Phase 1: 1 sample for top 6 (6 rollouts)
        for c in candidates:
            if time.time() - t0 > 0.6: break
            evaluate_candidate(c, 1)
            
        valid_cands = [c for c in candidates if candidate_counts[c] > 0]
        if not valid_cands: return None
        
        valid_cands.sort(key=lambda c: candidate_totals[c] / candidate_counts[c], reverse=True)
        best_cands = valid_cands[:2]
        
        # Phase 2: 3 more samples for top 2 (6 rollouts)
        for c in best_cands:
            if time.time() - t0 > 1.35: break
            evaluate_candidate(c, 3)
            
        best_action, best_val = None, -float('inf')
        for c in best_cands:
            if candidate_counts[c] > 0:
                avg = candidate_totals[c] / candidate_counts[c]
                if avg > best_val:
                    best_val, best_action = avg, c
