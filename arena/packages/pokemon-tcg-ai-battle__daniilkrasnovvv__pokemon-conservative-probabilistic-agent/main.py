...
def clone_attack_plan(src: AttackPlan) -> AttackPlan:
    return AttackPlan(
        attacker=src.attacker,
        target=src.target,
        attack_index=src.attack_index,
        remain_hp=src.remain_hp,
        needs_energy=src.needs_energy,
    )
...
def simulate_action(obs, action):
    """Run one original-style rollout without leaking simulated globals.

    The original agent stores its tactical plan and Lunatone usage in globals.
    AdvancedPolicy inside a simulated rollout mutates those globals.  Without
    restoration, a hypothetical line can affect later real selections in the
    same turn.  Everything else intentionally stays identical to the 826-score
    implementation, including deck sampling and the evaluation function.
    """
    global plan, ability_used, pre_turn

    saved_plan = clone_attack_plan(plan)
    saved_ability_used = ability_used
    saved_pre_turn = pre_turn
    ...
    try:
        ...
        plan = saved_plan
        ability_used = saved_ability_used
        pre_turn = saved_pre_turn
