...
def SEARCH_ALGO(obs_dict, obs):
    ...
    best_action, best_avg_val = None, -float('inf')
    ...
    try:
        for first in candidates:
            ...
            for _ in range(samples):
                ...
                ar = search_step(sbi.state.searchId, [first])
                ...
                total_val += evaluate_state(cur)
                valid_samples += 1
                
            if valid_samples > 0:
                avg_val = total_val / valid_samples
                if avg_val > best_avg_val:
                    best_avg_val, best_action = avg_val, first
