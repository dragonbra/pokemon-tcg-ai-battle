...
class ModelConfig:
    ...
    prize_feature_mode: str = "belief"
    prize_num_features: int = 12
    ...
    def validate(self) -> None:
        ...
        if self.prize_feature_mode not in ("control", "belief"):
            raise ValueError("prize_feature_mode must be control or belief")
        if self.prize_num_features != 12:
            raise ValueError("PrizeLedger v4 requires 12 numeric features")
...
class DirectedPrizeAttention(nn.Module):
    """Q=state/option intent, K=card resource identity, V=prize belief."""

    def __init__(self, d_model: int, heads: int, dropout: float):
        super().__init__()
        if d_model % heads:
            raise ValueError("d_model must divide prize-attention heads")
        ...
        # Both arms begin as the unchanged backbone. Prize context must earn its
        # influence during training, so the residual projection starts at zero.
    ...
    def forward(
        ...
        context = context.view(query.size(0), query.size(1), self.d_model)
        return self.output(context)


class IDOnlyPointerPolicy(nn.Module):
    """Four encoder layers plus one selected-option GRU pointer decoder."""
    ...
    def __init__(self, config: ModelConfig):
        ...
        self.prize_key_num = nn.Sequential(nn.Linear(3, d), nn.GELU(), nn.Linear(d, d))
        self.prize_value_num = nn.Sequential(
            nn.Linear(config.prize_num_features, d), nn.GELU(), nn.Linear(d, d)
        )
        self.prize_key_norm = nn.LayerNorm(d)
        self.prize_value_norm = nn.LayerNorm(d)
        self.prize_attention = DirectedPrizeAttention(d, config.heads, config.dropout)
        self.prize_state_norm = nn.LayerNorm(d)
        self.prize_option_norm = nn.LayerNorm(d)
    ...
    def encode(self, batch: dict[str, Tensor]) -> tuple[Tensor, Tensor]:
        ...
        prize_num = batch["prize_num"]
        if self.config.prize_feature_mode == "control":
            neutral = torch.zeros_like(prize_num)
            neutral[..., 0] = prize_num[..., 0]
            prize_num = neutral
        prize_card = self.card(batch["prize_card"])
        prize_key = self.prize_key_norm(
            prize_card + self.prize_key_num(prize_num[..., :3])
        )
        prize_value = self.prize_value_norm(
            prize_card + self.prize_value_num(prize_num)
        )
        prize_queries = torch.cat([state_repr.unsqueeze(1), option_repr], dim=1)
        prize_context = self.prize_attention(
            prize_queries, prize_key, prize_value, batch["prize_mask"]
        )
        state_repr = self.prize_state_norm(state_repr + prize_context[:, 0])
        option_repr = self.prize_option_norm(option_repr + prize_context[:, 1:])
    ...
    def teacher_logits(self, batch: dict[str, Tensor]) -> Tensor:
        ...
        for step in range(batch["targets"].size(1)):
            pointer = (self.pointer_query(hidden).unsqueeze(1) * keys).sum(-1) / math.sqrt(self.config.d_model)
...
def collate_examples(rows: list[dict[str, Any]]) -> dict[str, Tensor]:
    ...
    prize_count = max(1, max(len(row["prize_card"]) for row in rows))
    ...
    prize_card = np.zeros((batch_size, prize_count), dtype=np.int64)
    prize_num = np.zeros((batch_size, prize_count, 12), dtype=np.float32)
    prize_mask = np.zeros((batch_size, prize_count), dtype=np.bool_)
    ...
    prize_source_code = np.zeros(batch_size, dtype=np.int64)
    for index, row in enumerate(rows):
        entities = len(row["entity_cat"])
        options = len(row["option_cat"])
        prizes = len(row["prize_card"])
        if entities:
        ...
        if prizes:
            prize_card[index, :prizes] = np.asarray(row["prize_card"], dtype=np.int64)
            prize_num[index, :prizes] = np.asarray(row["prize_num"], dtype=np.float32)
            prize_mask[index, :prizes] = True
        ...
        prize_source_code[index] = int(row.get("prize_source_code", 0))
    return {
        ...
        "prize_card": torch.from_numpy(prize_card),
        "prize_num": torch.from_numpy(prize_num),
        "prize_mask": torch.from_numpy(prize_mask),
        "prize_source_code": torch.from_numpy(prize_source_code),
    }
...
from prize_belief import PrizeLedger


class MarniePrizeRuntime:
    """Greedy decoder with an optional CUDA device and PrizeLedger."""
    ...
    def __init__(self, checkpoint_path: str | Path, deck: list[int]):
        ...
        try:
            ...
            pass
        requested_device = os.environ.get("PTCG_BC_DEVICE", "cpu").strip().lower()
        ...
        self.ledger = PrizeLedger(self.deck_ids)
    ...
    def decode(self, obs: dict[str, Any]) -> list[int]:
        ...
        batch = move_batch(collate_examples([row]), self.device)
        with torch.inference_mode():
            ...
            chosen = torch.zeros((1, option_count), dtype=torch.bool, device=self.device)
            action: list[int] = []
            for step in range(max_count):
                ...
                if step < min_count:
                    stop = stop.masked_fill(
                        torch.ones_like(stop, dtype=torch.bool),
                        torch.finfo(stop.dtype).min,
                    )
                choice = int(
                    torch.argmax(torch.cat([pointer, stop], dim=1), dim=1).item()
                )
