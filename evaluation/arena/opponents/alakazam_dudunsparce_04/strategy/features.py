...
from .card_metadata import CARD_METADATA_DIM
...
def feature_config_for_schema(schema_version: str) -> PTCGFeatureConfig:
    ...
    if schema_version == UNIVERSAL_FEATURE_SCHEMA:
        return PTCGFeatureConfig(
            ...
            state_token_count=40,
            candidate_numeric_dim=10 + CARD_METADATA_DIM,
            max_candidates=64,
            card_vocab_size=4096,
            action_type_vocab_size=32,
            schema_version=UNIVERSAL_FEATURE_SCHEMA,
            deck_token_count=60,
            deck_numeric_dim=6 + CARD_METADATA_DIM,
            entity_token_count=12,
            entity_numeric_dim=16 + CARD_METADATA_DIM,
...
def feature_schema_for_config(config: PTCGFeatureConfig) -> str:
    ...
    if config.schema_version == UNIVERSAL_FEATURE_SCHEMA:
        universal_shape = (
            config.state_numeric_dim,
            config.state_token_count,
            config.candidate_numeric_dim,
        ...
        if universal_shape == (
            64,
            40,
            10 + CARD_METADATA_DIM,
            60,
            6 + CARD_METADATA_DIM,
            12,
            16 + CARD_METADATA_DIM,
            32,
            8,
            64,
        ):
            return UNIVERSAL_FEATURE_SCHEMA
        raise ValueError("unsupported universal feature configuration")
...
def _card_metadata(observation: dict[str, Any], card_id: int | None) -> list[float]:
    if card_id is None:
        return [0.0] * CARD_METADATA_DIM
    metadata = observation.get("rl_card_metadata") or {}
    values = metadata.get(card_id, metadata.get(str(card_id), []))
    if not isinstance(values, list) or len(values) != CARD_METADATA_DIM:
        return [0.0] * CARD_METADATA_DIM
...
def _deck_from_observation(observation: dict[str, Any]) -> list[int]:
    values = observation.get("rl_deck") or []
    result: list[int] = []
    for value in values:
        try:
...
def _visible_card_counts(player: dict[str, Any]) -> dict[int, Counter[int]]:
    zones = {
        "hand": Counter(),
        "discard": Counter(),
        "field": Counter(),
    }
    for card in _cards(player, "hand"):
        card_id = _card_id(card)
        if card_id is not None:
            zones["hand"][card_id] += 1
    for card in _cards(player, "discard"):
        card_id = _card_id(card)
        if card_id is not None:
            zones["discard"][card_id] += 1
    for area in ("active", "bench"):
        for card in _cards(player, area):
            card_id = _card_id(card)
            if card_id is not None:
                zones["field"][card_id] += 1
    return zones


def _universal_deck_context(
    observation: dict[str, Any],
    player: dict[str, Any],
    config: PTCGFeatureConfig,
) -> tuple[list[int], list[list[float]]]:
    deck = _deck_from_observation(observation)
    counts = Counter(deck)
    visible = _visible_card_counts(player)
    ordered = sorted(deck)[: config.deck_token_count]
    card_ids: list[int] = []
    numeric: list[list[float]] = []
    for card_id in ordered:
        initial = counts[card_id]
        hand = visible["hand"][card_id]
        discard = visible["discard"][card_id]
        field = visible["field"][card_id]
        known = min(initial, hand + discard + field)
        remaining = max(0, initial - known)
        card_ids.append(min(config.card_vocab_size, card_id + 1))
        numeric.append(
            [
                initial / 4.0,
                hand / 4.0,
                ...
                field / 4.0,
                remaining / 4.0,
                remaining / max(1.0, float(initial)),
            ]
            + _card_metadata(observation, card_id)
        )
    while len(card_ids) < config.deck_token_count:
        card_ids.append(0)
        numeric.append([0.0] * config.deck_numeric_dim)
...
def _entity_context(
    ...
    entities: list[tuple[int, int, int, dict[str, Any] | None]] = []
    for owner, current in ((0, player), (1, opponent)):
        active = _cards(current, "active")
        entities.append((owner, 0, 0, active[0] if active else None))
        bench = _cards(current, "bench")
        for slot in range(5):
            entities.append((owner, 1, slot, bench[slot] if slot < len(bench) else None))

    card_ids: list[int] = []
    numeric: list[list[float]] = []
    for owner, zone, slot, card in entities[: config.entity_token_count]:
        card_id = _card_id(card)
        card_ids.append(min(config.card_vocab_size, card_id + 1) if card_id is not None else 0)
        owner_state = player if owner == 0 else opponent
        hp = _normal((card or {}).get("hp", 0), 400.0)
        max_hp = _normal((card or {}).get("maxHp", (card or {}).get("hp", 0)), 400.0)
        damage = max(0.0, max_hp - hp)
        energies = (card or {}).get("energies", (card or {}).get("energyCards", [])) or []
        ...
        numeric.append(
            [
                ...
                max_hp,
                damage,
                _normal(len(energies), 10.0),
                _normal(len(tools), 4.0),
                1.0 if owner_state.get("asleep", False) else 0.0,
                1.0 if owner_state.get("burned", False) else 0.0,
                1.0 if owner_state.get("confused", False) else 0.0,
                1.0 if owner_state.get("paralyzed", False) else 0.0,
                1.0 if owner_state.get("poisoned", False) else 0.0,
                1.0 if (card or {}).get("appearThisTurn", False) else 0.0,
                _normal((card or {}).get("serial", 0), 256.0),
                _normal(len((card or {}).get("preEvolution", []) or []), 4.0),
            ]
            + _card_metadata(observation, card_id)
        )
    while len(card_ids) < config.entity_token_count:
        card_ids.append(0)
        numeric.append([0.0] * config.entity_numeric_dim)
    return card_ids, numeric


def _universal_history(
    observation: dict[str, Any],
    config: PTCGFeatureConfig,
) -> tuple[list[int], list[list[float]]]:
    history = [item for item in observation.get("rl_history") or [] if isinstance(item, dict)]
    history = history[-config.history_token_count :]
    card_ids: list[int] = []
    numeric: list[list[float]] = []
    offset = config.history_token_count - len(history)
    for index, item in enumerate(history):
        card_id = _as_int(item.get("cardId"), 0)
        card_ids.append(min(config.card_vocab_size, card_id + 1) if card_id > 0 else 0)
        numeric.append(
            [
                _normal(item.get("type", 0), 32.0),
                _normal(item.get("cardId", 0), 4096.0),
                _normal(item.get("attackId", 0), 2000.0),
                _normal(item.get("targetCount", 0), 32.0),
                _normal(item.get("selectionType", 0), 16.0),
                _normal(item.get("selectionContext", 0), 64.0),
                _normal(offset + index, float(max(1, config.history_token_count - 1))),
                _normal(item.get("effectStep", 0), 32.0),
            ]
        )
    while len(card_ids) < config.history_token_count:
        card_ids.insert(0, 0)
        numeric.insert(0, [0.0] * config.history_numeric_dim)
    return card_ids, numeric


def _universal_numeric(
    observation: dict[str, Any],
    player: dict[str, Any],
    opponent: dict[str, Any],
    deck: list[int],
    ...
    visible = _visible_card_counts(player)
    known = sum(visible[zone].total() for zone in visible)
    own_field = [*_cards(player, "active"), *_cards(player, "bench")]
    opponent_field = [*_cards(opponent, "active"), *_cards(opponent, "bench")]
    status_own = sum(
        bool(player.get(key, False)) for key in ("asleep", "burned", "confused", "paralyzed", "poisoned")
    )
    status_opponent = sum(
        bool(opponent.get(key, False))
        for key in ("asleep", "burned", "confused", "paralyzed", "poisoned")
    )
    history = [item for item in observation.get("rl_history") or [] if isinstance(item, dict)]
    return [
        len(deck) / 60.0,
        len(Counter(deck)) / 60.0,
        known / 60.0,
        max(0, len(deck) - known) / 60.0,
        len(_cards(player, "hand")) / 20.0,
        len(_cards(player, "discard")) / 60.0,
        len(own_field) / 6.0,
        len(opponent_field) / 6.0,
        status_own / 5.0,
        status_opponent / 5.0,
        min(len(history), 32) / 32.0,
        _normal(observation.get("rl_effect_step", 0), 32.0),
        _normal((observation.get("current") or {}).get("turn", 0), 100.0),
        _normal(len((observation.get("select") or {}).get("option") or []), 64.0),
        _normal(opponent.get("handCount", 0), 20.0),
        _normal(opponent.get("deckCount", 0), 60.0),
        _normal(opponent.get("prizeCount", len(opponent.get("prize") or [])), 6.0),
        1.0 if not opponent.get("hand") else 0.0,
    ]
...
def encode_observation(
    ...
    if config.state_numeric_dim not in (24, 32, 36, 40, 46, 64):
        raise ValueError(
            "unsupported feature schema state width: "
            f"{config.state_numeric_dim}; expected 24, 32, 36, 40, 46, or 64"
        )
    ...
    deck = _deck_from_observation(observation)
    if config.schema_version == UNIVERSAL_FEATURE_SCHEMA:
        state_numeric.extend(_universal_numeric(observation, player, opponent, deck))
    ...
    extra: dict[str, Any] = {}
    if config.schema_version == UNIVERSAL_FEATURE_SCHEMA:
        deck_ids, deck_numeric = _universal_deck_context(observation, player, config)
        entity_ids, entity_numeric = _entity_context(observation, player, opponent, config)
        history_ids, history_numeric = _universal_history(observation, config)
        extra = {
            "deck_card_ids": deck_ids,
            "deck_card_numeric": deck_numeric,
            "entity_card_ids": entity_ids,
            "entity_numeric": entity_numeric,
            "history_card_ids": history_ids,
            "history_numeric": history_numeric,
            "expert_ids": _as_int(observation.get("rl_expert_id"), 1),
        }
    ...
    for index, option in enumerate(options[: config.max_candidates]):
        ...
        uses_visible_card_resolution = config.schema_version in {
            FEATURE_SCHEMA_VERSION,
            UNIVERSAL_FEATURE_SCHEMA,
        }
        card_id = (
            _visible_option_card_id(option, select, current, your_index)
            if uses_visible_card_resolution
        ...
        if uses_visible_card_resolution and target_id is None and option_type == 13:
        ...
        if config.schema_version == UNIVERSAL_FEATURE_SCHEMA:
            candidate_values.extend(_card_metadata(observation, card_id))
        action_numeric.append(candidate_values)
