...
class MarnieIDOnlyProbabilityEnsembleRuntime:
    """Greedy pointer decoder that mixes two policies in probability space."""

    def __init__(
        self,
        checkpoint_left: str | Path,
        checkpoint_right: str | Path,
        deck: list[int],
        right_weight: float,
    ):
        ...
        try:
            ...
            pass
        left = torch.load(checkpoint_left, map_location="cpu", weights_only=True)
        right = torch.load(checkpoint_right, map_location="cpu", weights_only=True)
        if left["model_config"] != right["model_config"]:
            raise ValueError("ensemble checkpoints use different model configurations")
        config = ModelConfig(**left["model_config"])
        self.left_model = IDOnlyPointerPolicy(config)
        self.right_model = IDOnlyPointerPolicy(config)
        self.left_model.load_state_dict(left["state_dict"], strict=True)
        self.right_model.load_state_dict(right["state_dict"], strict=True)
        self.left_model.eval()
        self.right_model.eval()
        if len(deck) != 60:
            raise ValueError(f"registered deck must contain 60 cards, got {len(deck)}")
        if not 0.0 <= float(right_weight) <= 1.0:
            raise ValueError(f"right_weight must be in [0, 1], got {right_weight}")
    ...
    @staticmethod
    def _step_logits(model, option_values, keys, hidden, available, chosen, min_count, step):
        pointer = (
    ...
    def decode(self, obs: dict[str, Any]) -> list[int]:
        ...
        config = self.left_model.config
        if not isinstance(options, list) or not options or len(options) > config.max_options:
            return []
        option_count = len(options)
        min_count = max(0, as_int(select.get("minCount")))
        max_count = max(min_count, as_int(select.get("maxCount"), option_count))
        if min_count > option_count or min_count > config.max_action_steps:
            return []
        max_count = min(max_count, option_count, config.max_action_steps)
        ...
        with torch.inference_mode():
            left_state, left_options = self.left_model.encode(batch)
            right_state, right_options = self.right_model.encode(batch)
            left_keys = self.left_model.pointer_key(left_options)
            right_keys = self.right_model.pointer_key(right_options)
            left_hidden = torch.tanh(self.left_model.decoder_init(left_state))
            right_hidden = torch.tanh(self.right_model.decoder_init(right_state))
            available = batch["option_mask"].clone()
            chosen = torch.zeros((1, option_count), dtype=torch.bool)
            action: list[int] = []
            for step in range(max_count):
                left_logits = self._step_logits(
                    self.left_model,
                    left_options,
                    left_keys,
                    left_hidden,
                ...
                right_logits = self._step_logits(
                    self.right_model,
                    right_options,
                    right_keys,
                    right_hidden,
                ...
                probabilities = (
                    (1.0 - self.right_weight) * torch.softmax(left_logits, dim=1)
                    + self.right_weight * torch.softmax(right_logits, dim=1)
                )
                choice = int(torch.argmax(probabilities, dim=1).item())
                if choice == option_count:
                    break
                action.append(choice)
                chosen[0, choice] = True
                left_hidden = self.left_model.decoder(left_options[:, choice], left_hidden)
                right_hidden = self.right_model.decoder(right_options[:, choice], right_hidden)
        return action
