...
for _p in (os.path.join(PACKAGE_ROOT, "alak_w.json"),
           os.path.join(PACKAGE_ROOT, "alak_evo2_weights.json"),
...
file_path = os.path.join(PACKAGE_ROOT, "deck.csv")
...
class _EpisodeRuntime:
    """Small mutable state scoped to one episode and both player indexes.

    The search layer calls the heuristic for simulated states belonging to
    either player.  Keeping usage flags per player, and cloning this object for
    every search branch, prevents a simulated ability from leaking into the
    live opponent episode (or into another episode handled by the same actor).
    """

    __slots__ = ("last_turn", "used")
    ...
    def clone(self):
        ...
        other.last_turn = self.last_turn.copy()
        other.used = {player: flags.copy() for player, flags in self.used.items()}
        return other

    def sync(self, state):
        player = int(getattr(state, "yourIndex", 0))
        turn = int(getattr(state, "turn", 0))
        if self.last_turn.get(player) != turn:
            self.last_turn[player] = turn
            self.used[player] = set()
        return player

    def is_used(self, player, card_id):
        return int(card_id) in self.used.setdefault(int(player), set())

    def mark_used(self, player, card_id):
        self.used.setdefault(int(player), set()).add(int(card_id))


# Explicit metadata is injected by the PPO opponent actor when available.  A
# bounded registry keeps interleaved episodes isolated without retaining every
# game ever seen by a long-lived worker.  Legacy callers that do not provide
# metadata use a single sequential context and remain interface-compatible.
_EPISODE_RUNTIMES = OrderedDict()
_LEGACY_RUNTIME = _EpisodeRuntime()
_MAX_EPISODE_RUNTIMES = 4096
_ACTIVE_EPISODE_ID = None


def _metadata_value(obs_dict, names):
    for name in names:
        value = obs_dict.get(name)
...
def _stable_state_digest(obs_dict):
    """Return a fast, deterministic digest for legacy observations."""
    search_input = obs_dict.get("search_begin_input")
    if search_input is not None:
        if isinstance(search_input, bytes):
            payload = search_input
        else:
            payload = str(search_input).encode("utf-8", "replace")
    else:
        # Do not depend on dict insertion order or Python's randomized hash.
        compact = {
            "current": obs_dict.get("current"),
...
def _episode_context(obs_dict, *, reset=False):
    """Get (episode_id, decision_idx, runtime, state_digest).

    `_opponent_episode_id` and `_opponent_decision_idx` are private transport
    fields; the public agent also accepts the non-prefixed spellings for
    offline callers.  With no metadata, turn/action count gives a stable
    decision index and the state digest keeps determinization repeatable.
    """
    global _LEGACY_RUNTIME, _ACTIVE_EPISODE_ID
    episode_value = _metadata_value(
        obs_dict,
        ("_opponent_episode_id", "episode_id", "episodeId", "game_id", "gameId"),
    )
    explicit = episode_value is not None
    episode_id = str(episode_value) if explicit else "__legacy__"
    if reset and explicit:
        _EPISODE_RUNTIMES.pop(episode_id, None)
    if explicit:
        runtime = _EPISODE_RUNTIMES.get(episode_id)
        if runtime is None:
            runtime = _EpisodeRuntime()
            _EPISODE_RUNTIMES[episode_id] = runtime
        else:
            _EPISODE_RUNTIMES.move_to_end(episode_id)
        while len(_EPISODE_RUNTIMES) > _MAX_EPISODE_RUNTIMES:
            _EPISODE_RUNTIMES.popitem(last=False)
    else:
        if reset or (_ACTIVE_EPISODE_ID is not None and _ACTIVE_EPISODE_ID != episode_id):
            _LEGACY_RUNTIME = _EpisodeRuntime()
        runtime = _LEGACY_RUNTIME
    _ACTIVE_EPISODE_ID = episode_id

    decision_value = _metadata_value(
        obs_dict,
        ("_opponent_decision_idx", "decision_idx", "decisionIndex"),
    )
    if decision_value is not None:
        ...
        try:
            ...
            decision_idx = turn * 1024 + turn_action
        except (TypeError, ValueError):
            decision_idx = 0
    if explicit and decision_idx == 0:
        runtime = _EpisodeRuntime()
        _EPISODE_RUNTIMES[episode_id] = runtime
        _EPISODE_RUNTIMES.move_to_end(episode_id)
    state_digest = "" if explicit else _stable_state_digest(obs_dict)
    return episode_id, decision_idx, runtime, state_digest


def _derive_search_seed(episode_id, decision_idx, state_digest=""):
    """Stable per-decision seed; never consumes process-global RNG state."""
    master = os.environ.get("ALAK_SEARCH_SEED", "271828")
    payload = f"{master}|{episode_id}|{int(decision_idx)}|{state_digest}".encode("utf-8")
...
def heuristic_scores(obs, runtime=None):
    state = obs.current
    select = obs.select
    ...
    runtime = runtime if runtime is not None else _EpisodeRuntime()
    runtime_player = runtime.sync(state)
    ability_used_dudunsparce = runtime.is_used(runtime_player, Dudunsparce)
    ability_used_fezandipiti = runtime.is_used(runtime_player, Fezandipiti_ex)
...
def _post_pick(obs, picked_idx, runtime=None):
    if runtime is None:
        return
    sel = obs.select
    ...
    if o.type == OptionType.ABILITY:
        ...
        if card is not None and card.id in (Dudunsparce, Fezandipiti_ex):
            player = runtime.sync(obs.current)
            runtime.mark_used(player, card.id)


def _agent_impl(obs_dict, runtime=None):
    obs = to_observation_class(obs_dict)
    if obs.select is None: return my_deck
    scores = heuristic_scores(obs, runtime)
    desc = sorted(range(len(scores)), key=lambda i: scores[i], reverse=True)
    if desc: _post_pick(obs, desc[0], runtime)
    return desc[:obs.select.maxCount]


def _heuristic_agent(obs_dict, runtime=None):
    try:
        return _agent_impl(obs_dict, runtime)
    except Exception:
        try:
            obs = to_observation_class(obs_dict)
            if obs.select is None: return my_deck
...
# The public agent uses 3x3 branching and an 0.80s soft budget.  Wall-clock
# cutoffs make the selected action depend on CPU contention, so this arena
# profile instead caps native expansions.  The wall clock is only a loose
# watchdog for a wedged native search call.
N_DET = 2                  # determinizations
K_OPP = 2                  # opponent branching at ply-2 MAIN
MAX_SUBSTEPS = 28          # per greedy-complete rollout
SEARCH_NODE_BUDGET = max(1, int(os.environ.get("ALAK_SEARCH_NODE_BUDGET", "96")))
SEARCH_WATCHDOG_S = max(1.0, float(os.environ.get("ALAK_SEARCH_WATCHDOG_S", "5.0")))
# Kept as a compatibility alias for notebooks importing this constant.  It is
# no longer a soft search budget and never controls normal expansion count.
TIME_BUDGET_S = SEARCH_WATCHDOG_S
SEARCH_MAX_OPTS = 20
SEARCH_MAX_CAND = 5
...
class _SearchBudgetSlice:
    """Equal per-(determinization, candidate) share of a global budget."""

    __slots__ = ("shared", "limit", "used")
    ...
    def consume(self):
        if self.used >= self.limit or not self.shared.consume():
            return False
        self.used += 1
        return True
...
def _watchdog_expired(deadline):
    return deadline is not None and time.monotonic() > deadline


def _budgeted_search_step(sid, choice, budget):
    if budget is not None and not budget.consume():
        return None
    return search_step(sid, choice)


def _report_search_stats():
    global _stats_reported
    if _stats_reported:
        return
    if os.environ.get("ALAK_SEARCH_STATS") == "1" and (
            _stats["agent_calls"] or _stats["calls"]):
        _stats_reported = True
        avg = _stats["ms"] / _stats["calls"] if _stats["calls"] else 0.0
        avg_nodes = _stats["nodes"] / _stats["calls"] if _stats["calls"] else 0.0
        complete_pair_rate = (_stats["pair_completed"] / _stats["pair_attempted"]
                              if _stats["pair_attempted"] else 0.0)
        considered_rate = _stats["considered"] / _stats["calls"]
        override_rate = (_stats["overrides"] / _stats["considered"]
                         if _stats["considered"] else 0.0)
        eligible_rate = (_stats["search_eligible"] / _stats["agent_calls"]
                         if _stats["agent_calls"] else 0.0)
        attempt_rate = (_stats["calls"] / _stats["search_eligible"]
                        if _stats["search_eligible"] else 0.0)
        print("[alak_det] agent_calls=%d search_eligible=%d search_calls=%d "
              "search_begin_ok=%d search_begin_fail=%d avg_ms=%.1f "
              "overrides=%d considered=%d fail=%d "
              "rollout_cap=%d budget_exhausted=%d watchdog=%d avg_nodes=%.1f "
              "pair_attempted=%d pair_completed=%d pair_truncated=%d "
              "slice_exhausted=%d pair_node_budget=%.1f complete_pair_rate=%.3f "
              "eligible_rate=%.3f attempt_rate=%.3f considered_rate=%.3f "
              "override_rate=%.3f" %
              (_stats["agent_calls"], _stats["search_eligible"], _stats["calls"],
               _stats["search_begin_ok"], _stats["search_begin_fail"], avg,
               _stats["overrides"], _stats["considered"],
               _stats["fail"], _stats["rollout_cap"], _stats["budget_exhausted"],
               _stats["watchdog"], avg_nodes,
               _stats["pair_attempted"], _stats["pair_completed"],
               _stats["pair_truncated"], _stats["slice_exhausted"],
               _stats["pair_node_budget_sum"] / _stats["calls"],
               complete_pair_rate, eligible_rate, attempt_rate,
               considered_rate, override_rate))


def _report_search_decision_stats(pair_attempted, pair_completed, pair_truncated,
                                  slice_exhausted, pair_node_budget, nodes,
                                  node_limit):
    if os.environ.get("ALAK_SEARCH_DECISION_STATS") != "1":
        return
    complete_pair_rate = pair_completed / pair_attempted if pair_attempted else 0.0
    print("[alak_det_decision] pair_attempted=%d pair_completed=%d "
          "pair_truncated=%d slice_exhausted=%d pair_node_budget=%d "
          "complete_pair_rate=%.3f nodes=%d/%d" %
          (pair_attempted, pair_completed, pair_truncated, slice_exhausted,
           pair_node_budget, complete_pair_rate, nodes, node_limit))


atexit.register(_report_search_stats)

# ---- archetype templates for opponent belief ----
_TEMPLATES = []  # list[(name, Counter, list[int])]
for _d in (
    os.path.join(PACKAGE_ROOT, "top20_decks"),
...
def _sample_hidden(state, me_i, rng=None):
...
def _make_hidden_sampler(state, me_i, rng=None):
    """Precompute static belief pools for one root decision.

    The old implementation rebuilt visible counters, matched every top20
    archetype, and expanded both 60-card pools for every determinization.  A
    determinization only needs fresh shuffles; all of that bookkeeping is
    invariant while the root search is running.  Keeping the same pool and
    only copying/shuffling it cuts Python overhead without changing the
    ...
    own_seen = _my_visible(state, me_i)
    own_remaining = []
    for cid, n in Counter(my_deck).items():
        own_remaining.extend([cid] * max(0, n - own_seen.get(cid, 0)))
    n_prize_hidden = sum(1 for c in me.prize if c is None)
    own_need = me.deckCount + n_prize_hidden
    if len(own_remaining) < own_need:
        own_remaining += [DUMMY_ENERGY] * (own_need - len(own_remaining))
    ...
    if tpl is not None:
        ...
        op_pool = []
        for cid, n in cnt.items():
            op_pool.extend([cid] * max(0, n - op_seen.get(cid, 0)))
    else:
        etop = max(etype.items(), key=lambda x: x[1])[0] if etype else 7
        energy_id = _BASIC_ENERGY.get(etop, 7)
        top_card = max(op_seen.items(), key=lambda x: x[1])[0] if op_seen else None
        op_pool = ([top_card] * 30 if top_card else []) + [energy_id] * 30
        op_pool += [DUMMY_BASIC] * 8
    n_op_prize_hidden = sum(1 for c in op.prize if c is None)
    op_need = op.deckCount + n_op_prize_hidden + op.handCount
    if len(op_pool) < op_need:
        op_pool += [DUMMY_ENERGY] * (op_need - len(op_pool))

    def sample():
        # Copies are required because search_begin owns the arrays passed to
        # it and the next determinization must receive a fresh permutation.
        remain = own_remaining[:]
        rng.shuffle(remain)
        your_deck = remain[:me.deckCount]
        fill = iter(remain[me.deckCount:own_need])
        your_prize = [c.id if c is not None else next(fill, DUMMY_ENERGY)
                      for c in me.prize]
...
def _greedy_pick(obs, runtime=None):
    ...
    if n == 0:
        return [], []
    try:
        sc = heuristic_scores(obs, runtime)
    except Exception:
        sc = list(range(n, 0, -1))
...
def _greedy_complete_turn(sid, cur, owner, watchdog_deadline=None,
                          budget=None, runtime=None):
    """Step greedily while it's `owner`'s turn. Stops at hand-off / terminal /
    fixed expansion budget. The deadline is only a watchdog. Returns
    (sid, obs, complete), where budget/watchdog/cap truncation is incomplete."""
    for _ in range(MAX_SUBSTEPS):
        ...
        if cs.yourIndex != owner or cur.select is None:
            return sid, cur, True
        if _watchdog_expired(watchdog_deadline) or (budget is not None and budget.exhausted):
            return sid, cur, False
        choice, _ = _greedy_pick(cur, runtime)
        if not choice:
            return sid, cur, False
        try:
            _post_pick(cur, choice[0], runtime)
            ss = _budgeted_search_step(sid, choice, budget)
            if ss is None:
                return sid, cur, False
        except Exception:
            return sid, cur, False
    ...
    cs = cur.current
    if (cs is None or (cs.result is not None and cs.result >= 0)
            or cs.yourIndex != owner or cur.select is None):
        return sid, cur, True
    _stats["rollout_cap"] += 1
    return sid, cur, False


def _advance_forced(sid, cur, owner, watchdog_deadline=None, limit=8,
                    budget=None, runtime=None):
    """Resolve non-MAIN sub-selects for `owner` (e.g. promote after KO,
    setup active) so we reach a MAIN decision or hand-off. Returns
    (sid, obs, complete); budget/watchdog/limit truncation is incomplete."""
    for _ in range(limit):
        ...
        if _watchdog_expired(watchdog_deadline) or (budget is not None and budget.exhausted):
            return sid, cur, False
        ch, _ = _greedy_pick(cur, runtime)
        if not ch:
            return sid, cur, False
        try:
            _post_pick(cur, ch[0], runtime)
            ss = _budgeted_search_step(sid, ch, budget)
            if ss is None:
                return sid, cur, False
        except Exception:
            return sid, cur, False
        sid, cur = ss.searchId, ss.observation
    cs = cur.current
    complete = (cs is None or cur.select is None or cs.yourIndex != owner
                or cur.select.context == SelectContext.MAIN
                or (cs.result is not None and cs.result >= 0))
    return sid, cur, complete


# ==================== 2-ply minimax ====================

def _search_decide(obs, base_order, base_scores, *, runtime=None,
                   episode_id="__legacy__", decision_idx=0, state_digest=""):
    global _search_reported
    if not (USE_SEARCH and _SEARCH_IMPORT_OK):
        return None
    st = obs.current
    sel = obs.select
    ...
    if getattr(obs, "search_begin_input", None) is None:
        if not _search_reported:
            print("[alak_det] search_begin_input=None -> heuristic fallback", file=sys.stderr)
    ...
    t0 = time.monotonic()
    _stats["calls"] += 1
    watchdog_deadline = t0 + SEARCH_WATCHDOG_S
    budget = _SearchBudget(SEARCH_NODE_BUDGET)
    decision_rng = random.Random(
        _derive_search_seed(episode_id, decision_idx, state_digest)
    )
    acc = {i: 0.0 for i in cand}
    n_eval = {i: 0 for i in cand}
    eval_dets = {i: set() for i in cand}
    # A shared sequential counter lets early candidates consume the entire
    # budget. Reserve the same hard allowance for every (det, candidate)
    # pair. At most pair_count-1 remainder nodes are deliberately left unused
    # rather than introducing candidate-order-dependent extra work.
    pair_count = N_DET * len(cand)
    pair_node_budget = budget.limit // pair_count if pair_count else 0
    pair_attempted = 0
    pair_completed = 0
    pair_truncated = 0
    slice_exhausted = 0

    def record_pair(complete, branch_budget):
        nonlocal pair_completed, pair_truncated, slice_exhausted
        if complete:
            pair_completed += 1
        else:
            pair_truncated += 1
            if branch_budget.exhausted:
                slice_exhausted += 1

    began = False
    watchdog_hit = False
    # Build the belief pools once.  This is deliberately outside the
    # determinization loop; only the shuffle should vary between samples.
    sample_hidden = _make_hidden_sampler(st, me_i, decision_rng)
    try:
        for det in range(N_DET):
            if _watchdog_expired(watchdog_deadline):
                watchdog_hit = True
                break
            if budget.exhausted:
                break
            ...
            try:
                ...
                began = True
                _stats["search_begin_ok"] += 1
            except Exception as e:
                _stats["search_begin_fail"] += 1
                _stats["fail"] += 1
                _stats["ms"] += (time.monotonic() - t0) * 1000.0
                _stats["nodes"] += budget.used
                if not _search_reported:
                    print(f"[alak_det] search_begin failed: {e!r} -> heuristic fallback", file=sys.stderr)
                    _search_reported = True
                return None
            ...
            for idx in cand:
                ...
                if budget.exhausted:
                    break
                branch_budget = _SearchBudgetSlice(budget, pair_node_budget)
                pair_attempted += 1
                branch_runtime = runtime.clone() if runtime is not None else _EpisodeRuntime()
                # ply 1: take idx, greedy-complete our turn
                try:
                    _post_pick(obs, idx, branch_runtime)
                    ss = _budgeted_search_step(root_sid, [idx], branch_budget)
                    if ss is None:
                        record_pair(False, branch_budget)
                        continue
                except Exception:
                    record_pair(False, branch_budget)
                    continue
                sid1, cur = ss.searchId, ss.observation
                sid1, cur, complete = _greedy_complete_turn(
                    sid1, cur, me_i, watchdog_deadline,
                    budget=branch_budget, runtime=branch_runtime,
                )
                if _watchdog_expired(watchdog_deadline):
                    watchdog_hit = True
                    record_pair(False, branch_budget)
                ...
                if (cs is None or (cs.result is not None and cs.result >= 0)
                    ...
                    n_eval[idx] += 1
                    eval_dets[idx].add(det)
                    record_pair(True, branch_budget)
                    continue
                # advance opponent's forced sub-selects to MAIN
                sid1, cur, complete = _advance_forced(
                    sid1, cur, 1 - me_i, watchdog_deadline,
                    budget=branch_budget, runtime=branch_runtime,
                )
                if not complete:
                    record_pair(False, branch_budget)
                ...
                _, op_order = _greedy_pick(cur, branch_runtime)
                worst = None
                opponent_count = min(K_OPP, len(op_order))
                opponent_complete = True
                for k in range(opponent_count):
                    if _watchdog_expired(watchdog_deadline):
                        watchdog_hit = True
                    ...
                    if branch_budget.exhausted:
                        opponent_complete = False
                        break
                    opponent_runtime = branch_runtime.clone()
                    try:
                        _post_pick(cur, op_order[k], opponent_runtime)
                        ss2 = _budgeted_search_step(sid1, [op_order[k]], branch_budget)
                        if ss2 is None:
                            opponent_complete = False
                            break
                    except Exception:
                    ...
                    sid2, cur2, complete = _greedy_complete_turn(
                        sid2, cur2, 1 - me_i, watchdog_deadline,
                        budget=branch_budget, runtime=opponent_runtime,
                    )
                    if not complete:
                        opponent_complete = False
                        break
                    # resolve our forced replacement (promote after KO)
                    sid2, cur2, complete = _advance_forced(
                        sid2, cur2, me_i, watchdog_deadline, limit=6,
                        budget=branch_budget, runtime=opponent_runtime,
                    )
                    if not complete:
                        opponent_complete = False
        ...
        _stats["nodes"] += budget.used
        _stats["pair_attempted"] += pair_attempted
        _stats["pair_completed"] += pair_completed
        _stats["pair_truncated"] += pair_truncated
        _stats["slice_exhausted"] += slice_exhausted
        _stats["pair_node_budget_sum"] += pair_node_budget
        if budget.exhausted or slice_exhausted:
            _stats["budget_exhausted"] += 1
        _report_search_decision_stats(
            pair_attempted,
            pair_completed,
            pair_truncated,
            slice_exhausted,
        ...
        if watchdog_hit or _watchdog_expired(watchdog_deadline):
            _stats["watchdog"] += 1
            return None
        # Pick over exactly the same determinization samples as heur_top;
        # matching only the count can compare different hidden states.
        n_top = n_eval.get(heur_top, 0)
        if n_top == 0:
            return None
        top_dets = eval_dets[heur_top]
        evaluated = [i for i in cand if eval_dets[i] == top_dets]
        ...
        _stats["fail"] += 1
        if not _search_reported:
            print(f"[alak_det] search crashed: {e!r} -> heuristic fallback", file=sys.stderr)
            _search_reported = True
        return None
    finally:
        if began:
...
def agent(obs_dict):
    _stats["agent_calls"] += 1
    if not isinstance(obs_dict, dict) or obs_dict.get("select") is None:
        return my_deck
    # Process episode metadata before parsing so an explicit decision zero
    # resets stale per-episode state even when the observation is malformed.
    episode_id, decision_idx, runtime, state_digest = _episode_context(obs_dict)
    try:
        obs = to_observation_class(obs_dict)
    except Exception:
        return _heuristic_agent(obs_dict, runtime)
    if obs.select is None:
        _episode_context(obs_dict, reset=True)
        return my_deck

    sel = obs.select
    # Parse and score the live state once. Search rollouts receive already
    # materialized Observation objects and use branch-local runtime clones.
    try:
        base_scores = heuristic_scores(obs, runtime)
        n = len(sel.option)
        base_order = sorted(range(n), key=lambda i: base_scores[i], reverse=True)
    except Exception:
        return _heuristic_agent(obs_dict, runtime)
    fallback = base_order[:sel.maxCount]
    if sel.context != SelectContext.MAIN:
        if fallback:
            _post_pick(obs, fallback[0], runtime)
        return fallback

    _stats["search_eligible"] += 1
    pick = _search_decide(
        obs,
        base_order,
        base_scores,
