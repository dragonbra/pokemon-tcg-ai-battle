"""Kaggle PTCG AI Battle Challenge — Mega Lucario ex baseline.

Based on kojimar's LB-1076 agent (exp14), adapted for our three-layer repo structure.
Key features:
  - AttackPlan dynamic planning each turn
  - Crustle wall guard (skip ex attacks into Crustle)
  - Water deck detection (adjust Hero Cape priority)
  - Low-deck protection (stop drawing when deckCount <= 10)
  - Crash-safe wrapper (never crash → never auto-lose)
"""
...
class AttackPlan:
    __slots__ = ("attacker", "target", "attack_index", "remain_hp", "needs_energy")
...
def target_score(pokemon: Pokemon) -> int:
    ...
    if pokemon.id == C.SNOVER:
        score += 400 if pokemon.hp <= 70 else 150
...
class LucarioPolicy:
            ...
            for attack_index in range(2):
                ...
                for target_index, op_pokemon in enumerate(self._opponent_board()):
                    ...
                    if (
                        ...
                        and (card_table[my_pokemon.id].ex or card_table[my_pokemon.id].megaEx)
                    ):
                        continue
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id in {C.MAKUHITA, C.HARIYAMA}:
            ...
            if self._opponent_is_crustle_wall():
                score += 500 if energy_count < 3 else 30
            else:
    ...
    def _score_option(self, option) -> float:
        ...
        if option.type == OptionType.ATTACK:
            if (
                ...
                and (card_table[self.me.active[0].id].ex or card_table[self.me.active[0].id].megaEx)
    ...
    def _score_setup_bench(self, card: Pokemon | Card) -> int:
        ...
        if card.id == C.LUNATONE:
            return 4
        if isinstance(card, (Card, Pokemon)):
    ...
    def _score_discard(self, card: Pokemon | Card) -> float:
        ...
        if cid == C.BASIC_FIGHTING_ENERGY:
            return 100
        if data and data.cardType in {CardType.ITEM, CardType.TOOL, CardType.STADIUM}:
            return 60
        if data and data.basic and self.field_counts.get(cid, 0) >= 1:
            return 50
        if data and data.cardType == CardType.SUPPORTER:
            return 40
        if cid in {C.RIOLU, C.MEGA_LUCARIO_EX, C.HARIYAMA, C.MAKUHITA}:
            return 5
        return 30
