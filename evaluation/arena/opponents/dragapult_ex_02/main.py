...
file_path = Path(__file__).resolve().parent / "deck.csv"
...
def _policy_agent(obs_dict: dict) -> list[int]:
    ...
    active_pokemon = next((card for card in my_state.active if card != None), None)
    target_pokemon = next((card for card in op_state.active if card != None), None)
    kieran_damage_mode = (
        active_pokemon != None
        and active_pokemon.id == Dragapult_ex
        and len(active_pokemon.energies) >= 2
        and target_pokemon != None
        and (card_table[target_pokemon.id].ex or card_table[target_pokemon.id].name.endswith(" V"))
        and not no_damage_dex(target_pokemon.id)
        and damage < target_pokemon.hp <= damage + 30
    )
    kieran_switch_mode = False
    ...
    def hand_score(id: int, ignore_count: bool):
        ...
        if id == Dreepy:
            ...
            if kieran_damage_mode:
                score = 70000
            elif kieran_switch_mode:
                score = 40000
        elif id == Crispin:
    ...
    if context == SelectContext.MAIN:
        ...
        kieran_switch_mode = not can_main_attack and bench_attacker and not can_switch
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            if context == SelectContext.FIRST_EFFECT and effect_card_id == Kieran:
                # Kieran's first mode is the switch; its second is the damage boost.
                score = 1 if kieran_switch_mode else -1
            ...
            if context == SelectContext.FIRST_EFFECT and effect_card_id == Kieran:
                score = 1 if not kieran_switch_mode else -1
...
def _normalize_action(obs_dict: dict, action) -> list[int]:
    select = obs_dict.get("select") if isinstance(obs_dict, dict) else None
    if not select:
        return my_deck
    options = select.get("option") or []
    ...
    cleaned = []
    for item in action:
        if isinstance(item, int) and 0 <= item < len(options) and item not in cleaned:
            cleaned.append(item)
...
def agent(obs_dict: dict) -> list[int]:
    try:
        return _normalize_action(obs_dict, _policy_agent(obs_dict))
    except Exception:
