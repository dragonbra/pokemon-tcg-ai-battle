...
def heuristic_agent(obs):
    ...
    op_visible_ids = {
        card.id
        for card in op_state.active + op_state.bench + op_state.discard
        if card is not None
    }
    ...
    if facing_great_tusk:
        ...
        low_deck = getattr(my_state, "deckCount", 999) <= max(
            LOW_DECK_COUNT,
            great_tusk_deck_floor,
        )
    ...
    for o in select.option:
        ...
        if o.type == OptionType.NUMBER:
            ...
            score = 0 if context == SelectContext.IS_FIRST else 1
