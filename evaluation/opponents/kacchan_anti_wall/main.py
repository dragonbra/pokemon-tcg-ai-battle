"""Kacchan anti-wall Lucario opponent - extracted from kacchan_anti_wall notebook"""
...
class LucarioPolicy:
    def __init__(self, obs: Observation):
        ...
        self.facing_crustle = any(p is not None and p.id == C.CRUSTLE for p in (self.opponent.active + self.opponent.bench))
    ...
    def _energy_target_score(self, pokemon: Pokemon, active: bool) -> int:
        ...
        if pokemon.id in {C.MAKUHITA, C.HARIYAMA}:
            ...
            score += 900 if (self.facing_crustle and energy_count < 3) else 0
            ...
            score -= 500 if self.facing_crustle else 0
        return score
    ...
    def _score_to_hand(self, card: Pokemon | Card) -> float:
        ...
        if card.id == C.MAKUHITA:
            ...
            score += 250 if self.facing_crustle else 0
        elif card.id == C.MAKUHITA and self.facing_crustle:
            score += 200
...
def _legal_fallback(select) -> list[int]:
    ...
    if n == 0 or select.maxCount <= 0:
        return []
    lo = max(select.minCount, 0)
    k = min(max(lo, 1), select.maxCount, n)
    return list(range(k))
...
def agent(obs_dict: dict) -> list[int]:
    ...
    try:
        ...
        st = obs.current
        if (st is not None and select.context == SelectContext.MAIN
                and st.turnActionCount >= MAX_TURN_ACTIONS):
            for i, o in enumerate(select.option):
                if o.type == OptionType.END:
                    return [i]

        result = LucarioPolicy(obs).choose()
        n = len(select.option)
        result = [i for i in result if isinstance(i, int) and 0 <= i < n]
        seen = set(); dedup = []
        for i in result:
            if i not in seen:
