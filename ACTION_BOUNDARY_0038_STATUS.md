...
0038 第一版、CUDA 接入与 50 完整回合防挂死截断适配已完成，当前停止在启动门前，等待用户明确确认后才能启动 V7。
...
- 下一正式版本已预留为 `V7_turn_limit_cuda_fresh_rl`，但未启动。CPU 与 CUDA 都以 `full_round_draw_limit=50` 为单一合同；CUDA 映射到 turn index 99。触发后 Episode 为有效 `turn_limit_draw`，终局 reward 0，最后真实决策 transition 为 `done=true, next_value=0`，不伪造 winner，并单独记录 `rollout/cuda_turn_limit_draws`。
