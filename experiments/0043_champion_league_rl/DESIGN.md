...
- Archived: `V3_generalist_focal_001_067` inherited V2 U207, completed source U207, emitted healthy U208, then lost its CUDA driver context during the next PPO update. `V4_generalist_focal_001_067_u208_restart` reproduced the same driver failure during PPO before emitting any new checkpoint. Both failed runs remain immutable evidence; neither may receive appended training.
- Running: `V5_generalist_focal_001_067_u208_micro512` inherits the complete V3 U208 model-only checkpoint with a fresh optimizer and newly collected on-policy data. It keeps logical minibatch 2,048, three epochs, all loss/LR/sample semantics, G1/update-0 reference anchor, 256-game rollout, CUDA Engine 2.0 and complete frozen opponents unchanged. Only the physical forward/backward graph changed from 1,024 to 512 rows, accumulating four physical graphs per logical 2,048 optimizer step. Its stable online W&B run is `0043-v5-generalist-focal-001-067-u208-micro512`.
...
- Generalist readiness evidence covers 67 schedules / 17,152 focal lanes with exact long-run equality. V3 source U207 completed 127-127-2 and emitted U208 with behavior KL 0.000276 and reference KL 0.01233. After the reproducible driver failures near the old ~15.1 GiB PPO peak, V5 micro512 completed all 27/27 U209 optimizer steps at roughly 11.2 GiB peak, emitted U209, recorded source-U208 rollout 126-129-1, behavior KL 0.000172, reference KL 0.01131, clip fraction 0.00492, zero routing failure and zero feature D2H, and continued into source U209. These rollout values are training health diagnostics, not Frozen checkpoint-strength evidence.
- PFSP persistence now occurs only after PPO and checkpoint success, so a failed PPO rollout cannot bias a future curriculum. Because V3/V4 predated this atomic boundary and their failed source-U208 attempts had already touched run-local PFSP files, V5 conservatively starts from the V2 final committed PFSP state. This excludes one successful V3 U207 observation batch rather than double-counting failed U208 batches; C020 sampling weights remain frozen within their ten-update window.

V1–V4 are immutable stopped/failed runs. Formal selected-checkpoint reports are indexed under `experiments/0043_champion_league_rl/evaluation/`. V5 owns its separate formal version root and W&B identity and is running as `ptcg-0043-v5-generalist.service`.
