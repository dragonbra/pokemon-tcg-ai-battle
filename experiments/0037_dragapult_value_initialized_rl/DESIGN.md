...
The auxiliary opponent-archetype and final-diff heads from supervised training remain frozen provenance tensors and are not evaluated or optimized during PPO. V3/V4 retain every original Encoder tensor as a frozen base and add rank-8, alpha-16 LoRA to board-Encoder layers 0, 1 and 3. Each selected layer adapts attention QKV, attention output and both FFN matrices; all LoRA output factors start at zero, so checkpoint 0 is functionally identical to the source actor. The actor contains four board, one event and two option `TransformerEncoderLayer` blocks (seven total). V4 additionally tunes the affine weight/bias of every LayerNorm below the state and option encoders; the shared prototype encoder remains frozen and cacheable.

The same adapted focal hidden features feed both Action Decoder and Value Head. Policy and Value losses therefore both backpropagate through focal LoRA and, in V4, LayerNorm. The separately loaded Frozen-0806 opponent actor has no adapters and receives no gradient. V3/V4 trainable parameters are the full Action Decoder, 107,520 LoRA parameters, Value queries/blocks/final norm/value scalar head, and—for V4 only—12,160 LayerNorm parameters. Actor decoder, Value, LoRA and LayerNorm learning rates are respectively `1e-5`, `1e-4`, `3e-5` and `1e-5`.
...
V3 `V3_lora_r8_eval5_50u` is retained as a zero-update infrastructure failure: its detached launcher was externally terminated after creating empty version directories and before config/W&B initialization. Two fair 50-update arms therefore start at V4/V5 from the same friend-0806 actor and 0036 V2 epoch-5 critic, use identical update schedules and evaluate on the same frozen greedy suite every five updates. V4 `V4_lora_r8_eval5_50u` trains Decoder + Value + LoRA. V5 `V5_lora_r8_layernorm_eval5_50u` adds state/option LayerNorm tuning. Neither arm uses explicit PBRS: terminal reward remains the only environment reward, while adapted `V(s)` supplies the TD/GAE potential-like baseline. Selection or continuation to 200 updates requires comparable checkpoint-50 frozen evidence; sampled rollout peaks are not sufficient.
