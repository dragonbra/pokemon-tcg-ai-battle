...
Two fair 50-update arms start from the same friend-0806 actor and 0036 V2 epoch-5 critic, use identical update schedules and evaluate on the same frozen greedy suite every five updates. V3 `V3_lora_r8_eval5_50u` trains Decoder + Value + LoRA. V4 `V4_lora_r8_layernorm_eval5_50u` adds state/option LayerNorm tuning. Neither arm uses explicit PBRS: terminal reward remains the only environment reward, while adapted `V(s)` supplies the TD/GAE potential-like baseline. Selection or continuation to 200 updates requires comparable checkpoint-50 frozen evidence; sampled rollout peaks are not sufficient.
