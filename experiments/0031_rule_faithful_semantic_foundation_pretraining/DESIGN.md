...
BC performs exactly one optimization pass over train and a complete teacher-forced plus greedy validation pass each epoch. CUDA training uses fused AdamW and prefetch depth 4. Teacher loss is dense cross entropy over `[batch, vocabulary, step]` logits with padded targets set to `ignore_index=-100`; its reduction remains the mean over valid teacher steps. Canonical gzip JSON rows are decoded with `orjson` 3.11.5. Formal runs use private W&B project `dragon_bra/pokemon-tcg-policy-learning`; source provenance remains in manifests/audits and never reaches actor forward.
...
The active formal version is `V4_lr5e4_no_early_stop_b512`: learning rate 0.0005, batch and validation batch 512, bfloat16, fused AdamW, dynamic length bucketing, prefetch depth 4, and `early_stopping_patience=0`. Its original 100-batch full-data preflight measured **629.8 decisions/s**, 7.5% data wait, and 8.11/9.89 GB peak CUDA allocated/reserved memory, 11.2% faster than batch 384.

Before continuing from epoch 2, an exact-path profile measured about 401 ms/batch forward, 270 ms/batch backward, 23 ms/batch H2D, 1.3 ms/batch clipping, and 2.7 ms/batch fused AdamW. Replacing prototype advanced indexing with embedding lookup, using dense ignored-target teacher CE, and decoding rows with `orjson` raised a same-period 620.3 decisions/s reference to 721.4 over 150 measured batches (+16.3%) and 706.5 over 300 measured batches (+13.9%). Peak CUDA memory remained approximately 8.32/9.90 GB. Exact epoch-2 validation measured loss delta `5.96e-8`, gradient relative L2 `9.65e-4`, maximum absolute gradient delta `7.63e-6`, finite gradients for all 197 parameter tensors, and maximum BF16 logit delta `9.77e-4`.

Pinned-memory stream prefetch, prefetch depth 8, parallel collation, reduced scalar synchronization, manually expanded GRU, and decoder-only Inductor were not admitted because they were slower, numerically outside the accepted boundary, or unsupported by the formal toolchain. The configured epoch limit is intentionally nonbinding at 1,000,000; training stops only by user decision or an operational fault. W&B run ID is `0031_rule_faithful_semantic_foundation_pretraining--v-cbffa699bd`.

The implementation passes 61 project semantic, model, fixed-bucket, checkpoint, and training tests plus 41 relevant repository experiment/W&B tests. The chronological benchmark artifact is `.tmp/0031_feature_benchmark/run-20260804-v2/feature_benchmark.json`; full-data batch preflights are `.tmp/0031_training_optimization/full_serial_b256.json`, `full_serial_b384.json`, and `full_serial_b512.json`. Full dataset verification is `.tmp/0031_training_optimization/full_dataset_verification.json`; the completed dataset CUDA/fixed-bucket smoke is `.tmp/0031_rule_faithful_bc_smoke/run-c81d419894`. Epoch-2 equivalence and admitted throughput evidence are `.tmp/0031_training_optimization/exact_epoch2_equivalence.json`, `final_candidate_20260805_b512_300.json`, and `orjson_embedding_20260805_b512_150.json`.
