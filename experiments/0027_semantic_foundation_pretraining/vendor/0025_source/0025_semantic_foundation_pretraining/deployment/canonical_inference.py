"""Model-only portable inference for the 0025 canonical semantic policy."""
...
class PortableCanonicalPolicy:
    ...
    def from_checkpoint(
        ...
        if not isinstance(metadata, dict) or metadata.get("arm") != "canonical_semantic":
            raise ValueError("checkpoint is not the canonical_semantic arm")
        ...
        if not isinstance(model_contract, dict) or not isinstance(model_contract.get("config"), dict):
            raise ValueError("checkpoint has no canonical model config")
