...
SCHEMA = importlib.import_module(f"{BASE}.features.canonical.schema")
DATASET = importlib.import_module(f"{BASE}.training.canonical_dataset")
...
class CanonicalDatasetTests(unittest.TestCase):
    ...
    def test_verified_iteration_is_complete_and_actor_is_target_free(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual(set(batch), importlib.import_module(
                f"{BASE}.model.canonical.policy"
            ).EXPECTED_BATCH_KEYS)
