"""Train the 0025 canonical semantic policy on the audited James Cox corpus."""
...
from .model.canonical import (
    ...
    CanonicalSemanticPolicy,
)
from .training.canonical_dataset import CanonicalDecisionDataset
from .training.trainer import train_ablation


DATASET_ROOT = Path(
    "rl_runs/0025_semantic_foundation_pretraining/dataset/"
    "V2_james_cox_raging_bolt_canonical"
)
PROTOTYPES = Path(
...
def _implementation_sha256() -> str:
    ...
    files = [
        ...
        *(package / "features/canonical").glob("*.py"),
        *(package / "model/canonical").glob("*.py"),
        package / "features/prototypes.py",
        package / "knowledge/ledger.py",
        package / "knowledge/state.py",
        package / "training/canonical_dataset.py",
...
def _smoke_paths():
    ...
    root = Path(".tmp/0025_canonical_bc_smoke") / f"run-{uuid.uuid4().hex[:10]}"
...
def main() -> None:
    ...
    parser.add_argument("--version", default="V4_canonical_semantic_foundation")
    ...
    if not args.smoke and not torch.cuda.is_available():
        raise RuntimeError("formal 0025 canonical BC training requires CUDA")
        ...
        os.environ.update(
            {
                ...
                "WANDB_NAME": f"0025 · canonical semantic · {paths.version_name}",
                "WANDB_RUN_GROUP": PROJECT_ID,
                "WANDB_RUN_ID": wandb_run_id(PROJECT_ID, paths.version_name),
                "WANDB_JOB_TYPE": "bc",
                "WANDB_TAGS": "0025,james_cox,raging_bolt,canonical_semantic",
            }
        )

    model = CanonicalSemanticPolicy(model_config, prototypes)
    parameter_count = sum(parameter.numel() for parameter in model.parameters())
    arm_contract = {
        "model": "CanonicalSemanticPolicy",
    ...
    config = {
        "schema_version": "0025_canonical_bc_training_v1",
        ...
        "exact_deck_sha256": EXACT_DECK_SHA256,
    ...
    paths.model_contract.write_text(
        json.dumps(
            {
                "schema_version": "0025_canonical_model_contract_v1",
                "arm": arm_contract,
    ...
    print(
        json.dumps(
            {
                "event": "0025_canonical_bc_preflight",
    ...
    print(
        json.dumps(
            {
                "event": "0025_canonical_bc_complete",
                "run_root": str(paths.run_root),
                "best": summary["best"],
            },
            sort_keys=True,
