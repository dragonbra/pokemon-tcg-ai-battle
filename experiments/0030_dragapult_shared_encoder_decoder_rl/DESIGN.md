...
V3 was stopped at the user's request after update 83 completed. Its original `checkpoint_retention=6` implementation retained the best checkpoint plus recent checkpoints and deleted intermediate model-only files; consequently updates 53 and 75 are not recoverable locally or from W&B. The surviving V3 checkpoints at stop time are update 50 and updates 79-83. This retention behavior is retired: subsequent 0030 runs use the explicit `checkpoint_retention: all` contract and contain no training-time checkpoint deletion path. Any future checkpoint cleanup requires prior explicit user authorization.
