...
`V1_shared_encoder_cached_decoder` was interrupted before update 1 after its padded trajectory cache caused falling RAM and swap growth. Its update-0 checkpoint and baseline metrics are preserved as failure evidence and it will not be resumed. The formal corrected version is `V2_trimmed_fp16_prototype_cache`. Checkpoints are atomic model-only payloads containing only `action_decoder.*` and `value_head.*`, bound to the initial checkpoint and immutable representation hashes. Optimizer, scheduler, scaler, RNG, dataloader, rollout and replay state are forbidden. Retention is finite.

Implementation and performance gates are complete. The final 64-game / 64-worker official-engine benchmark reached 1.227 games/s with zero errors, within about 1.8% of the approximately 1.25 games/s 0023 V6 baseline. A 128-worker calibration exceeded the 60-second response timeout, so the formal run uses the highest tested zero-error setting of 64 workers. PPO made zero encoder calls and reached roughly 10,632 cached decoder decisions/s.

V2 is now running online. Its initial frozen greedy snapshot is 66-36 (64.71%, 102 games). Source policy update 0 then sampled 227-285 (44.34%, 512 games) and produced checkpoint update 1 after 58,685 focal decisions. The rollout took 377.74 seconds (1.355 games/s), representation hash remained unchanged, W&B is active, and update 1 completed all four PPO epochs without KL early stop. These sampled numbers are training diagnostics, not checkpoint-1 greedy strength evidence.

For context, 0022 V2's first sampled update was 140-372 (27.34%) and its initial frozen snapshot was 43-53 (44.79%), but its 48-deck/older-policy contract differs from 0030's 51-deck shared-0028 contract. 0022 was faster at 2.565 games/s with 128 workers. Therefore 0030 currently has a stronger within-run starting signal but does not match 0022's throughput; only future same-contract frozen greedy evaluations can establish policy improvement.
