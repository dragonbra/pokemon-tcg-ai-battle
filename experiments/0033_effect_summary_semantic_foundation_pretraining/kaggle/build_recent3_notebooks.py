"""Build 0033 recent-three-day Kaggle data and quick-look BC notebooks."""
...
RECENT_DAYS = (
    ("01_day_0801", "01_day_0801.ipynb", "2026-08-01", "PTCG 0033 01 Day 0801"),
    ("02_day_0802", "02_day_0802.ipynb", "2026-08-02", "PTCG 0033 02 Day 0802"),
    ("03_day_0803", "03_day_0803.ipynb", "2026-08-03", "PTCG 0033 03 Day 0803"),
)
...
def _text_transform(source: str) -> str:
    source = source.replace("0032", "0033")
    source = source.replace(
        "0033_audited_semantic_foundation_pretraining",
        "0033_effect_summary_semantic_foundation_pretraining",
    )
    source = source.replace(
        "0033_audited_semantic_decision_v2",
    ...
    source = source.replace("ptcg_0033_{PART_NAME}", "ptcg_0033_{PART_NAME}")
    source = source.replace("horizen12/ptcg-0033-source", SOURCE_DATASET)
...
def attach_label_audit(day_manifest: dict) -> dict:
    ...
    return {
        "schema_version": "0033_attach_label_audit_v1",
        "date": day,
        "status": "passed",
...
TRAINING_FIND_SOURCE_ROOT = r'''def find_source_root() -> Path:
    ...
    raise FileNotFoundError(
        "expected one complete 0033 source root or one flat source dataset, "
...
def build_daily_notebooks() -> list[str]:
    daily_builder = _load_module(ROOT / "build_daily_notebooks.py", "_0033_daily_builder")
    created: list[str] = []
    for directory, filename, day, title in RECENT_DAYS:
        ...
        metadata = {
            "id": f"horizen12/ptcg-0033-{directory.replace('_', '-')}",
            "title": title,
            "code_file": filename,
            "language": "python",
            ...
            "keywords": ["pokemon-tcg", "0033", "effect-summary", "winner-bc"],
            "dataset_sources": [
...
def _transform_training_cell(source: str) -> str:
    ...
    source = source.replace("ptcg_0033_rolling_t4_train", "ptcg_0033_recent3_rolling_t4_train")
    source = source.replace(
        "if not SMOKE and EPOCHS != 5:\n        raise ValueError(\"formal 0033 training must use exactly five epochs\")",
...
def build_training_notebook() -> str:
    ...
    notebook.cells[0].source = (
        "# PTCG 0033 recent-three-day rolling BC\n\n"
        "Randomly initializes the 0033 effect-summary SemanticPolicy and rolls once "
    ...
    metadata = {
        "id": "horizen12/ptcg-0033-04-train-recent3-rolling-t4",
        "title": "PTCG 0033 04 Train Recent3 Rolling T4",
        ...
        "enable_tpu": False,
        "enable_internet": False,
        "keywords": ["pokemon-tcg", "0033", "rolling", "attach-bc"],
        "dataset_sources": [SOURCE_DATASET],
        "competition_sources": [],
        "kernel_sources": [
            "horizen12/ptcg-0033-01-day-0801",
            "horizen12/ptcg-0033-02-day-0802",
            "horizen12/ptcg-0033-03-day-0803",
        ],
        "model_sources": [],
    }
    (target_dir / "kernel-metadata.json").write_text(
