...
0040 measures the long-run PPO learning curve of exact deck 007 from the stronger paired 0809 pretraining release. It preserves the repaired 0038 Action Boundary, reward, rollout and PPO semantics. Training does not add Seat residual, Opponent Meta, Plan Core, Value search, new rewards, or a new opponent pool. Phase 4 separately evaluates a default-off, evaluation-only U270 Value Search V0; it does not change the trained policy or PPO trajectory.
...
## U270 Value Search V0 evaluation

Phase 4 attaches the matching U270 `value_head` to a policy-conditioned local reranker only in the official CPU evaluation path. The normal U270 Policy first selects a semantic family. Exact Phase-3-audited Basic placement, fixed-source manual Energy target, Retreat payment, own Switch target, focal-owned Boss target, and Energy Switch destination nodes may then fork one official `SearchStep` per same-family candidate. Every branch starts from the same Search root and a fork of the same causal context; mutable ledger, known-card memory, event history and decision index are independent, while immutable prototypes and model weights are shared. Candidate features use the unchanged production compiler and one batched Value forward. The hook defaults to off and fails closed to the Policy selection.

The Value contract is focal terminal win/loss: terminal reward is `+1/-1/0`, GAE trains Query 0, and inference returns `2*sigmoid(logit)-1`; higher is better, so V0 uses argmax and keeps Policy on exact ties. Attack, End, cross-turn, opponent-owned, random, hidden-sensitive, forced singleton, multi-step and unaudited nodes remain excluded.

The fixed paired CPU256 result is archived in [`VALUE_SEARCH_V0_CPU256_RESULT.md`](VALUE_SEARCH_V0_CPU256_RESULT.md). Search OFF reproduced historical U270 exactly at `162-94-0` (`63.28125%`). Search ON was `120-136-0` (`46.875%`), a `-16.40625 pp` delta with `14 L->W` and `56 W->L`; 2,257 decisions evaluated 9,519 candidates and made 858 overrides with zero fallback. This is negative evidence for the exact pure-argmax V0 contract, not a change to training semantics or a general result for other search designs.
