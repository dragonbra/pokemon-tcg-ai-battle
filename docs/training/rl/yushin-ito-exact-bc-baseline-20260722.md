...
- 工作包：当时的 `work/alakazam_bc_v1/`（已退役）
- 历史 submission 源：当时的 `submission/alakazam_bc_v1/`（已退役）
- Kaggle archive：当时的 `submission/dist/alakazam_bc_v1.tar.gz`（当前不再保留本地副本）
...
- Kaggle submission：ref `54908815`，文件 `alakazam_bc_v1.tar.gz`，状态 `PENDING`（2026-07-22 17:14:19 UTC）。
