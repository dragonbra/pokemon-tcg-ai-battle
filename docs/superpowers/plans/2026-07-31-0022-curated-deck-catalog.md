...
- [ ] Classify James Cox & Henry Chao's exact hash as `Raging Bolt ex / Mega Kangaskhan ex`.
- [ ] Classify the Kaggle Dragapult list containing Dunsparce as `Dragapult ex / Dunsparce`.
- [ ] Synchronize every directory field and reject stale paths.
...
- Produces: two explicit `external_references` entries with exact hashes and provenance.

- [ ] Preserve the Limitless list's 1-1 Dunsparce/Dudunsparce tech note.
- [ ] Copy the arena deck byte-for-byte and identify it as an official recorded arena reference useful as a benchmark target.
- [ ] Do not attach Kaggle leaderboard metrics to either external reference.
...
- [ ] Render all 34 exact decks using official card names and card images.
- [ ] Display Kaggle usage/rank/score/record only for Kaggle decks and source facts only for external references.
- [ ] Verify 100 Kaggle memberships, 32 unique Kaggle hashes, two external references, 34 exact 60-card files, matching hashes, valid links, and no stale names.
