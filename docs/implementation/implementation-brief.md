...
状态：历史设计任务书。其 `submission/` 源包与打包脚本已于 2026-07-26 退役；以下路径和验收项仅用于解释早期 baseline 的形成过程。

## 当时的实现结果

- 官方资源位于 `data/official/`；当时的实际提交源位于 `submission/`。
- 当时 `python scripts/check_assets.py` 通过。
- 当时两套提交分别位于 `submission/official_water/` 和 `submission/alakazam_v1/`，每套 archive 顶层都包含 `main.py`、`deck.csv` 和 `cg/`。
