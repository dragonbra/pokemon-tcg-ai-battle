...
def _enriching_draw_route(current: dict[str, Any], player: dict[str, Any]) -> bool:
    ...
    if (
        ...
        # The attachment is legal even when this Dunsparce was played this
        # turn. Evolution must wait, so prepare the next-turn Dudunsparce
        # draw route instead of discarding a scarce Enriching Energy window.
        return True
...
def _discarded_abra_successor_recovery_due(
    ...
    if any(pokemon.get("id") in ATTACK_LINE for pokemon in _bench(player)):
        return False
    discard = set(_discard_ids(player))
    if not {ABRA, BASIC_PSYCHIC}.issubset(discard):
        return False
