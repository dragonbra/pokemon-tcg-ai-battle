# Alakazam V5 Mixed Strategy Specification

`v5_mixed` 不改变 V5 牌表。它的核心目标是：每回合优先形成一次有意义的
Alakazam 攻击，同时用可见状态保证下一只打手的最短攻击路径。所有选择都从
官方 simulator 返回的合法 option 中确定性地排序。

公开 notebook 提供的是状态建模方向，而不是可直接复制的权重或卡组：本版本吸收
...
- Xerosic's Machinations 只有在第二回合以后、可见的对手 Active Alakazam 当前
  Powerful Hand 对我方 Active 致命，而降到三张手牌后不再致命，或高置信地改变
  Alakazam mirror 交换时使用。对手手牌至少四张只是必要条件，不是充分条件。
