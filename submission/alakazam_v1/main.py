...
def _option_card_id(
    ...
    if option_type == 7:  # PLAY always indexes your hand.
        ...
        return hand[index].get("id") if owner == _your_index(current) and 0 <= index < len(hand) else None
...
def _choose_card_option(
    ...
    *,
    ...
    def category(card_id: int | None) -> int:
        ...
        if effect_id == HILDA:
            wanted = EVOLUTION if effect_step == 0 else {TELEPATH_ENERGY, BASIC_PSYCHIC, ENRICHING_ENERGY}
        ...
        if effect_id == POFFIN:
            ...
            # less. Shaymin has 80 HP and must not be treated as a target.
            wanted = {ABRA, DUNSPARCE, PSYDUCK}
            return 0 if card_id in wanted else 4
...
def _choose_energy_option(options: list[dict[str, Any]], current: dict[str, Any], player: dict[str, Any]) -> list[int]:
...
def _choose_switch_option(options: list[dict[str, Any]], current: dict[str, Any]) -> list[int]:
    ...
    def score(item: tuple[int, dict[str, Any]]) -> tuple[int, int, int, int, int]:
        ...
        return 0, 0, -{ALAKAZAM: 100, KADABRA: 70, ABRA: 45, DUNSPARCE: 20}.get(target_id, 10), 0, index
    ranked = sorted(enumerate(options), key=score)
    return [ranked[0][0]] if ranked else []
...
def _main_action(obs: dict[str, Any]) -> list[int]:
    ...
    def score(item: tuple[int, dict[str, Any]]) -> tuple[int, int, int]:
        ...
        if option_type == 13:
            if attack_is_ko:
                return (0, 0, index)
            # A legal attack is the fallback after all productive setup/draw
            # actions. This avoids spending the whole turn drawing without a
            # clear knockout threshold.
            return (65, -_estimated_attack_damage(active_id, hand_count), index)
        ...
        if option_type == 7:
            ...
            if card_id == BOSS_ORDERS:
                ...
                damage = _estimated_attack_damage(active_id, hand_count)
                target_can_ko = any(
                    int(target.get("hp", 9999)) <= damage
                    for target in _bench(opponent)
                )
                return (5 if target_can_ko and attack_is_legal and not attack_is_ko else 26, 0, index)
            ...
            if card_id == RARE_CANDY and ALAKAZAM in hand:
                return (6, 0, index)
            if card_id == POFFIN:
                return (12 if bench_space > 0 and setup_needed else 30, 0, index)
            if card_id == POKE_PAD:
