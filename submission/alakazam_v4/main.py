"""Alakazam V4: a deterministic Alakazam policy with early attack planning.

V4 keeps the V3 legal-option handling while making the direct Abra -> Alakazam
...
def _fezandipiti_needed(
    ...
    if _can_knockout(current, player):
        return False
    if _active_can_use_hilda_rare_candy(player):
        return False
    ...
    return not _has_usable_attack_path(current, player) and hand_count < 15
...
def _xerosic_is_worth_playing(current: dict[str, Any], player: dict[str, Any]) -> bool:
    ...
    if hand_count < 4 or _own_turn_number(current) < 2:
        return False
    return _opponent_is_alakazam_deck(current) or hand_count >= 8
    ...
    *,
    ...
    def category(card_id: int | None) -> int:
        ...
        if effect_id == POKE_PAD:
            active = _active(player)
            if _active_can_become_alakazam_now(player) or _active_can_use_hilda_rare_candy(
                player
            ):
                wanted = {ALAKAZAM}
...
def _main_action(obs: dict[str, Any]) -> list[int]:
    ...
    direct_hand_route = _active_can_become_alakazam_now(player)
    direct_hilda_route = _active_can_use_hilda_rare_candy(player)
    ...
    def score(item: tuple[int, dict[str, Any]]) -> tuple[int, int, int]:
        ...
        if option_type == 8:
            ...
            if energy_id == TELEPATH_ENERGY:
                if telepath_requires_abra:
                    return (40, 0, index)
                if target_id == ALAKAZAM and not _has_psychic_energy(target):
                    if active_id == ALAKAZAM and not _has_psychic_energy(active):
                        return (1 if target is active else 5, 0, index)
                    if target_area == 5:
                        return (2, 0, index)
                    return (4, 0, index)
            ...
            if energy_id == BASIC_PSYCHIC:
                if active_id == ALAKAZAM and not _has_psychic_energy(active):
                    if target is active:
                        return (1, 0, index)
                    if target_id == ALAKAZAM and target_area == 5:
                        return (5, 0, index)
                if active_id == FEZANDIPITI_EX and target is active and any(
                    pokemon.get("id") == ALAKAZAM and _has_psychic_energy(pokemon)
                    for pokemon in bench
                ):
                    return (2, 0, index)
                if target_id == ALAKAZAM and not _has_psychic_energy(target):
                    return (3 if target_area == 5 else 4, 0, index)
                if target_id == ALAKAZAM:
                    return (28, 0, index)
                return (8 if target_id in {KADABRA, ABRA} else 17, 0, index)
        ...
        if option_type == 7:
            ...
            if card_id == FEZANDIPITI_EX:
                return (3 if fezandipiti_needed else 130, 0, index)
            if card_id == XEROSIC:
...
def agent(obs_dict: dict[str, Any]) -> list[int]:
    """Return one legal action for the deterministic Alakazam V4 policy."""
