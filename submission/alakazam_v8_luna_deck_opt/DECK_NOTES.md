...
| Pokémon | Alakazam | 743 | 4 | 已填写 | **卡面事实：** Stage 2，进化时可使用 `Psychic Draw` 抽 3；`Powerful Hand` 需要一张 Psychic，并为手牌中的每张卡在对手 Active 放置 2 个伤害指示物，也就是每张手牌 20 点伤害。<br>**V7 已关注：** 先完成所有仍有明确收益的进化、Ability、过牌、Boss、Xerosic 和接力准备，再提交攻击；不要连续消耗 Rare Candy 制造没有即时收益的第二、第三只 Alakazam。<br>**V8 建议：** 每次攻击前重新计算手牌和牌库，不要把 `Enriching Energy` 当作 Psychic；如果已能确定 KO 或完成最后 Prize，攻击价值高于可选的过牌。 这里我补充一下。“如果确定能 KO，攻击的价值就高于可选过牌”这一点不一定完全正确。也就是说，如果我们后场仍然有 Abra 存在的话，我们可能可以通过过牌，让这回合给它进化成 Kadabra，这样能确保我们的打手不断进化链。这个是比直接进行进攻更重要的。我们在进攻之前，要确保这回合可以利用的资源已经全部用到了，让场面达到我们可以做到的最优状态。否则，如果你浪费了进化机会，或者一些能创造价值的支援者的机会，转而直接进攻的话，你会损失很多东西，也就是损失了你这一回合本可以做出的更多操作。|
| Pokémon | Dunsparce | 305 | 3 | 已填写 | **卡面事实：** 70 HP Basic，能被 Buddy-Buddy Poffin 找到；`Trading Places` 用一张无色交换自己与一只 Bench Pokémon，但这是攻击提交，`Ram` 需要两张无色造成 20 点。<br>**V7 已关注：** Dunsparce 的主要职责是铺在 Bench 上，等待 `Dudunsparce` 和 `Enriching Energy` 形成过牌引擎；不把 `Trading Places` 当普通换位。<br>**V8 建议：** 这里你刚刚的建议完全是错的呀！我们说了，规则里面使用进攻这个回合就结束了，所以你不可能使用那个 Trading Places，让你的胡地来到前场之后再一次进行攻击，这是不可能的。我说了，这个宝可梦的这个技能就没有适合使用的时机，任何时候我觉得你都不会需要用到它。 |
| Pokémon | Dudunsparce | 66 | 2 | 已填写 | **卡面事实：** Stage 1，`Run Away Draw` 每回合抽 3；只要因此抽到卡，就把这只 Dudunsparce、进化堆叠和所有附着卡洗回牌库。<br>**V7 已关注：** 只有存在合法接班者时才使用 Ability；Bench 为空时不能把唯一 Active 洗回牌库。洗回数量要计入牌库模型：牌库净减少量按抽 3 减去洗回的本体和全部附着卡计算。<br>**V8 建议：** 本卡只有两张，比 V7 少一条循环资源；应把它当成一次有成本的手牌恢复，而不是每回合默认使用。终局时可以识别 `Run Away Draw → 换上带 Psychic 的 Alakazam → Powerful Hand` 闭环，但非终局仍要保留接力和牌库安全。这个地方我再重新说一下：你要发现 Dudunsparce 的这个 Ability 其实是一次换位资源。也就是说，我们可以通过使用它的 Ability，让前场的这个 Dudunsparce 回到牌库，这样就能让后场的 Alakazam 到前面来进行攻击了。这是非常重要的一点。比如一个可能的局面：在第二回合，我们的前场是一个没有能量的 Dunsparce，后场是一个可以通过 Rare Candy、Alakazam 以及 Fusion Energy 进行进攻的 Alakazam。这时候，我们不要因为觉得找不到换位资源而放弃第二回合的进攻，而是可以通过 Arven 或者 Nest Ball 之类的、能检索宝可梦或道具的卡牌去找到 Dudunsparce，然后让前场的 Dunsparce 进化成 Dudunsparce，再使用它的 Ability "Runaway Draw" 回到牌库。这样一来，它更大的价值在于能让我们后场的 Alakazam 顺利来到前场进行进攻。这也就是说，假设我们的前场是一个 Dunsparce，想要在第二回合就让 Alakazam 发起进攻的话，找到一张 Dudunsparce 的进化型并进化它，随后使用它的 Ability，也是达成这件事的必要条件之一。 |
| Pokémon | Fezandipiti ex | 140 | 1 | 已填写 | **卡面事实：** Basic Pokémon ex，带 Rule Box、通常给对手两张 Prize；若我方 Pokémon 在对手上一回合被击倒，可使用一次 `Flip the Script` 抽 3，每回合最多使用一次。<br>**V7 已关注：** 手牌被干扰后、没有立即 KO 且仍有 Alakazam 或高伤害路线时，可以先 Bench 再使用 Ability；不要把它的 Ability 当作 Supporter。<br>**V8 建议：** 它不能被 Poffin、Poké Pad 或其它“无 Rule Box Pokémon”搜索直接找到，也不应为了多抽 3 张而延误最后 Prize 攻击；把两 Prize 风险和 Bench 空间一起计算。这张卡牌在目前的策略里面，我发现基本是没有被使用的，我觉得这不太对。现在我们先不要考虑它的 Prize 更多、会被敌人威胁到的这个问题。如果说前场（也就是上一个回合）我们的宝可梦被敌人昏厥了，在它的 Ability 是可以触发的状态下，那我们就需要去发动它的 Ability 进行过牌。这是我们很重要的一个充手牌的手段之一。通过这些过牌，我们才能够获取更多的资源，从而有更好的机会去完成游戏计划 |
...
| Trainer | Enhanced Hammer | 1081 | 4 | 已填写 | **卡面事实：** Item，只能弃掉对手一只 Pokémon 身上的一张 Special Energy，不能处理 Basic Energy。<br>**V7 已关注：** 只有在实际看到特殊能量，且弃除后能改变 Powerful Hand 的伤害、KO 或对手攻击门槛时，才提高优先级；面对 Mist 等保护性特殊能量时，先拆除再重新判断伤害。<br>**V8 建议：** 数量由 2 增至 4，说明构筑更重视特殊能量干扰，但不等于任何特殊能量都应立即拆；无合法目标、对手只有基础能量或拆除不改变本回合结果时保留。Item Lock 时不能使用。我补充说明一点：这张卡牌其实在任何时候都可以使用。也就是说，当对方场上有特殊能量满足使用条件时，我们其实优先就是要用的。因为这张卡牌放进来的目的就是减少对方的能量，这可能会有更深远的意义（例如让对方因为缺少能量，在后面的回合无法进攻）。所以只要能使用这张卡牌，我们就一定要用。只是在选择目标能量时，有优先级顺序：1. 像我们之前提到的特殊岩石能量以及 Mist Energy，会更优先被作为目标进行选择 2. 如果这两种能量在前场的 Active 宝可梦身上，那就是最优先被选择的 |
...
| Trainer | Sacred Ash | 1129 | 1 | 已填写 | **卡面事实：** Item，把弃牌区最多五只 Pokémon 洗回牌库，不会直接把它们放回手牌。<br>**V7 已关注：** 多只 Abra 线被击倒、后续搜索资源不足时用它恢复进化链密度；它不是当前回合立即拿到 Alakazam 的替代品。<br>**V8 建议：** 由于牌库保护线存在，使用后还要通过 Poffin、Poké Pad、Hilda 或 Dawn 把洗回的 Pokémon 找出来；只洗回一张没有现实搜索路线的 Pokémon，通常不值得消耗 Item 和增加牌库。Item Lock 时不能使用。我补充说明一点：在你使用这张牌的时候，你要尽可能多地去选择，也就是能选多少张弃牌区的宝可梦，你就要选择多少张。因为我们这套卡组最担心的是，在我们的 Alakazam 被昏厥了之后，没有足够的后备宝可梦继续持续地发动进攻。我认为你选择的优先级是这样的：1. 基础目标（优先满足 Abra 系列）：我们先选一只 Abra，再选一只 Kadabra，再选一只 Alakazam；然后再选一只 Abra，再选 Kadabra，再选 Alakazam。在 Abra 系列里面，我们的选择顺序： (a) 优先从底座（基础宝可梦）往三阶选。(b) 二阶（Kadabra）在有 rare candy 资源的情况下可能不那么重要，你需要根据场况判断一下。2. 备用目标：在优先满足并选择了所有的 Abra 系列之后，如果还没有达到五只宝可梦，我们再选择 Dunsparce 这一系列的 或者其他的 宝可梦回到牌库。 |
| Trainer | Poké Pad | 1152 | 4 | 已填写 | **卡面事实：** Item，只能从牌库找一只没有 Rule Box 的 Pokémon；可以找 Abra、Kadabra、Alakazam、Dunsparce、Dudunsparce 和 Shaymin，不能找 Fezandipiti ex。<br>**V7 已关注：** 搜索目标要服务本回合可进化或下一次可攻击的具体阶段，不能用“找一只 Pokémon”代替完整攻击路线；它不能找 Energy。<br>**V8 建议：** 四张提供较高的进化链检索密度，但每次都会消耗牌库；低牌库或 Item Lock 时分别执行保护线和禁用 Item 的判断。这里我展开说一下一些比较明确的点。我发现你经常会在第一回合不必要的时候去使用这张卡。什么叫不必要呢？假设现在是第一回合，我们的场面已经有其他的资源能够做出两只 Abra 和一只 Dunsparce 的时候，我们没有任何理由去使用这张牌。因为你用这张牌找出来、铺出新的底座，没有什么实际的收益。反倒是留到下一回合，假设我们需要一只 Kadabra，或者是需要一只 Alakazam，甚至需要 Dudunsparce 找出来去进行换位的时候，我们就可以用这张牌找出来这张关键的宝可梦。所以这是一张非常重要的牌。我们的进化轴以及各种资源的管理，其实只要和宝可梦相关，绝大部分重要的宝可梦都可以通过这张牌找出来。你很重要的一点，就是去保留这张牌的资源，去让它发挥真正的价值。 |
...
| Trainer | Lana’s Aid | 1184 | 1 | 已填写 | **卡面事实：** Supporter，从弃牌区以任意组合拿最多三张无 Rule Box Pokémon 和 Basic Energy 到手牌；不能拿回 Fezandipiti ex 或 Special Energy。<br>**V7 已关注：** 需要同时恢复 Pokémon 与 Basic Psychic 时优先于 Night Stretcher；恢复动作必须能构成当前或下一次现实攻击，而不是只让手牌看起来更多。<br>**V8 建议：** V8 没有 `Wondrous Patch`，Lana's Aid 是重要的弃牌区 Psychic 接力入口；但它占用唯一 Supporter，若本回合有确定 Boss KO 或终局攻击，就不能为了非必要恢复而延后攻击。不不不不不，我仍然要说它是最重要的宝可梦接力方式。因为在我们的卡牌中，如我所说，在不考虑弃牌区资源回收的情况下，我们最多只能做出 4 只 Alakazam 的打手。假设我们的场面需要新的打手，那么我们使用这张牌的逻辑，应该和之前的 Super Rod 有点类似：我们需要尽可能先回收 Abra 这一系列、最终能进化成 Alakazam 进行进攻的打手。相反，除非在必须的情况下——也就是它是最后一个行动的位置，并且只有通过它才能拿到 Psychic Energy 进行进攻——否则我们尽可能不需要让它拿能量，而是主要让它拿宝可梦 |
...
| Trainer | Dawn | 1231 | 4 | 已填写 | **卡面事实：** Supporter，从牌库各找一只 Basic Pokémon、Stage 1 Pokémon 和 Stage 2 Pokémon；不能找 Energy。<br>**V7 已关注：** 适合一次补齐 `Abra → Kadabra → Alakazam` 的进化链，也可以按合法路线准备 Dunsparce/Dudunsparce；但不能只为了让场面看起来完整而消耗唯一 Supporter。<br>**V8 建议：** 先确认三类进化卡都有现实用途和合法入场时间；如果 Boss 能立刻拿 Prize、Lana 能补齐下一只攻击或 Xerosic 能改变本回合结果，Dawn 不一定是最高优先级。我要对 V7 的卡组现状进行一点修正。如我刚刚所说，我觉得你对 Dudunsparce 这张卡牌的理解还不够透彻。它是一个潜在的换位资源，也是一个补充手牌的资源。也就是说，我们不需要每次都用它去拿 Abra 这个系列的宝可梦。反倒是要多考虑以下情况：1. 如果我们场面上一只 Dunsparce 都没有，那我们是不是应该拿一只 Dunsparce？2. 如果我们的 Dunsparce 在前场，可以通过进化成 Dudunsparce 进行换位，这时候我们的二阶宝可梦是不是也应该拿一个 Dudunsparce？并且我还有一个原则要说明：使用它去拿宝可梦的时候，尽可能地多拿，不要出现明明可以拿三张、你却只拿了一张的情况 |
| Trainer | Nighttime Mine | 1266 | 2 | 已填写 | **卡面事实：** Stadium；场上双方每一只 Tera Pokémon 使用的攻击费用增加一张无色。它不是增伤卡，也不是只作用于对手。<br>**V7 已关注：** V7 没有这张牌，不能从对手 ID 或构筑预设它的价值。<br>**V8 建议：** 只在 observation 中实际看到 Tera Pokémon，且增加一张无色确实能改变对手攻击节奏或阻止其攻击时优先使用；若对手没有 Tera，通常没有即时收益。V8 自身没有 Tera Pokémon，因此对我方常规 Alakazam 攻击没有额外费用，但仍要注意 Stadium 替换和对手场面变化。不不不，我这里要对这张卡说一个补充：任何时候只要能使用这张卡，我们就要立刻使用。原因有以下几点：1. 这张卡对我们的卡组没有任何限制，而且对方的场地我们普遍也用不到。 2. 在这种情况下，我们使用这张场地卡，可以顶掉对面潜在可能已经放在场上的、对我们不利的场地卡。3. 如果对面有 Tera Pokémon 的话，我们这张卡是实打实能影响对方进攻节奏的。所以，这张卡牌一旦到了手上，你就要立刻打出去，它的价值是非常高的。 |
