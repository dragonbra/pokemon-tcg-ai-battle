...
def _can_use_enriching_for_draw(player: dict[str, Any]) -> bool:
    ...
    return bool(
        ...
        and _has_psychic_energy(active)
        and not _has_unenergized_attack_line(player)
    ...
    def score(item: tuple[int, dict[str, Any]]) -> tuple[int, int, int]:
        ...
        if option_type == 10:
            ...
            if card_id == FEZANDIPITI_EX and not fezandipiti_needed:
                return (130, 0, index)
            if card_id == DUDUNSPARCE and deck_count <= 1:
                # Run Away Draw returns Dudunsparce after drawing the cards
                # that remain; only an empty/one-card deck is kept guarded.
                return (100, 0, index)
        ...
        if option_type == 12:
            ...
            return (
                ...
                12 if own_turn > 1 and _retreat_improves_attack(player) else 100,
                0,
                index,
            )
        if option_type == 14:
