...
def _enriching_draw_route(current: dict[str, Any], player: dict[str, Any]) -> bool:
    ...
    if (
        ...
        and DUDUNSPARCE in _hand_ids(player)
...
def _dudunsparce_net_deck_change(attached_count: int) -> int:
    """Compute draw-three minus Dudunsparce and all attached cards."""
    return 3 - (1 + max(0, int(attached_count)))
...
def _dudunsparce_deck_delta(
    ...
    Run Away Draw shuffles the selected Dudunsparce and all cards attached to
    that instance back into the deck.  When the observation identifies a
    field target, use it; otherwise retain the old first-visible fallback for
    lightweight callers that do not carry option coordinates.
    """
    dudunsparce = None
    ...
    if dudunsparce is None:
        return 3
    return _dudunsparce_net_deck_change(len(dudunsparce.get("energyCards") or []))
...
def _pokemon_from_option(option: dict[str, Any], current: dict[str, Any]) -> dict[str, Any] | None:
    ...
    area = option.get("inPlayArea", option.get("area"))
    index = option.get(
        "inPlayIndex", option.get("indexInArea", option.get("index"))
    )
...
def _bench_handoff_preparation_due(
    ...
    def valid_target(target: dict[str, Any] | None) -> bool:
        ...
        # handoff.  Keep Abra on the stricter evolution-route check below.
        if target.get("id") in {KADABRA, ALAKAZAM}:
            return True
        return target.get("id") == ALAKAZAM or _evolution_target_is_legal(
...
def _has_visible_bench_handoff_route(
    ...
    for pokemon in _bench(player):
        ...
        if (
            ...
            and (
                KADABRA in hand_ids
                or (ALAKAZAM in hand_ids and RARE_CANDY in hand_ids and not item_lock)
            )
        ):
            return True
    return False
...
def _bench_insurance_due(
    ...
    has_anchor_option = any(
        ...
        or (
            option.get("type") == 8
            and _option_card_id(option, select, current) == TELEPATH_ENERGY
            and (
                option.get("inPlayArea", option.get("area")) == 5
                and (_pokemon_from_option(option, current) or {}).get("id") in ATTACK_LINE
                or option.get("inPlayArea", option.get("area")) == 4
                and (_pokemon_from_option(option, current) or {}).get("id") in ATTACK_LINE
            )
        )
        for option in options
    ...
    *,
    ...
    def category(card_id: int | None) -> int:
        ...
        if effect_id == POFFIN:
            ...
            # the Dunsparce engine. Field count alone is not enough: an
            # unenergized Abra is not a ready replacement after a knockout.
            wanted = (
                {ABRA}
                if _attack_line_count(player) < 3
                or _ready_attack_line_count(player) < 2
                else {DUNSPARCE}
            )
            return 0 if card_id in wanted else 4
        ...
        if effect_id in {NIGHT_STRETCHER, LANAS_AID}:
            ...
            if effect_id == LANAS_AID:
                if needs_energy and card_id == BASIC_PSYCHIC:
                    return 0
                if needs_pokemon and card_id == ABRA:
                    return 0
                return 4
...
def _choose_energy_option(
    ...
    def score(item: tuple[int, dict[str, Any]]) -> tuple[int, int]:
        ...
        # The active Pokémon is the default target, with protective energies
        # taking priority when they block Powerful Hand.
        if owner != _your_index(current) and energy_id not in BASIC_ENERGY_IDS:
            if area == 4:
                if (
                    energy_id in PROTECTIVE_DAMAGE_ENERGIES
                    and (_active(player) or {}).get("id") == ALAKAZAM
                ):
                    return -320, index
                return -260, index
...
def _select_effect(obs: dict[str, Any]) -> list[int]:
    ...
    if context in {1, 2, 5, 6, 7, 8, 9, 10, 21, 26, 29}:
        ...
        if effect_id == LANAS_AID:
            ...
            # mandatory lower bound (normally 1), so do not let it suppress a
            # second card when both visible recovery gaps exist.  Keep the
            # selection minimal: one card for one gap, two for two gaps.
            needs_pokemon, needs_energy = _recovery_needs(current, player)
...
def _main_action(obs: dict[str, Any]) -> list[int]:
    ...
    def score(item: tuple[int, dict[str, Any]]) -> tuple[int, int, int]:
        ...
        if option_type == 7:
            ...
            if card_id in {DAWN, HILDA, LANAS_AID} and gain > 0:
                ...
                if card_id == LANAS_AID:
                    if bench_handoff_preparation_due and _recovery_can_complete_route(
                        current, player, LANAS_AID
                    ):
                        return (0, 0, index)
    ...
    if not rankable:
        ...
        # intended policy, but returning its sole legal option is safer than
        # fabricating an index or crashing the submission.
        rankable = list(enumerate(options))
