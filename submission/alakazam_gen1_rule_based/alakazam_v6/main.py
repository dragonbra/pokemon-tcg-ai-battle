"""Alakazam V6: a rule-semantic strategy rebuild on the V5 AutoIter runtime.

The deck and simulator bridge stay fixed. V6 adds explicit turn memory and a
...
class TurnMemory:
    ...
    def sync(self, current: dict[str, Any], player: dict[str, Any]) -> None:
...
def _retreat_improves_attack(player: dict[str, Any]) -> bool:
    ...
    if not active:
        return False
    if active.get("id") != DUNSPARCE and _has_attack_ready_pokemon(player):
        return False
    return any(
        pokemon.get("id") == ALAKAZAM and _has_psychic_energy(pokemon)
...
def _dudunsparce_deck_delta(player: dict[str, Any]) -> int:
    dudunsparce = next(
...
def _recovery_needs(current: dict[str, Any], player: dict[str, Any]) -> tuple[bool, bool]:
    ...
    hand_has_line = bool(hand_ids & ATTACK_LINE)
    needs_pokemon = not field_has_line and not hand_has_line
    needs_energy = any(
        pokemon.get("id") in ATTACK_LINE and not _has_psychic_energy(pokemon)
        for pokemon in field
    )
    if (field_has_line or hand_has_line) and not _psychic_energy_in_hand(player):
        needs_energy = True
...
def _select_effect(obs: dict[str, Any]) -> list[int]:
    ...
    _TURN_MEMORY.sync(current, player)
    ...
    def preparation_is_due() -> bool:
        ...
        for option in options:
            ...
            if option_type == 9:
                ...
                deck_delta = (
                    _dudunsparce_deck_delta(player)
    ...
    def score(item: tuple[int, dict[str, Any]]) -> tuple[int, int, int]:
        ...
        if option_type == 10:
            ...
            if card_id == FEZANDIPITI_EX and not fezandipiti_needed:
                return (130, 0, index)
            if card_id in DRAW_CARDS or card_id == FEZANDIPITI_EX:
                if card_id == DUDUNSPARCE and _can_use_enriching_for_draw(player):
                    return (5, -gain, index)
        ...
        if option_type == 8:
            ...
            if energy_id == ENRICHING_ENERGY and target_id == DUDUNSPARCE:
                if not _can_use_enriching_for_draw(player):
                    return (100, 0, index)
                if _v6_draw_is_blocked(
                    current,
                    player,
...
def agent(obs_dict: dict[str, Any]) -> list[int]:
    """Return one legal action for the deterministic Alakazam V6 policy."""
