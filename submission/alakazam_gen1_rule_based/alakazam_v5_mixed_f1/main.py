...
def _attack_continuity(current: dict[str, Any], player: dict[str, Any]) -> dict[str, int | bool]:
    ...
    alakazam_count = sum(pokemon.get("id") == ALAKAZAM for pokemon in field_line)
    ...
    recovery_available = bool(
        recovery_count
        and set(_hand_ids(player)) & {NIGHT_STRETCHER, LANAS_AID, SACRED_ASH}
    )
    next_attack_path = bool(
        ready_line_count
        or direct_path
        or natural_path
        or recovery_available
    )
    ...
    return {
        ...
        "ready_attacker_count": ready_line_count,
...
def _retreat_improves_attack(player: dict[str, Any]) -> bool:
    """Allow retreat when a charged Abra-line attacker can take over."""
    active = _active(player)
...
def _xerosic_is_worth_playing(current: dict[str, Any], player: dict[str, Any]) -> bool:
    ...
    if hand_count < 4 or _own_turn_number(current) < 2:
        return False
    # The opponent's card identities are hidden.  The exact lethal-to-
    # non-lethal test is a useful signal, but too narrow for mirror play;
    # restore V5's high-confidence Alakazam-deck fallback when no stronger
    # action is competing for this Supporter turn.
    return _xerosic_changes_opponent_attack(current, player) or _opponent_is_alakazam_deck(
        current
    ) or hand_count >= 8
    ...
    *,
    ...
    def category(card_id: int | None) -> int:
        ...
        if effect_id == HILDA:
            if effect_step == 0:
                if _active_hilda_rare_candy_route(current, player) or (
                    ...
                    and continuity["ready_attacker_count"] >= 2
                ):
                    wanted = {DUDUNSPARCE}
                else:
                    wanted = EVOLUTION
        ...
        if effect_id == POFFIN:
            ...
            # Before the first Alakazam, a card in hand is not an attacker on
            # the field.  Keep completing the actual Abra line; only after a
            # real attacker and a ready replacement exist should Poffin turn
            # toward the Dunsparce engine.
            wanted = (
                {ABRA}
                if continuity["field_count"] < 3
                or continuity["ready_attacker_count"] < 2
                else {DUNSPARCE}
            )
            return 0 if card_id in wanted else 4
...
def _main_action(obs: dict[str, Any]) -> list[int]:
    ...
    first_alakazam_established = bool(continuity["alakazam_count"])
    ready_attacker_count = int(continuity["ready_attacker_count"])
    # Before the first Alakazam, preserve V5's tempo-oriented field objective.
    # After it lands, use ready attackers rather than raw Bench count to decide
    # whether setup must continue for the next prize exchange.
    setup_needed = bool(
        attack_line_count < 3
        or (
            first_alakazam_established
            and (ready_attacker_count < 2 or not continuity["next_attack_path"])
        )
    )
        ...
        if option_type == 7:
            ...
            if card_id == POFFIN:
                return (
                    ...
                    if bench_space > 0
                    and (
                        continuity["field_count"] < 3
                        or ready_attacker_count < 2
                    )
                    else 12
                    if bench_space > 0
                    else 30,
