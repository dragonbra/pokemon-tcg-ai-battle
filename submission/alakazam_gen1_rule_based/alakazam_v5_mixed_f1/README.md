# Alakazam V5 Mixed F1 Submission

`v5_mixed_f1` 是固定 V5 牌表上的第一轮回退修正版：保留 V5 的 `deck.csv`、公开 API、
...
共 60 张。`v5_mixed_f1/deck.csv` 必须与 `alakazam_v5/deck.csv` 完全一致。review 中的 Hisuian Heavy Ball、Snorlax、
...
F1 针对 `v5_mixed` 评测中的节奏回退修正：先恢复首只 Alakazam 的建立速度，
再用 ready attacker 数量约束后续接力，避免把牌库保护误当成攻击连续性。
...
- Xerosic 优先使用可见的 lethal 防守判断，同时保留 V5 的 Alakazam 对手/大手牌
  fallback；Boss/Hammer/Retreat 仍然要求可证明的攻击或 Prize 收益。
- 能量和恢复按最短攻击路径选择：没有场上 Abra 系列时先恢复 Abra；每只攻击线
  Pokémon 默认只贴第一张 Psychic Energy，Telepath 不贴重复能量。
- Xerosic 保留可见致命防守判断，同时恢复 V5 的 Alakazam 对手/大手牌 fallback，
  不要求知道对手的隐藏牌身份。
...
bash scripts/package_submission.sh alakazam_v5_mixed_f1
```
