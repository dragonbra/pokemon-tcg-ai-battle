"""Guarded focal-only PPO orchestration retained for 0024 diagnostics."""
...
SECONDARY_DECK_ID = "rmy_teal_mask_ogerpon_001"
LIVE_OPPONENT_WEIGHTS = {SECONDARY_DECK_ID: 10}
...
class LeagueTrainingConfig:
    ...
    checkpoint_keep_gate_snapshots: int = 4
...
def compose_training_config(initialization: dict[str, object], config: LeagueTrainingConfig, *,
    ...
    return {
        **initialization, "schema_version": "0024_focal_league_ppo_v1",
        "run": asdict(config), "foundation": identity,
        "focal_deck_id": FOCAL_DECK_ID, "catalog_count": catalog_count,
...
def schedule_jobs(plugins: tuple[DeckPlugin, ...], *, count: int, update: int, seed: int, runtime_root: Path, evaluation: bool = False) -> list[RolloutJob]:
    ...
    # Live deck.
    matchups: list[tuple[DeckPlugin, LeaguePolicyView]] = []
    for view in views:
        for item in plugins:
            if not evaluation and item.deck_id == focal.deck_id:
                continue
            weight = 1 if evaluation or view is LeaguePolicyView.FROZEN else LIVE_OPPONENT_WEIGHTS.get(item.deck_id, 1)
            matchups.extend((item, view) for _ in range(weight))
    if evaluation:
...
def run_training(config: LeagueTrainingConfig, *, deck_root: Path = DEFAULT_DECK_ROOT) -> int:
    if (config.games_per_update < 1
            or (config.duration_hours is not None and config.duration_hours <= 0)
    ...
    os.environ.update({
        ...
        "WANDB_TAGS": "0024,league,ppo,mega_lopunny,mega_froslass,focal_only",
    })
    ...
    started = time.time()
    deadline = (
        None if config.duration_hours is None
        else started + config.duration_hours * 3600
    )
    update = 0
    ...
    try:
        with TrainingLogger(paths.metrics, paths.tensorboard) as logger:
            ...
            while (deadline is None or time.time() < deadline) and not stop.requested:
