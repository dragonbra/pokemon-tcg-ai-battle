"""Continuous focal-primary multi-decoder League PPO runner for 0024."""
...
SOURCE_PROJECT_ID = "0023_mega_lopunny_ex_mega_froslass_ex_002_league_training"
SOURCE_VERSION = "V8_add_rmy_ogerpon_default"
SOURCE_ROOT = Path("rl_runs") / SOURCE_PROJECT_ID / "versions" / SOURCE_VERSION
SOURCE_UPDATE = 0
LOPUNNY_SOURCE_DECK_ID = "mega_lopunny_ex_001"
LOPUNNY_DECODER_TARGETS = frozenset({
    "mega_lopunny_ex_001",
    "mega_lopunny_ex_mega_froslass_ex_001",
    "mega_lopunny_ex_mega_froslass_ex_002",
})
# Retained only for resolver regression coverage; formal 0024 launch never uses 0022.
...
def resolve_v8_initial_checkpoint_map(
    plugins: tuple[DeckPlugin, ...], *, source_root: Path = SOURCE_ROOT,
) -> tuple[
    dict[str, Path], dict[str, tuple[str, str]],
    dict[str, dict[str, object]], dict[str, object],
]:
    """Resolve immutable 0023 V8 assets, with the explicit Lopunny override."""
    status_path = source_root / "artifact/status.json"
    catalog_path = source_root / "artifact/league_catalog.json"
    if not status_path.is_file() or not catalog_path.is_file():
        raise FileNotFoundError(f"0023 V8 provenance is incomplete: {source_root}")
    status = json.loads(status_path.read_text(encoding="utf-8"))
    if status.get("state") != "initialized" or status.get("training_started") is not False:
        raise ValueError("0023 V8 must remain an immutable initialization source")
    catalog = json.loads(catalog_path.read_text(encoding="utf-8"))
    records = catalog.get("decks")
    if not isinstance(records, list):
        raise ValueError("0023 V8 catalog has no deck list")
    source = {
        str(record["deck_id"]): record for record in records
    ...
    if set(source) != set(current):
        raise ValueError(
            "0023 V8 catalog differs from 0024 catalog: "
            f"missing_current={sorted(set(source) - set(current))}; "
            f"new_current={sorted(set(current) - set(source))}"
        )
    if not LOPUNNY_DECODER_TARGETS <= set(current):
        raise ValueError("0024 catalog lacks a required Mega Lopunny decoder target")
    ...
    for plugin in plugins:
        source_deck_id = (
            LOPUNNY_SOURCE_DECK_ID
            if plugin.deck_id in LOPUNNY_DECODER_TARGETS
            else plugin.deck_id
        )
        source_record = source[source_deck_id]
        source_deck_sha256 = str(source_record.get("deck_sha256"))
        if plugin.deck_id not in LOPUNNY_DECODER_TARGETS and source_deck_sha256 != plugin.deck_sha256:
            raise ValueError(f"0023 V8 exact deck changed for {plugin.deck_id}")
        checkpoint = source_root / "checkpoint/decks" / f"{source_deck_id}.pt"
        if not checkpoint.is_file():
            raise FileNotFoundError(f"0023 V8 lacks decoder for {source_deck_id}")
        checkpoint_audit = load_decoder_checkpoint(
            checkpoint,
            expected_foundation_sha256=EXPECTED_WEIGHTS_SHA256,
            expected_deck_id=source_deck_id,
            expected_deck_sha256=source_deck_sha256,
        )
        if checkpoint_audit.update != SOURCE_UPDATE:
            raise ValueError(f"0023 V8 decoder update is not {SOURCE_UPDATE}: {source_deck_id}")
        checkpoints[plugin.deck_id] = checkpoint
        expectations[plugin.deck_id] = (source_deck_id, source_deck_sha256)
        audit[plugin.deck_id] = {
            "initialization": (
                "cross_loaded_0023_focal_decoder"
                if plugin.deck_id != LOPUNNY_SOURCE_DECK_ID
                and plugin.deck_id in LOPUNNY_DECODER_TARGETS
                else "inherited_0023_v8"
            ),
            ...
            "target_deck_sha256": plugin.deck_sha256,
    ...
    return checkpoints, expectations, audit, metadata
...
def resolve_initial_checkpoint_map(
    ...
    if missing_current:
        raise ValueError(f"previous Live decks disappeared from 0024: {missing_current}")
...
def resolve_project_version_checkpoint_map(
    ...
    """Resolve every current Live deck from a prior 0024 version checkpoint."""
    ...
    if missing_current:
        raise ValueError(
            "source League catalog differs from current 0024 catalog: "
    ...
    for plugin in plugins:
        if plugin.deck_id in new_current:
            ...
            audit[plugin.deck_id] = {
                ...
                "reason": "deck_absent_from_source_0024_catalog",
            }
            continue
        ...
        checkpoints[plugin.deck_id] = checkpoint
        audit[plugin.deck_id] = {
            "initialization": "inherited_0024_version",
...
def _probe_metrics(
    ...
    probe_ids = ("alakazam_dudunsparce_001", SECONDARY_DECK_ID)
...
def run_league_training(
    ...
    if focal.deck_id != FOCAL_DECK_ID:
        raise ValueError(f"0024 focal deck must be {FOCAL_DECK_ID}, got {focal.deck_id}")
    identity = verify_foundation()
    initial_sources, initial_expectations, initialization_audit, initialization_source = (
        resolve_v8_initial_checkpoint_map(plugins)
    )
    ...
    paths = initialize_league_version(
        ...
        initial_checkpoint_expectations=initial_expectations,
    )
    ...
    cross_loaded_count = sum(
        record["initialization"] == "cross_loaded_0023_focal_decoder"
        for record in initialization_audit.values()
    )
    inherited_count = len(initialization_audit) - cross_loaded_count
    _atomic_json(initialization_audit_path, {
        "schema_version": "0024_live_initialization_audit_v1",
        "source_project": initialization_source["source_project"],
        "source_version": initialization_source["source_version"],
        "source_update": initialization_source["source_update"],
        ...
        "cross_loaded_0023_focal_decoder_count": cross_loaded_count,
        "foundation_initialized_count": 0,
        "decks": initialization_audit,
    })
    _atomic_json(paths.config, {
        **initialized,
        "schema_version": "0024_multidecoder_league_ppo_v1",
        "run": asdict(config),
        ...
        "reference_kl_contract": "fixed_to_each_deck_0024_initialization",
        "secondary_policy": {
            "deck_id": SECONDARY_DECK_ID,
            "live_opponent_weight": LIVE_OPPONENT_WEIGHTS[SECONDARY_DECK_ID],
            "frozen_opponent_weight": 1,
            "contract": "independent_decoder_value_optimizer",
        },
    ...
    os.environ.update({
        ...
        "WANDB_TAGS": "0024,league,multidecoder,marnie,teal_mask_ogerpon,continuous",
    })
    started = time.time()
    update = 0
    ...
    try:
        with TrainingLogger(paths.metrics, paths.tensorboard) as logger:
            while not stop.requested and (max_updates is None or update < max_updates):
                ...
                secondary_live_scheduled = sum(
                    job.opponent_deck_id == SECONDARY_DECK_ID
                    and job.opponent_view is LeaguePolicyView.LIVE
                    for job in jobs
                )
                episodes = collector.collect(jobs)
                if any(not episode.valid for episode in episodes):
                    raise RuntimeError("0024 League rollout contains official-engine errors")
                deck_metrics: dict[str, float] = {}
                deck_metrics["rollout/secondary/live_opponent_weight"] = float(
                    LIVE_OPPONENT_WEIGHTS[SECONDARY_DECK_ID]
                )
                deck_metrics["rollout/secondary/live_scheduled_episodes"] = float(
                    secondary_live_scheduled
                )
                ...
                deck_metrics["rollout/secondary/actor_decisions"] = float(
                    decisions_by_deck.get(SECONDARY_DECK_ID, 0)
                )
                if missing_live:
                    raise RuntimeError(
