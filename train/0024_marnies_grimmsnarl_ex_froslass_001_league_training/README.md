# 0024 Mega Lopunny ex Limitless Focal / Mega Froslass ex League Training
...
```text
rl_runs/0024_marnies_grimmsnarl_ex_froslass_001_league_training/versions/V<n>_<tag>/
...
```bash
python3 -m train.0024_marnies_grimmsnarl_ex_froslass_001_league_training verify-foundation
python3 -m train.0024_marnies_grimmsnarl_ex_froslass_001_league_training validate-decks
python3 -m train.0024_marnies_grimmsnarl_ex_froslass_001_league_training initialize --version V1_initial_league
python3 -m train.0024_marnies_grimmsnarl_ex_froslass_001_league_training audit-version --version V1_initial_league
python3 -m train.0024_marnies_grimmsnarl_ex_froslass_001_league_training smoke-rollout --device cuda:0 --workers 4
python3 -m train.0024_marnies_grimmsnarl_ex_froslass_001_league_training canary-ppo --device cuda:0 --workers 4
python3 -m train.0024_marnies_grimmsnarl_ex_froslass_001_league_training benchmark-workers --device cuda:0 --workers 128 --games 256 --coalesce-ms 5 --output .tmp/evaluation/0024_worker_scaling/workers-128.json
python3 -m train.0024_marnies_grimmsnarl_ex_froslass_001_league_training train-league --version V7_from_v6_limitless_focal_24h --initial-version V6_from_v2_update3_20h_gpu_resume --device cuda:0 --workers 128 --coalesce-ms 5 --games-per-update 512
```
...
session. Omit `--min-hours` so the local process monitor runs continuously, retain that session ID,
and wait on it repeatedly; do not use a detached watchdog, and do not package or run competing GPU
work while the session is active.
