...
from ..training.league_run import (
    ...
    resolve_v8_initial_checkpoint_map,
)
...
class LeagueTrainingTest(unittest.TestCase):
    def test_curated_catalog_uses_most_common_marnie_focal(self) -> None:
    ...
    def test_existing_0024_version_checkpoint_can_initialize_restart(self) -> None:
        plugin = next(
            item for item in load_deck_plugins(DEFAULT_DECK_ROOT)
            if item.deck_id == "mega_lopunny_ex_mega_froslass_ex_002"
        )
    ...
    def test_existing_0024_version_foundation_initializes_new_current_deck(self) -> None:
        plugins = load_deck_plugins(DEFAULT_DECK_ROOT)
        inherited = next(item for item in plugins if item.deck_id == "dragapult_ex_001")
    ...
    def test_v8_resolver_cross_loads_the_focal_lopunny_decoder(self) -> None:
        target_ids = {
            "mega_lopunny_ex_001",
        ...
        plugins = tuple(
            item for item in load_deck_plugins(DEFAULT_DECK_ROOT)
            if item.deck_id in target_ids
        )
        source = next(item for item in plugins if item.deck_id == "mega_lopunny_ex_001")
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            (root / "artifact").mkdir()
            (root / "artifact/status.json").write_text(
                json.dumps({"state": "initialized", "training_started": False}),
                encoding="utf-8",
            )
            (root / "artifact/league_catalog.json").write_text(
                json.dumps({"decks": [
            ...
            checkpoint = root / "checkpoint/decks/mega_lopunny_ex_001.pt"
            checkpoint.parent.mkdir(parents=True)
            checkpoint.touch()
            decoder_audit = SimpleNamespace(
                update=0, checkpoint_sha256="e" * 64,
                foundation_sha256="f" * 64,
            )
            with patch.object(league_run, "load_decoder_checkpoint", return_value=decoder_audit):
                paths, expectations, audit, metadata = resolve_v8_initial_checkpoint_map(
                    plugins, source_root=root
                )
            self.assertEqual(
                paths["mega_lopunny_ex_mega_froslass_ex_001"], checkpoint
            )
            self.assertEqual(
                expectations["mega_lopunny_ex_mega_froslass_ex_002"],
                (source.deck_id, source.deck_sha256),
            )
            self.assertEqual(
                audit["mega_lopunny_ex_mega_froslass_ex_001"]["initialization"],
                "cross_loaded_0023_focal_decoder",
            )
            self.assertEqual(metadata["source_update"], 0)
    ...
    def test_formal_config_preserves_initialized_decoder_assets(self) -> None:
        ...
        self.assertEqual(result["schema_version"], "0024_focal_league_ppo_v1")
    ...
    def test_league_cli_has_no_mutable_initial_version_override(self) -> None:
    ...
    def test_worker_benchmark_cli_requires_explicit_output(self) -> None:
        args = build_parser().parse_args([
            ...
            "--output", ".tmp/evaluation/0024_worker_scaling/workers-128.json",
        ])
    ...
    def test_engine_worker_import_path_is_torch_light(self) -> None:
        completed = subprocess.run(
            [sys.executable, "-c", (
                ...
                "importlib.import_module('train.0024_marnies_grimmsnarl_ex_froslass_001_league_training.rollout.worker'); "
    ...
    def test_secondary_ogerpon_has_tenfold_live_sampling_only(self) -> None:
        plugins = load_deck_plugins(DEFAULT_DECK_ROOT)
        jobs = schedule_jobs(
            plugins, count=218, update=0, seed=3, runtime_root=Path("runtime")
        )
        counts = Counter((job.opponent_view, job.opponent_deck_id) for job in jobs)
        ordinary = "dragapult_ex_001"
        secondary = "rmy_teal_mask_ogerpon_001"
        self.assertEqual(
            counts[(LeaguePolicyView.LIVE, secondary)],
            10 * counts[(LeaguePolicyView.LIVE, ordinary)],
        )
        self.assertEqual(
            counts[(LeaguePolicyView.FROZEN, secondary)],
            counts[(LeaguePolicyView.FROZEN, ordinary)],
        )
