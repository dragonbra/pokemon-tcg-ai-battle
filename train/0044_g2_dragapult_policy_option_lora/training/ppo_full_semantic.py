"""0044 PPO with frozen semantic encoders and explicit strategy adapters."""
...
class PPOConfig:
    ...
    def validate(self) -> None:
        ...
        if self.protocol_version != "ppo_protocol_v2":
            raise ValueError("0044 formal PPO must use ppo_protocol_v2")
        if min(
            self.epochs,
            self.batch_size,
...
class PPOTrainer:
    def __init__(
        ...
        *,
        ...
        if config.meta_anchor_coef != model.integrated_flags.meta_anchor_coef:
            raise ValueError(
                "PPO meta_anchor_coef must match the selected 0044 preset: "
    ...
    def set_reference_model(self, reference_model: SemanticActorCritic) -> None:
        """Replace the complete immutable G2 KL anchor; optimizer state is untouched."""
    ...
    def _reference_log_probs(self, batch: PreparedBatch) -> torch.Tensor:
        ...
        with torch.no_grad():
            ...
            for start in range(0, batch.decisions, size):
                ...
                set_batch_own_archetype_ids(self.reference_model, batch, indices)
                validated, state, options, value, _, context = (
                    self.reference_model.encode_with_strategy(features)
                )
                evaluated = evaluate_actions_encoded(
    ...
    def update(self, batch: PreparedBatch, *, update: int = 0) -> dict[str, float]:
        ...
        for epoch, order in enumerate(orders, start=1):
            ...
            for indices in epoch_minibatches(order, self.config.batch_size):
                ...
                print(
                    f"\r[0044 PPO][update {update:04d} epoch {epoch}/{self.config.epochs}] "
