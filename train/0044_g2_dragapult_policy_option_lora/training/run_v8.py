"""V8: restart V6 U33 as local U0 with focal exact deck 003."""
...
SOURCE_PARENT_UPDATE = 33
PARENT_VERSION = "V6_u4_core16_benchmark_v2"
PARENT_ROOT = ROOT / "rl_runs/0044_g2_dragapult_policy_option_lora/versions" / PARENT_VERSION
PARENT_CHECKPOINT = PARENT_ROOT / "checkpoint/update-000033.pt"
PARENT_CHECKPOINT_SHA256 = "a7bc5a488ad031c07be96576113762b40cb75c51d46370d1e7eace22df818cbf"
VERSION_ROOT = ROOT / "rl_runs/0044_g2_dragapult_policy_option_lora/versions" / VERSION
WANDB_RUN_ID = "0044-v8-u33-deck003-core16-benchmark-v2"
FOCAL_DECK_ID = "003"
...
def readiness(*, version_root: Path = VERSION_ROOT) -> dict[str, object]:
    ...
    if missing:
        raise FileNotFoundError(f"V8 launch artifacts missing: {missing}")
    ...
    if sha256_file(PARENT_CHECKPOINT) != PARENT_CHECKPOINT_SHA256:
        raise RuntimeError("V8 parent checkpoint hash changed")
    ...
    if (
        ...
        raise RuntimeError("V8 parent is not exact V6 U33")
    ...
    if (
        ...
        raise RuntimeError("V8 low-LR contract changed")
    return {
        "schema_version": "0044_v8_launch_readiness_v1",
...
def main() -> int:
    ...
    run(
        ...
        wandb_name="0044 · V8 V6-U33 → local-U0 · Deck 003 · Core-16",
