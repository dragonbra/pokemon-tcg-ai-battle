...
from ..league.sampler import build_schedule, build_uniform_schedule
...
def readiness() -> dict[str, Any]:
    ...
    if registry.latest_champion_policy_id != "Champion-G2":
        raise RuntimeError("0044 V1 requires Champion-G2 as the registered latest champion")
    return {
...
def _jobs(
    ...
    league = schedule_builder(
        ...
        policy_ids=policy_ids, latest_champion_policy_id="Champion-G2",
        curriculum=curriculum,
    )
    if focal_deck_ids != (FOCAL_DECK_ID,):
        raise ValueError("0044 V1 focal deck must be exactly 007")
    ...
    jobs = [
        RolloutJob(
            ...
            focal_own_archetype_id=own_by_deck[FOCAL_DECK_ID],
...
def _checkpoint(model, update: int, *, version: str = VERSION) -> dict[str, Any]:
    return {
        ...
        "metadata": {
            ...
            "opponent_meta_class_count": 15, "focal_deck_id": FOCAL_DECK_ID,
            "parent_policy_id": "Champion-G2", "parent_checkpoint_update": 407,
        },
    }
...
def run(
    *, updates: int | None, wandb_mode: str, launch_formal: bool,
    ...
    if opponent_sampling_mode not in {"pfsp_mixture", "uniform_001_067"}:
        raise RuntimeError("formal opponent sampling mode is unsupported")
    if opponent_sampling_mode == "uniform_001_067" and parent_pfsp_state is not None:
    ...
    focal_deck = _cards(registry, FOCAL_DECK_ID)
    model, source = load_actor_critic(deck=focal_deck, device=device, integrated_flags=preset("FULL_MODEL"))
    ...
    if parent_checkpoint is not None:
        reference_model, _ = load_actor_critic(
            deck=focal_deck, device=device, integrated_flags=preset("FULL_MODEL")
        )
    ...
    _atomic_json(paths["artifact"] / "training_config.json", {
        ...
        "reference_anchor_policy_id": "Champion-G2",
        "reference_anchor_update": 407,
        "periodic_evaluation": {
    ...
    _atomic_json(status, {
        ...
        "parent_checkpoint": str(parent_checkpoint) if parent_checkpoint else None,
        "reference_anchor_policy_id": "Champion-G2", "reference_anchor_update": 407,
        "focal_deck_ids": list(focal_deck_ids),
        "focal_schedule": "fixed_007_v1",
        "opponent_sampling_mode": opponent_sampling_mode,
    })
    ...
    with TrainingLogger(paths["artifact"] / "training_metrics.jsonl", paths["tensorboard"]) as logger:
        ...
        while updates is None or update < updates:
            ...
            if periodic_evaluation_due(checkpoint_update):
            ...
            _atomic_json(status, {"state": "running", "checkpoint_update": checkpoint_update, "rollout_source_policy_update": update, "wandb_run_id": effective_run_id, "reference_anchor_policy_id": "Champion-G2", "reference_anchor_update": 407, "focal_deck_ids": list(focal_deck_ids), "focal_schedule": "fixed_007_v1"})
