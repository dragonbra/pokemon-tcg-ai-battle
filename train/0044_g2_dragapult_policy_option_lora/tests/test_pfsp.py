...
module = importlib.import_module("train.0044_g2_dragapult_policy_option_lora.league.pfsp")
...
def test_curriculum_is_frozen_for_ten_updates_and_refreshes_at_boundary() -> None:
    state = PFSPState()
    config = PFSPConfig(maximum_probability=1.0)
    kwargs = dict(
        deck_ids=("001", "002"), policy_ids=("Champion-G2",),
        config=config, training_deck_pool_hash="d" * 64,
        active_policy_pool_hash="p" * 64,
    )
    c0 = state.curriculum_for(0, **kwargs)
    state.observe(deck_id="001", policy_id="Champion-G2", result="loss", update=0)
...
def test_default_config_is_feasible_for_singleton_g2_policy_pool() -> None:
    curriculum = PFSPState().curriculum_for(
        0, deck_ids=("001",), policy_ids=("Champion-G2",),
        config=PFSPConfig(), training_deck_pool_hash="a",
        active_policy_pool_hash="b",
    )
    assert curriculum.policy_weights == {"Champion-G2": 1.0}
