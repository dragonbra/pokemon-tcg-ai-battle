...
assets_module = importlib.import_module("train.0044_g2_dragapult_policy_option_lora.assets")
...
routing = importlib.import_module("train.0044_g2_dragapult_policy_option_lora.cuda_engine_2.routing")
...
def test_all_256_lanes_resolve_numeric_decks_and_complete_policies() -> None:
    ...
    # The registry also contains evaluation-only historical anchors. Training
    # routing must consume only the explicitly admitted active pool.
    policies = list(assets.active_policy_ids)
    ...
    lanes = sampler.build_schedule(
        update=0, seed=430044001, deck_ids=decks, policy_ids=policies,
