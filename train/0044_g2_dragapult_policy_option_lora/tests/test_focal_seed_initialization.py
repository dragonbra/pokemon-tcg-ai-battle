...
taxonomy = importlib.import_module("train.0044_g2_dragapult_policy_option_lora.own_archetype")
...
def test_g1_is_unchanged_and_g2_rows_are_parent_copies():
    ...
    g2 = torch.load(ROOT.parents[1] / "rl_runs/0044_g2_dragapult_policy_option_lora/versions/V1_focal_002_007/checkpoint/update-000000.pt", map_location="cpu", weights_only=True)
...
def test_portable_parent_copy_and_manifest_identity():
    ...
    g2 = torch.load(ROOT.parents[1] / "rl_runs/0044_g2_dragapult_policy_option_lora/versions/V1_focal_002_007/artifact/focal_seed/model.bin", map_location="cpu", weights_only=True)
    for field in ("value_adapter_state_dict", "policy_strategy_adapter_state_dict"):
        ...
        for row in v2.classes[15:]:
            assert torch.equal(new[row.archetype_id], old[row.embedding_init_from])
    manifest = json.loads((ROOT.parents[1] / "rl_runs/0044_g2_dragapult_policy_option_lora/versions/V1_focal_002_007/artifact/focal_seed/manifest.json").read_text())
