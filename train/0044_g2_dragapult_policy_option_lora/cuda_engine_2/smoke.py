"""Run the bounded 0044 CUDA Engine 2.0 end-to-end policy smoke."""
...
def run(rule_pack: Path, build_dir: Path = DEFAULT_BUILD_DIR) -> dict[str, object]:
    ...
    for decisions in range(1, 1001):
        ...
        raise RuntimeError("0044 CUDA games did not finish within 1,000 decisions")
    torch.cuda.synchronize()
    statuses = [int(value) for value in arena.engine.statuses().cpu().tolist()]
    if statuses != [2, 2]:
        raise RuntimeError(f"0044 CUDA transition smoke failed: statuses={statuses}")
    return {
        "schema_version": "0044_cuda_engine_2_policy_transition_smoke_v1",
        "status": "PASS",
