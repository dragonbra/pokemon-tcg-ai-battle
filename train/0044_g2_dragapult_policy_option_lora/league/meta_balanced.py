...
EXPECTED_DECK_IDS = tuple(f"{value:03d}" for value in range(1, 68))
...
def _validated_members(mappings: Sequence[DeckMapping]) -> dict[int, tuple[str, ...]]:
    ...
    if tuple(sorted(deck_ids)) != EXPECTED_DECK_IDS or len(set(deck_ids)) != 67:
        raise ValueError("Meta-first schedule requires each exact deck 001-067 exactly once")
