"""The paired-0809 SemanticPolicy and Value head under the 0044 trainable boundary."""
...
class SemanticActorCritic(nn.Module):
    """0044 strategy-conditioned actor-critic over frozen semantic encoders."""
    ...
    def encode_with_strategy(self, batch: dict[str, Tensor]):
        ...
        return validated, state, options.policy_options, value, auxiliary, context
    ...
    def encode_dual_options(self, batch: dict[str, Tensor]):
        ...
        if self.policy_option_lora.is_zero_delta():
            # Exact G2 forward at U0 while retaining the LoRA gradient path.
    ...
    def assert_option_adaptation_contract(self) -> None:
        ...
        if trainable or parametrized:
            raise RuntimeError(
                "0044 immutable OptionEncoder base must be frozen and unparametrized; "
    ...
    def assert_trainable_contract(self) -> None:
        ...
        if value_ids.intersection(policy_ids):
            raise RuntimeError("0044 Value and Policy adapters share Parameter objects")
...
def load_actor_critic(
    ...
    if parameter_count != 56_352_322:
        raise ValueError(f"0044 focal actor parameter count mismatch: {parameter_count}")
