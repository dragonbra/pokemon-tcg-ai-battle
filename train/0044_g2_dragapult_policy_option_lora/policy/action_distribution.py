...
from .strategy_adapters import StrategyContext
...
def _decode(
    ...
    *,
    ...
    for step in range(int(maximum.max().item())):
        ...
        logits = head.logits(batch, options, state, context)
...
def sample_actions(
    ...
    *,
    ...
    with torch.inference_mode():
        batch, state, options, values, _, context = model.encode_with_strategy(features)
        return _decode(
            model.head,
            batch,
...
def sample_actions_with_encoding(
    ...
    *,
    ...
    with torch.inference_mode():
        ...
        batch, state, dual_options = model.encode_dual_options(features)
        options = dual_options.policy_options
        synchronize()
        encoded = time.perf_counter()
        values, auxiliary = model.value_and_aux_from_encoded(
            batch, state, dual_options.value_options
        )
        context = model.strategy_context(batch, values, auxiliary)
        synchronize()
        valued = time.perf_counter()
        actions = _decode(
            model.head, batch, model.actor_summary(state), options, values, context,
...
def greedy_actions(model: SemanticActorCritic, features: dict[str, Tensor]) -> list[SampledAction]:
    with torch.inference_mode():
        batch, state, options, values, _, context = model.encode_with_strategy(features)
        return _decode(
            model.head, batch, model.actor_summary(state), options, values, context, greedy=True
        )
...
def evaluate_actions(
    ...
    batch, state, options, value, _, context = model.encode_with_strategy(features)
