            ...
            for item in reference["shards"][source_split]:
                ...
                with gzip.open(path, "rt", encoding="utf-8") as handle:
                    for line in handle:
                        ...
                        if catalog is None:
                            raise ValueError(f"raw Episode is absent from 0032 catalog: {episode_id}")
...
def _compile_episode(
...
) -> list[tuple[Mapping[str, Any], dict[str, Any], dict[str, int]]]:
    ...
    for raw in rows:
        ...
        audit = audit_compiled_feature_input(raw, compiled, snapshot)
        output.append((raw, compiled, audit))
    return output
...
def _compiled_groups(
    ...
    workers: int,
    require_complete_catalog: bool,
) -> Iterator[list[tuple[Mapping[str, Any], dict[str, Any], dict[str, int]]]]:
    if workers < 1:
...
class _ShardWriter:
    def __init__(
        ...
        *,
        dataset_limit_bytes: int,
        linux_free_floor_bytes: int,
        c_free_floor_bytes: int,
    ):
        self.root = root
    ...
    def flush(self, split: str) -> None:
        ...
        rows.clear()
        guard_storage(self.root.parent, **self.storage_limits)
...
def materialize_canonical(
    ...
    *,
    ...
    if len(catalog_episodes) != len(catalog["episodes"]):
        raise ValueError("0032 catalog contains duplicate Episode IDs")
    ...
    processed_groups = 0
    feature_audit_counts: Counter[str] = Counter()
    try:
        for group in _compiled_groups(
            ...
            processed_groups += 1
            for raw, compiled, feature_audit in group:
                split = str(raw["split"])
                if (
                ...
                feature_audit_counts.update(feature_audit)
                counts[split] += 1
                for name in (
                    "card_cat",
                    "resource_cat",
        ...
        manifest = {
            ...
            "full_engine_prototype_sha256": _sha256(stage / "official_full_engine_prototypes_v2.json"),
            "decisions": sum(counts.values()),
            ...
            "feature_input_audit": {
                "status": "passed",
                "scope": "every canonical decision before shard write",
                **dict(sorted(feature_audit_counts.items())),
            },
            "storage_guard": {
                **initial_storage,
            ...
            + (stage / "official_full_engine_prototypes_v2.json").stat().st_size,
        }
