...
SCHEMA_VERSION = "0032_audited_winner_catalog_v1"
SPLIT_VERSION = "0032_episode_split_v1"
SPLIT_SEED = 20260802
...
def scan_archives(
    ...
    *,
    ...
    if not archive_paths and not patch_paths:
        raise ValueError("at least one archive or Episode JSON file is required")
    if workers < 1:
...
def merge_catalogs(base: dict[str, Any], delta: dict[str, Any]) -> dict[str, Any]:
    """Merge immutable catalog evidence and recompute the 0032 split/source vocabulary."""
    records_by_id: dict[int, dict[str, Any]] = {}
    duplicate_count = 0
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        help="Merge an already audited immutable prefix and recompute 0032 splits.",
    )
    parser.add_argument(
        "--normalize-existing-catalog",
        type=Path,
        help="Recommit an existing merged catalog under the current 0032 contract.",
    )
