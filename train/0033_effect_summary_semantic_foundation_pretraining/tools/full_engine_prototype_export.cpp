...
static void quoted(const std::string& value) {
    std::cout << '"';
...
int main() {
    ...
    std::cout << "{\"schema_version\":\"0031_official_full_engine_prototypes_v2\",\"cards\":[";
    for (std::size_t i = 0; i < card_ids.size(); ++i) {
        ...
        std::cout << "{\"card_id\":" << card.cardId
                  ...
                  << ",\"energy_type_mask\":" << static_cast<int>(card.energyType)
                  << ",\"tera\":" << card.tera
                  << ",\"energy_count\":" << static_cast<int>(card.energyCount)
                  << ",\"trash_my_turn_end\":" << card.trashMyTurnEnd
                  << ",\"cannot_to_hand_or_deck_in_trash\":" << card.cannotToHandOrDeckInTrash
                  << ",\"can_play_first_turn\":" << card.canPlayFirstTurn
                  << ",\"transform_only\":" << card.transformOnly
                  << ",\"can_trash\":" << card.canTrash
                  << ",\"to_bench\":" << card.toBench
                  << ",\"to_battle_field_only_setup\":" << card.toBattleFieldOnlySetup
                  << ",\"to_active_only_setup\":" << card.toActiveOnlySetup
                  << ",\"no_prize\":" << card.noPrize
                  << ",\"only_team_rocket\":" << card.onlyTeamRocket
                  ...
                  << ",\"erika\":" << card.erika
                  << ",\"larry\":" << card.larry
                  << ",\"team_rocket\":" << card.teamRocket
                  << ",\"ace_spec\":" << card.aceSpec
                  << ",\"can_use\":" << card.canUse
                  << ",\"number\":" << card.no
        ...
        std::cout << "],\"code\":";
        quoted(card.code);
        std::cout << ",\"evolves_from\":";
        quoted(card.evolvesFrom);
        std::cout << ",\"evolves_from_2\":";
        quoted(card.evolvesFrom2);
        std::cout << ",\"name\":";
        quoted(card.name);
        std::cout << ",\"name_en\":";
        quoted(card.nameEn);
    ...
    for (std::size_t i = 0; i < attack_ids.size(); ++i) {
        ...
        std::cout << ",\"damage_text\":";
        quoted(attack.damageText);
        std::cout << ",\"name\":";
        quoted(attack.name);
        std::cout << ",\"name_en\":";
        quoted(attack.nameEn);
        std::cout << ",\"text\":";
        quoted(attack.text);
        std::cout << ",\"text_en\":";
        quoted(attack.textEn);
        std::cout << ",\"last_cancel_fail_attack\":" << attack.lastCancelFailAttack;
        std::cout << '}';
    }
    std::cout << "]}\n";
}
