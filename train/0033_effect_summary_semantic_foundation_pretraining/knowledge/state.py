...
MOVE_CARD = 6
MOVE_CARD_REVERSE = 7
KNOWN_HAND_PUBLIC_DEPARTURES = frozenset({10, 11, 12})
LOG_NAMES = (
...
class CausalKnowledge:
    ...
    def _consume_opponent_hand(self, log: Mapping[str, Any], opponent: int) -> None:
        ...
        if log_type == DRAW_REVERSE:
            ...
            return
        if log_type in KNOWN_HAND_PUBLIC_DEPARTURES:
            serial = log.get("serial")
            if isinstance(serial, int) and not isinstance(serial, bool):
                self._known_opponent_hand.pop(serial, None)
            return
    ...
    def consume(
        ...
        if not isinstance(players, Sequence) or len(players) != 2:
            raise ValueError("0019 requires two-player observations")
