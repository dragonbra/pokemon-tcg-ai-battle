...
class WorkerLocalCompiler:
    ...
    def _validate(self, observation: Mapping[str, Any], *, require_options: bool) -> Mapping[str, Any]:
        current = observation.get("current")
        select = observation.get("select")
        if not isinstance(current, Mapping) or current.get("yourIndex") != self.actor:
            raise ValueError("0038 worker-local actor mismatch")
        if not isinstance(select, Mapping):
            raise ValueError("0038 worker-local observation has no select payload")
        options = select.get("option")
        if not isinstance(options, Sequence) or isinstance(options, (str, bytes)):
            raise ValueError("0038 worker-local observation has no legal options")
        ...
        if (
            ...
            raise ValueError("0038 worker-local selection bounds are invalid")
        if len(options) > 128 or minimum > 64:
            raise RuntimeError("observation exceeds 0038 inference bounds")
        if require_options and not options:
            raise ValueError("0038 tensor compilation requires at least one option")
        return select

    def observe_only(self, observation: Mapping[str, Any]):
        """Consume causal history without compiling tensors or calling a model."""
        self._validate(observation, require_options=False)
        return self.knowledge.consume(observation)

    def compile(self, observation: Mapping[str, Any]):
        select = self._validate(observation, require_options=True)
        ...
        snapshot = self.observe_only(observation)
