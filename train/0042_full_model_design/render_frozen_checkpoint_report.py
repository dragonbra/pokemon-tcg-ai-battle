"""Disabled historical seeded-512 report renderer.

Current 0042 reports are generated by the Policy-0809 CUDA-2048 formal path.
"""
...
def _catalog_rows() -> tuple[dict[str, dict[str, Any]], dict[str, Any]]:
    ...
    if len(rows) != 55 or {row["number"] for row in rows.values()} != {
        ...
        raise RuntimeError("Frozen-0809 presentation numbering is not exactly 001-055")
    focal = rows.get(FOCAL_DECK_ID)
    if focal is None or focal["number"] != "007":
        raise RuntimeError("Frozen-0809 focal Dragapult deck is not deck 007")
    return rows, catalog
...
def render(result: dict[str, Any], *, label: str, output: Path) -> dict[str, Any]:
    raise RuntimeError(
        "FATAL: legacy seeded-512 renderer is disabled for 0042 Policy-0809"
    )
    # Historical renderer retained below only for provenance.
    ...
    for game in result["games"]:
        if game["opponent_id"] not in catalog:
            raise ValueError(f"result contains an unknown Frozen-0809 deck: {game['opponent_id']}")
        by_opponent[game["opponent_id"]].append(game)
    if set(by_opponent) != set(catalog):
        raise ValueError("result does not cover all 55 Frozen-0809 decks")
...
<section><div class="candidate"><div><h2>我的卡组：007 · Dragapult ex</h2><p>Exact 60 cards · Frozen-0809 编号与 Policy-0809 报告完全一致</p></div><div class="representatives">{representative_images}</div></div><div class="deck-groups">{_card_groups(cards)}</div></section>
...
<div class="tools"><input id="search" type="search" placeholder="筛选 001–055 编号或牌型"></div><div class="table"><table><thead><tr><th>编号</th><th>Frozen-0809 卡组</th><th>对局</th><th>胜</th><th>负</th><th>平</th><th>胜率</th><th>先手胜/局</th><th>后手胜/局</th></tr></thead><tbody>{matchup_rows}</tbody></table></div></section>
