...
class InitializationContractTest(unittest.TestCase):
    ...
    def test_zero_gates_preserve_policy_value_and_mask(self) -> None:
    ...
    def test_manifest_declares_fresh_training_state(self) -> None:
        ...
        self.assertIn("g_V=g_pi=0", manifest["adapter_initialization"])
    ...
    def test_auxiliary_presets_preserve_core_rng_and_zero_shot_logits(self) -> None:
        ...
        integrated, _ = initialization.build_update0_model(
            self.deck, integrated_flags=presets.preset("FULL_MODEL")
        )
        base.assert_no_option_adaptation()
        integrated.assert_no_option_adaptation()
        with torch.inference_mode():
    ...
    def test_full_model_preset_loads_common_non_ppo_update0_strictly(self) -> None:
        presets = importlib.import_module(f"{PROJECT}.integrated.presets")
        model, _ = initialization.build_preset_from_common_update0(
            self.deck, presets.preset("FULL_MODEL"), device="cpu"
        )
        self.assertIsNotNone(model.prize_aux)
        self.assertEqual(float(model.value_adapter.gate), 0.0)
        self.assertEqual(float(model.policy_strategy_adapter.gate), 0.0)
