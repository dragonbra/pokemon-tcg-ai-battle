"""0042 model-only checkpoint contract; optimizer and rollout state are forbidden."""
...
CHECKPOINT_SCHEMA_VERSION = "0042_strategy_conditioned_model_only_v1"
MODEL_SCHEMA_VERSION = "0042_strategy_conditioned_v1"
ADAPTER_SCHEMA_VERSION = "0042_zero_gated_residual_v1"
...
def checkpoint_metadata(*, source_actor_sha256: str, source_value_sha256: str,
                        version: str = "V1_strategy_conditioned_base") -> dict[str, Any]:
    from .policy.own_archetype import (
    ...
    return {
        ...
        "own_archetype_vocabulary_version": OWN_ARCHETYPE_VOCABULARY_VERSION,
        "own_archetype_taxonomy_sha256": EXPECTED_TAXONOMY_SHA256,
        "no_option_lora": True,
...
def validate_model_only_payload(payload: dict[str, Any]) -> None:
    if payload.get("schema_version") != CHECKPOINT_SCHEMA_VERSION:
        raise ValueError("0042 checkpoint schema mismatch")
    present = sorted(FORBIDDEN_FIELDS.intersection(payload))
    if present:
        raise ValueError(f"0042 model-only checkpoint contains forbidden state: {present}")
    metadata = payload.get("metadata")
    expected = checkpoint_metadata(source_actor_sha256="", source_value_sha256="")
    if not isinstance(metadata, dict):
        raise ValueError("0042 checkpoint metadata missing")
    strict_metadata = {
        key: expected[key]
        for key in (
            "model_schema", "adapter_schema", "adapter_width", "own_embedding_dim",
            "own_archetype_vocabulary_version", "own_archetype_taxonomy_sha256",
            "no_option_lora",
        )
    }
    if {key: metadata.get(key) for key in strict_metadata} != strict_metadata:
        raise ValueError("0042 checkpoint strategy/taxonomy metadata mismatch")
    state = payload.get("state_dict")
    if not isinstance(state, dict):
        raise ValueError("0042 checkpoint state_dict missing")
    forbidden = [
        name for name in state
        if "lora" in name.lower() or ".parametrizations." in name
    ]
    if forbidden:
        raise ValueError(f"0042 checkpoint contains forbidden Option LoRA keys: {forbidden[:4]}")
    keys = (
    ...
    if actual_contract not in (expected_contract, LEGACY_ACTION_CONTRACT):
        raise ValueError("0042 checkpoint action contract mismatch")
