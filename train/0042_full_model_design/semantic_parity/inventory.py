"""Immutable path, weight, and runtime inventory for the 0042 parity audit."""
...
ARCHIVED_U230_INVENTORY_FIXTURE = (
    ROOT
    / "train/0042_full_model_design/tests/fixtures/semantic_parity_v1/"
...
print(json.dumps({
    ...
    'value_adapter_tensors': len(policy.value_adapter.state_dict()),
    'policy_strategy_adapter_tensors': len(policy.policy_strategy_adapter.state_dict()),
...
def build_runtime_inventory(*, checkpoint: Path, package_root: Path) -> dict[str, Any]:
    ...
    if portable_top_level != portable_required_top_level:
    ...
    component_hashes = {
        ...
        "value_adapter": _tensor_state_sha256(
            (name, value) for name, value in state.items()
            if name.startswith("value_adapter.")
        ),
        "policy_strategy_adapter": _tensor_state_sha256(
            (name, value) for name, value in state.items()
            if name.startswith("policy_strategy_adapter.")
        ),
    }
    ...
    portable_component_hashes = {
        ...
        "value_adapter": _tensor_state_sha256(
            package_model["value_adapter_state_dict"].items()
        ),
        "policy_strategy_adapter": _tensor_state_sha256(
            package_model["policy_strategy_adapter_state_dict"].items()
        ),
    }
    ...
    return {
        "schema_version": "0042_semantic_parity_runtime_inventory_v1",
        "git_commit": _git_commit(),
        "git_dirty": _git_dirty(),
        "paths": {
            "cuda_rollout_engine_entry": "train/0042_full_model_design/rollout/cuda_collector.py:CudaFullSemanticRolloutCollector.collect",
            "cuda_agent_entry": "engine_cuda/python/ptcg_cuda_engine/semantic0031_resident.py:run_resident_greedy_jobs",
            "cuda_model_entry": "train/0042_full_model_design/training/run_full_semantic.py:build_collector",
            "frozen_schedule_entry": "train/0042_full_model_design/evaluation/frozen_jobs.py:build_frozen_jobs",
            "package_build_entry": "train/0042_full_model_design/export_full_semantic_candidate.py:export_candidate",
            "package_agent_entry": "main.py:agent",
            "decision_gate": "train/0042_full_model_design/action_boundary/decision_gate.py:DecisionGate",
            "cuda_action_boundary": "train/0042_full_model_design/rollout/cuda_action_boundary.py:CudaActionBoundaryAdapter",
            "package_action_boundary": "strategy/deployment/compound_inference.py:PortableCompoundSemanticPolicy",
            "macro_planner": "train/0042_full_model_design/action_boundary/macro_planner.py:MacroPlanner",
            "official_pending_transaction": "train/0042_full_model_design/action_boundary/macro_protocol.py:PendingMacroTransaction",
            "official_observe_only": "train/0042_full_model_design/rollout/worker_compiler.py:WorkerLocalCompiler.observe_only",
        },
        "checkpoint": {
        ...
        "package": {
            ...
            "strict_load": (
                portable_top_level == portable_required_top_level
