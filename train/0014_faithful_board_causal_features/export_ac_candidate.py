"""Export a V9 AC checkpoint as a self-contained arena candidate."""
...
from strategy.ac_inference import AllFeaturePolicy

DECK = [int(line) for line in (ROOT / "deck.csv").read_text(encoding="utf-8").splitlines() if line.strip()]
_POLICY = AllFeaturePolicy.from_checkpoint(
...
def export_candidate(
    ...
    if metadata.get("schema_version") != "0014_ac_training_v1":
        raise ValueError("checkpoint is not a 0014 AC policy")
    ...
    copies = {
        ...
        "ac_inference.py": "ac_inference.py",
    ...
    manifest = {
        "schema_version": "0014_ac_candidate_v1",
