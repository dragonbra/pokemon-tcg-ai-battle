"""CPU inference session for the 0014 R8 faithful R2 option-only policy."""
...
class R8Policy:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R8 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R8 checkpoint metadata is missing model.ac.base")
        ...
        model = R8R2OptionOnlyPolicy(config, ontology_path=ontology_path)
