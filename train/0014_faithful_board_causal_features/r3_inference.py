"""CPU inference session for the 0014 R3 early-fusion policy."""
...
class R3Policy:
    def __init__(
        self,
        model: R3EarlyScenarioFusionPolicy,
        deck: Sequence[int],
        config: IDOnlyConfig,
    ) -> None:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R3 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R3 checkpoint metadata is missing model.ac.base")
