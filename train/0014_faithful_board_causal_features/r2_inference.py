"""CPU inference session for the 0014 R2 strong scenario policy."""
...
class R2Policy:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R2 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        ...
        model = R2StrongScenarioPolicy(config, ontology_path=ontology_path)
