"""Export a stopped 0014 A0 checkpoint as a self-contained arena candidate."""
...
def export_candidate(checkpoint: Path, source_package: Path, output: Path) -> dict[str, Any]:
    """Make a physical package; no runtime import reaches a numbered train project."""
    if output.exists():
    ...
    if not isinstance(model_config, dict):
        raise ValueError("0014 A0 checkpoint metadata is missing model configuration")
    ...
    shutil.copy2(project / "base_model.py", strategy / "model.py")
    ...
    manifest = {
        "schema_version": "0014_a0_candidate_v1",
...
def main() -> None:
    ...
    print(json.dumps(export_candidate(**vars(arguments)), ensure_ascii=False, indent=2, sort_keys=True))
