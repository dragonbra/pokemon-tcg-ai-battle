"""Immutable full-feature shards with a physically isolated faithful-0010 view."""
...
def _compact_records(records: list[Mapping[str, Any]]) -> dict[str, Tensor]:
    ...
    tensors: dict[str, Tensor] = {
        ...
        "a0_eligible": torch.tensor([record["a0_eligible"] for record in records], dtype=torch.bool),
...
def build_materialized_dataset(
    ...
    *,
    ...
    try:
        rows = (record.as_dict() for record in _raw_shards.iter_dataset(raw_root))
        ...
        if compiler_sha256() != compiler_commitment or _sha256(Path(__file__)) != materializer_commitment:
...
def iter_a0_batches(
    ...
    *,
    ...
    for expected in shards:
        ...
        if shuffle:
            eligible = eligible[torch.randperm(eligible.numel(), generator=torch.Generator().manual_seed(rng.randrange(2**63)))]
