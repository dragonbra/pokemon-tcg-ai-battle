"""Shared runtime allocation for self-contained 0014 feature-reader model families."""
...
from .a0_trainer import train
from .ac_data import iter_ac_batches
from .materialized import validate_materialized_dataset
...
def run_feature_model(
    *,
    ...
    parser = argparse.ArgumentParser(description=f"Train 0014 {family.upper()} policy")
    ...
    parser.add_argument("--smoke-tag", default=f"V0_{family}_noncanonical_smoke")
    arguments = parser.parse_args()
    if not torch.cuda.is_available():
        raise RuntimeError(f"0014 {family.upper()} requires CUDA")
    ...
    model = model_class(
        model_config, ontology_path=MODEL_READY_DATASET / "card_ontology.json"
    )
        ...
        os.environ.update(
            {
                ...
                "WANDB_JOB_TYPE": "bc",
                "WANDB_TAGS": f"0014,{family},all_features",
            }
        )
