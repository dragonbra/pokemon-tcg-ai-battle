"""CPU inference session for the 0014 R16 policy."""
...
class R16Policy:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R16 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R16 checkpoint metadata is missing model.ac.base")
        ...
        model = R16RuleContractPolicy(config, ontology_path=ontology_path)
