"""CPU inference session for the 0014 R13 policy."""
...
class R13Policy:
    def __init__(
        self, model: R13GradualOptionScalePolicy, deck: Sequence[int], config: IDOnlyConfig
    ) -> None:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R13 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R13 checkpoint metadata is missing model.ac.base")
        ...
        config = R13ModelConfig(
            ...
            option_evidence_dropout=float(raw["option_evidence_dropout"]),
            option_initial_scale=float(raw["option_initial_scale"]),
        )
        model = R13GradualOptionScalePolicy(config, ontology_path=ontology_path)
