"""CPU inference session for the 0014 R7 regularized option-query policy."""
...
class R7Policy:
    def __init__(
        self,
        model: R7RegularizedOptionQueryPolicy,
        deck: Sequence[int],
        config: IDOnlyConfig,
    ) -> None:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R7 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R7 checkpoint metadata is missing model.ac.base")
        ...
        model = R7RegularizedOptionQueryPolicy(config, ontology_path=ontology_path)
