...
def _paths(root: Path, version: str) -> VersionPaths:
    run = root / "rl_runs/0022_league_training/versions" / version
    ...
    return VersionPaths(
        project_id="0022_league_training",
        version_name=version,
        project_archive=root / "experiments/0022_league_training",
        run_root=run,
        artifact=artifact,
        checkpoints=checkpoint,
        tensorboard=run / "tensorboard",
        wandb=run / "wandb",
        evaluation=root / f"experiments/0022_league_training/evaluation/{version}.html",
