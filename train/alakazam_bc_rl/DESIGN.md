...
```text
官方 cg battle trace
  -> train.alakazam_bc_rl.training.build_bc_dataset
  -> ptcg_bc_v1 JSONL
  -> train.alakazam_bc_rl.training.train_behavior_cloning
  -> best_validation.pt
...
策略改进。失败分支的报告仍保存在对应的 `rl_runs/<run-name>/evaluation/`，不得用过程
指标替代结果护栏。
...
`train/alakazam_bc_rl/training/train_ppo.py` 已提供第一版 masked PPO-style terminal reward 微调器。它从
...
`train/alakazam_bc_rl/training/rewards.py` 的第一版 transition shaping 只使用可见的 Prize race、攻击准备度
