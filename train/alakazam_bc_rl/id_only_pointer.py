"""Reference ID-only pointer policy from the Yushin daily-winner Notebook.

This module intentionally preserves the reference representation: public card
IDs and structural locations only, without card metadata, deck conditioning,
expert conditioning, or hand-authored card semantics.  It exists solely as the
controlled baseline for experiment 0025.
"""
...
class IDOnlyCodec:
    ...
    def encode(self, observation: dict[str, Any], action: list[int]) -> dict[str, Any] | None:
        current = _get(observation, "current", {}) or {}
        select = _get(observation, "select", {}) or {}
        options = _as_list(select.get("option"))
        if not options or not valid_reference_action(action, select):
            return None
        ...
        return {
            ...
            "action": list(action),
            "min_count": min(max(_as_int(select.get("minCount"), 0), 0), self.config.max_action_steps),
            "max_count": min(max(_as_int(select.get("maxCount"), 0), len(action)), self.config.max_action_steps),
        }
...
def collate_id_only(rows: list[dict[str, Any]]) -> dict[str, Tensor]:
    ...
    batch_size = len(rows)
    entity_count = max(len(row["entity_cat"]) for row in rows)
