"""Train the 0032 audited semantic policy on official winner decisions."""
...
DATASET_ROOT = Path(
    "rl_runs/0034_clean_recent3_semantic_bc/dataset/"
    "V1_recent_winners_20260728_20260801"
)
PROTOTYPES = Path(
    "train/0034_clean_recent3_semantic_bc/assets/"
    "official_public_prototypes_v1.json"
)
ARM = "audited_semantic"
...
def _smoke_paths():
    ...
    root = Path(".tmp/0032_audited_bc_smoke") / f"run-{uuid.uuid4().hex[:10]}"
...
def main() -> None:
    ...
    parser.add_argument(
        "--version", default="V1_audited_foundation"
    )
    ...
    if not args.smoke and not torch.cuda.is_available():
        raise RuntimeError("formal 0032 audited BC training requires CUDA")
        ...
        os.environ.update(
            {
                ...
                "WANDB_NAME": f"0032 audited semantic {paths.version_name}",
                "WANDB_RUN_GROUP": PROJECT_ID,
                "WANDB_RUN_ID": wandb_run_id(PROJECT_ID, paths.version_name),
                "WANDB_JOB_TYPE": "bc",
                "WANDB_TAGS": "0032,audited,semantic,winner_bc",
            }
        )
    ...
    print(
        json.dumps(
            {
                "event": "0032_audited_bc_preflight",
    ...
    print(
        json.dumps(
            {
                "event": "0032_audited_bc_complete",
                "run_root": str(paths.run_root),
                "best": summary["best"],
            },
            sort_keys=True,
