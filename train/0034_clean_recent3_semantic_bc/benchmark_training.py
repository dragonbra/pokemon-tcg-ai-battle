"""Benchmark the formal 0032 train path without allocating a tracked version."""
...
def _train_epoch(*args: Any, **kwargs: Any) -> Any:
    """Import the shared logging stack only when a benchmark actually runs."""
    from .training.trainer import _train_epoch as implementation
...
def benchmark(
    *,
    ...
    if not torch.cuda.is_available():
        raise RuntimeError("0032 training benchmark requires CUDA")
    random.seed(seed)
    np.random.seed(seed)
    ...
    return {
        "schema_version": "0032_training_throughput_benchmark_v1",
