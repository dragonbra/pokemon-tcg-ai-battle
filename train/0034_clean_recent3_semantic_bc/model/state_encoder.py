...
class StateEncoder(nn.Module):
    ...
    def __init__(self, config: ModelConfig, prototypes: OfficialPrototypeEncoder):
        ...
        self.card_cat = CategoricalFields(CARD_CAT_VOCABS, d, skip_indices=(0,))
        self.card_num = NumericFields(WIDTHS.card_num, d)
        self.card_parent = nn.Linear(d, d, bias=False)
        self.resource_cat = CategoricalFields(RESOURCE_CAT_VOCABS, d, skip_indices=(0,))
        self.resource_num = NumericFields(WIDTHS.resource_num, d)
        self.event_cat = CategoricalFields(
            EVENT_CAT_VOCABS,
            d,
            skip_indices=(2, 3, 4, 7, 8, 9, 10),
        )
        ...
        self.event_source_relation = nn.Linear(d, d, bias=False)
        self.event_target_relation = nn.Linear(d, d, bias=False)
    ...
    def forward(
        ...
        events = (
            ...
            + prototype_memory.attack(batch.event_cat[..., 4])
            + self.segment.weight[4]
        )
        for projection, card_column in zip(
            self.event_card_roles,
            (2, 3, 7, 8, 9, 10),
            strict=True,
        ):
            events = events + projection(
                prototype_memory.card(batch.event_cat[..., card_column])
            )
        events = (
            events
            + self.event_source_relation(gather_one_based(cards, batch.event_source))
            + self.event_target_relation(gather_one_based(cards, batch.event_target))
        )
