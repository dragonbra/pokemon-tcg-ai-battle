...
BENCHMARK_SCHEMA_VERSION = "0035_lifetime_feature_benchmark_v2"
...
def _timing_summary(nanoseconds: Sequence[int]) -> dict[str, float]:
    ...
    return {
        "median": median,
        "mad": float(statistics.median(abs(value - median) for value in milliseconds)),
        "p95": _percentile(milliseconds, 0.95),
        "total": float(sum(milliseconds)),
    }


def _numeric_snapshot(value: object) -> dict[str, int | float] | None:
    """Read a public counter snapshot without coupling to one compiler design."""
...
def _compiler_counters(compiler: object) -> dict[str, dict[str, int | float]]:
    """Expose optional compiler diagnostics while keeping the benchmark generic."""

    result: dict[str, dict[str, int | float]] = {}
    for public_name in ("stats", "work_counters"):
        snapshot = _numeric_snapshot(getattr(compiler, public_name, None))
        if snapshot is not None:
            result[public_name] = snapshot
    return result


def _merge_counter_snapshots(
    target: dict[str, dict[str, int | float]],
    snapshot: Mapping[str, Mapping[str, int | float]],
) -> None:
    """Sum count-like values across fresh chronological compiler sessions.

    Derived rates are deliberately omitted: adding rates across sessions is not
    meaningful, while their numerator/denominator counters remain available.
    """

    for family, values in snapshot.items():
        merged = target.setdefault(family, {})
        for name, value in values.items():
            if isinstance(value, int):
                merged[name] = int(merged.get(name, 0)) + value
...
def _measure(
    ...
    *,
    ...
    compiler_counter_totals: dict[str, dict[str, int | float]] = {}
    compiler_sessions = 0
    compiled = 0
    ...
    if compiler_sessions:
        result["compiler_diagnostics"] = {
            "sessions": compiler_sessions,
...
def _aggregate_repetitions(
    ...
    """Keep the legacy result surface and add robust across-run dispersion."""

    if not measurements:
        raise ValueError("cannot aggregate an empty benchmark repetition set")
    first = measurements[0]
    stages = first["milliseconds_per_decision"]
    milliseconds: dict[str, dict[str, float]] = {}
    across_repetitions: dict[str, Any] = {
        "decisions_per_second": _distribution_summary(
            [float(item["decisions_per_second"]) for item in measurements]
        ),
        "milliseconds_per_decision": {},
    }
    for stage in stages:
        stage_summaries = [item["milliseconds_per_decision"][stage] for item in measurements]
        # The legacy fields now describe the median repetition rather than an
        # order-biased single arm.  MAD is within-decision dispersion, itself
        # aggregated across repetitions.
        milliseconds[stage] = {
            metric: float(statistics.median(summary[metric] for summary in stage_summaries))
            for metric in ("median", "mad", "p95", "total")
        }
        across_repetitions["milliseconds_per_decision"][stage] = {
            metric: _distribution_summary(
                [float(summary[metric]) for summary in stage_summaries]
            )
            for metric in ("median", "p95")
        }
    result: dict[str, Any] = {
        "compiled_decisions": int(first["compiled_decisions"]),
        "decisions_per_second": across_repetitions["decisions_per_second"]["median"],
        "milliseconds_per_decision": milliseconds,
        "repetitions": list(measurements),
        "across_repetitions": across_repetitions,
    }
    diagnostic_runs = [
        item["compiler_diagnostics"]
        for item in measurements
        if isinstance(item.get("compiler_diagnostics"), Mapping)
    ]
    if diagnostic_runs:
        result["compiler_diagnostics_per_repetition"] = diagnostic_runs
    return result
...
def run_benchmark(
    *,
    ...
    for repetition in range(repetitions):
        # Alternating the leading arm removes the fixed full-then-incremental
        # thermal/order bias while retaining a paired result for every repeat.
        order = list(measured_modes)
        if len(order) == 2 and repetition % 2:
            order.reverse()
        paired: dict[str, Any] = {"index": repetition, "order": order, "results": {}}
        for measured_mode in order:
            measurement = _measure(
                trajectories, mode=measured_mode, decisions=decisions
            )
            measurements[measured_mode].append(measurement)
            paired["results"][measured_mode] = measurement
        paired_repetitions.append(paired)
    results = {
        measured_mode: _aggregate_repetitions(measurements[measured_mode])
        for measured_mode in measured_modes
    }
