"""Benchmark the 0034 actor inside the fully resident official CUDA loop."""
...
def main() -> int:
    ...
    model_module = importlib.import_module("train.0037_dragapult_value_initialized_rl.model")
    ...
    with torch.inference_mode():
        ...
        torch.cuda.nvtx.range_push("0034_pod_native_resident_rollout")
        total_start.record()
    ...
    result = {
        "schema": "0034_pod_native_resident_throughput_v1",
        "passed": (
            decision_count > 0
            and illegal_count == 0
        ...
        "scope": "official CUDA engine + PolicyCodecV1 + 0034 actor fully resident rollout",
