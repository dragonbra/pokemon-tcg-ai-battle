"""Generate the self-contained, image-rich 0043 exact-deck asset browser."""
...
LEGACY_REPORT_ROOT = REPOSITORY_ROOT / "experiments/0043_champion_league_rl/deck_taxonomy"
...
def _text(profile: dict[str, Any], vocabulary: OwnArchetypeVocabulary) -> str:
    lines = [
        f"0043 DECK {profile['deck_id']} — {profile['name']}", "=" * 78, "", "IDENTITY",
...
def _detail_html(profile: dict[str, Any], vocabulary: OwnArchetypeVocabulary, previous_id: str | None, next_id: str | None) -> str:
    ...
    return f"""<!doctype html><html lang='zh-CN'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>0043 卡组 {profile['deck_id']} · {escape(profile['name_cn'])}</title><style>{BASE_CSS}</style></head><body>
<header><h1>{profile['deck_id']} · {escape(profile['name_cn'])}</h1><p>英文资产名：{escape(profile['name'])}</p><p>0043 精确 60 张卡组资产 · Meta 分类（Own Archetype V2）· Policy-0809 CUDA-2048 报告视觉语言</p></header><main>
...
def _index_html(profiles: list[dict[str, Any]], vocabulary: OwnArchetypeVocabulary) -> str:
    ...
    return f"""<!doctype html><html lang='zh-CN'><head><meta charset='utf-8'><meta name='viewport' content='width=device-width,initial-scale=1'><title>0043 · 67 Exact Deck Assets</title><style>{BASE_CSS}</style></head><body>
<header><h1>0043 · 001–067 精确卡组资产</h1><p>67 套正式训练卡组；页面参照 Policy-0809 CUDA-2048 正式评测报告的视觉语言，直接从项目内精确卡组注册表、Own Archetype V2 和官方卡牌数据生成。</p></header><main>
...
def generate() -> dict[str, Any]:
    ...
    manifest = {
        "schema_version": "0043_deck_asset_browser_v2", "deck_count": len(registry.decks),
    ...
    pointer = "<!doctype html><meta charset='utf-8'><title>0043 Deck Assets moved</title><meta http-equiv='refresh' content='0;url=../../../train/0043_champion_league_rl/assets/decks/index.html'><p><a href='../../../train/0043_champion_league_rl/assets/decks/index.html'>打开 canonical 0043 Deck Asset Browser</a></p>\n"
