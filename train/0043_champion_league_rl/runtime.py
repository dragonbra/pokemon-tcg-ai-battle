...
def audit_runtime_tree() -> str:
    ...
    if manifest.get("schema_version") != "0043_semantic_runtime_manifest_v1":
...
def _deck(deck_id: str) -> tuple[int, ...]:
    ...
    if deck is None:
        raise AssetIntegrityError(f"unregistered 0043 deck: {deck_id}")
...
def load_policy(policy_id: str, *, deck_id: str):
    ...
    if policy is None:
        raise AssetIntegrityError(f"unregistered 0043 policy: {policy_id}")
