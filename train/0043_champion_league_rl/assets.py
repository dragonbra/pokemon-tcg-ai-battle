"""Fail-closed loaders for 0043 project-local immutable assets."""
...
POLICY_REGISTRY_SCHEMA = "0043_policy_registry_v1"
EVALUATION_REGISTRY_SCHEMA = "0043_evaluation_registry_v1"


class AssetIntegrityError(RuntimeError):
    """An immutable 0043 asset cannot be reconstructed exactly."""
...
class AssetRegistry:
    ...
    def load(cls, project_root: Path) -> "AssetRegistry":
        ...
        if deck_payload.get("schema_version") != DECK_REGISTRY_SCHEMA:
            raise AssetIntegrityError("unsupported 0043 deck registry schema")
        if policy_payload.get("schema_version") != POLICY_REGISTRY_SCHEMA:
            raise AssetIntegrityError("unsupported 0043 policy registry schema")
        if evaluation_payload.get("schema_version") != EVALUATION_REGISTRY_SCHEMA:
            raise AssetIntegrityError("unsupported 0043 evaluation registry schema")
        decks = tuple(
            DeckAsset(
    ...
    def validate_all(self) -> AssetAudit:
        ...
        if tuple(deck.deck_id for deck in self.decks) != expected_ids:
            raise AssetIntegrityError(
                "0043 deck IDs must be ordered, contiguous zero-padded numeric identities"
            )
