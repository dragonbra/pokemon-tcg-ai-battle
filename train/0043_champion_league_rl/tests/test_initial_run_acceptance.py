...
def test_balanced_focal_schedule_covers_all_67_decks_every_update() -> None:
    ...
    lanes = module.balanced_focal_schedule(430043918, deck_ids=decks)
...
def test_balanced_focal_schedule_is_seeded_and_rotates_extra_lanes() -> None:
    decks = tuple(f"{index:03d}" for index in range(1, 68))
    first = module.balanced_focal_schedule(430043918, deck_ids=decks)
    same = module.balanced_focal_schedule(430043918, deck_ids=decks)
    next_update = module.balanced_focal_schedule(430043919, deck_ids=decks)
    assert first == same
