...
def test_complete_imported_asset_registry_passes() -> None:
    ...
    assert audit.policy_ids == ("Champion-G1", "Champion-G2", "Policy-0809")
...
def test_policy_paths_use_semantic_directories_not_hash_directories() -> None:
    ...
    assert set(paths) == {"Policy-0809", "Champion-G1", "Champion-G2"}
    assert not (PROJECT_ROOT / "assets/policies/blobs").exists()


def test_frozen_meta_pool_is_distinct_from_training_pool() -> None:
    registry = AssetRegistry.load(PROJECT_ROOT)
    evaluation = registry.evaluations[0]
    assert evaluation.evaluation_id == "FrozenMeta256-V1"
    assert evaluation.deck_pool_role == "evaluation"
    assert evaluation.policy_id == "Policy-0809"
    assert sum(row.games for row in evaluation.entries) == 256
    assert len(evaluation.entries) == 55
    assert {row.deck_id for row in evaluation.entries} == {
