...
gate = importlib.import_module(
    "train.0043_champion_league_rl.evaluation.g2_candidate_gate"
)
renderer = importlib.import_module(
    "train.0043_champion_league_rl.evaluation.render_g2_candidate_gate"
)


def test_seeded_cohorts_are_exact_and_disjoint() -> None:
    assert gate.RANDOM_SAMPLE_SEED == 430043407
...
def test_u407_checkpoint_gate_rejects_wrong_update(tmp_path: Path) -> None:
    ...
    torch.save({"schema_version": "0043_focal_v1_model_only_v1", "update": 406}, checkpoint)
