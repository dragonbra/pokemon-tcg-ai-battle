"""0043 PPO with frozen semantic encoders and explicit strategy adapters."""
...
class PPOConfig:
    ...
    def validate(self) -> None:
        ...
        if self.protocol_version != "ppo_protocol_v2":
            raise ValueError("0043 formal PPO must use ppo_protocol_v2")
        if min(
            self.epochs,
            self.batch_size,
        ...
        if self.meta_anchor_coef <= 0:
            raise ValueError("0043 meta_anchor_coef must be explicitly positive")
        if min(
    ...
    def __init__(
        ...
        *,
        ...
        if config.meta_anchor_coef != model.integrated_flags.meta_anchor_coef:
            raise ValueError(
                "PPO meta_anchor_coef must match the selected 0043 preset: "
    ...
    def update(self, batch: PreparedBatch, *, update: int = 0) -> dict[str, float]:
        ...
        for epoch, order in enumerate(orders, start=1):
            ...
            for indices in epoch_minibatches(order, self.config.batch_size):
                ...
                print(
                    f"\r[0043 PPO][update {update:04d} epoch {epoch}/{self.config.epochs}] "
