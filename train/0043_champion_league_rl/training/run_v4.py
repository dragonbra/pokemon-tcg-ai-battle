"""V4 restart of the 001-067 generalist run after V3's GPU-driver failure."""
...
VERSION = "V4_generalist_focal_001_067_u208_restart"
START_UPDATE = 208
FOCAL_DECK_IDS = tuple(f"{index:03d}" for index in range(1, 68))
PARENT_ROOT = ROOT / "rl_runs/0043_champion_league_rl/versions/V3_generalist_focal_001_067"
PARENT_CHECKPOINT = PARENT_ROOT / "checkpoint/update-000208.pt"
PARENT_PFSP_STATE = PARENT_ROOT / "artifact/pfsp_state.json"
VERSION_ROOT = ROOT / "rl_runs/0043_champion_league_rl/versions" / VERSION
WANDB_RUN_ID = "0043-v4-generalist-focal-001-067-u208-restart"
...
def readiness(*, version_root: Path = VERSION_ROOT) -> dict[str, object]:
    ...
    if training_decks != FOCAL_DECK_IDS:
        raise RuntimeError("V4 focal pool is not exact 001-067")
    required = (
    ...
    if parent.get("schema_version") != "0043_focal_v1_model_only_v1":
        raise RuntimeError("V4 parent checkpoint schema changed")
    if parent.get("update") != START_UPDATE:
        raise RuntimeError("V4 parent checkpoint is not U208")
    state = parent["state_dict"]
    if state["value_head.heads.archetype.1.weight"].shape != (15, 320):
        raise RuntimeError("V4 parent opponent Meta head changed")
    own_keys = (
    ...
    if any(state[key].shape != (29, 16) for key in own_keys):
        raise RuntimeError("V4 parent own-deck embedding shape changed")
    ...
    return {
        "schema_version": "0043_v4_generalist_restart_readiness_v1",
        ...
        "parent": {
            ...
            "pfsp_note": "includes completed source-U208 rollout observations discarded from failed PPO U209",
        },
        "focal": {
            "deck_ids": list(FOCAL_DECK_IDS), "lanes_per_rollout": 256,
            "per_deck_games": [3, 4],
            "schedule": "seeded_frequency_balanced_random_v1",
            "resident_cohort_key": "opponent_policy_id_only",
        },
        "optimizer": {"initialization": "fresh", "ppo_contract": "unchanged"},
...
def main() -> int:
    ...
    run(
        ...
        wandb_name="0043 · V4 generalist 001-067 · U208 restart",
