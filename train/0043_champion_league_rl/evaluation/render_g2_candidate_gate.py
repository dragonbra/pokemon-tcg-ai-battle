...
def aggregate(*, allow_partial: bool = False) -> dict[str, Any]:
    ...
    return {
        "schema_version": "0043_u407_g2_candidate_gate_aggregate_v1",
        "status": (
...
"""
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>0043 U407 G2 Candidate Gate</title><style>{style}</style></head><body><main>
<header><p>0043 CHAMPION LEAGUE RL · PROMOTE DIAGNOSTIC</p><h1>U407 · G2 Candidate Gate</h1><p>Champion-G1 vs U407 G2 Candidate · Frozen Policy-0809 · 已完成 {payload['completed_decks']}/{payload['target_decks']} focal decks × 2 arms × CUDA-2048</p></header>
...
"""
    return f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{row['deck_id']} · G1 vs U407</title><style>{style}</style></head><body><main><div class="hero"><div><a class="back" href="../index.html">← 返回总览</a><p>0043 G2 CANDIDATE · CUDA-2048</p><h1>{row['deck_id']} · {html.escape(row['display_name'])}</h1></div><div class="hero-art">{cards}</div></div>
...
def publish(*, allow_partial: bool = False) -> Path:
    ...
    reverse_temporary.write_text(json.dumps({
        "schema_version": "0043_evaluation_reverse_link_v1",
