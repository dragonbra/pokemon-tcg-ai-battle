...
def validate_u407_checkpoint(path: Path = U407_CHECKPOINT) -> dict[str, Any]:
    ...
    if (
        payload.get("schema_version") != "0043_focal_v1_model_only_v1"
...
def initial_state() -> dict[str, Any]:
    return {
        "schema_version": "0043_u407_g2_candidate_gate_state_v1",
...
def _g1_candidate(deck: tuple[int, ...], deck_id: str, own_id: int, output: Path,
    ...
    # Strict-load their identical tensors into 0043-native adapters so the
...
def run_arm(*, deck_id: str, arm: str, output_root: Path) -> dict[str, Any]:
    ...
    report = {
        "schema_version": "0043_u407_g2_candidate_arm_v1",
