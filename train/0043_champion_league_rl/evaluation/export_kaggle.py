"""Export one 0043 checkpoint as a self-contained Kaggle submission package."""
...
BASE_PORTABLE = (
    ROOT / "rl_runs/0043_champion_league_rl/versions/"
    "V1_focal_002_007/artifact/focal_seed/model.bin"
)
PACKAGE_SCHEMA = "0043_champion_league_kaggle_package_v1"

MAIN = '''"""Kaggle entrypoint for one immutable 0043 deployment identity."""
import hashlib
import json
import os
from pathlib import Path
...
def _manifest():
    ...
    if payload.get("schema_version") != "0043_champion_league_kaggle_package_v1":
        raise RuntimeError("0043 Kaggle package schema mismatch")
    ...
    if not isinstance(expected, dict) or actual != set(expected):
        raise RuntimeError("0043 Kaggle package file inventory mismatch")
    mismatched = [name for name, digest in expected.items() if _sha256(ROOT / name) != digest]
    if mismatched:
        raise RuntimeError(f"0043 Kaggle package hash mismatch: {mismatched}")
    return payload
...
def export(
    *, checkpoint: Path, deck_id: str, output: Path, archive: Path,
    ...
    if output.exists() or archive.exists():
        raise FileExistsError("0043 Kaggle output/package archive already exists")
