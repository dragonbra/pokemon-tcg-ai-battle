"""Build and validate immutable corrected 0021 full-feature shards."""
...
SCHEMA_VERSION = "0021_actor_relative_option_set_cache_v1"
TERMINATION_CODES = {"forced_max": 1, "optional_stop": 2}
DECK_OOD_SPLIT_SEED = "0021_deck_ood_v1"
DECK_OOD_MINIMUM_EPISODES = 50
DECK_OOD_MAXIMUM_EPISODES = 1000
DECK_OOD_COUNT = 20
...
def choose_deck_ood_hashes(
    trajectories: Sequence[Mapping[str, Any]],
    *,
    minimum_episodes: int = DECK_OOD_MINIMUM_EPISODES,
    maximum_episodes: int = DECK_OOD_MAXIMUM_EPISODES,
    deck_count: int = DECK_OOD_COUNT,
    seed: str = DECK_OOD_SPLIT_SEED,
) -> tuple[str, ...]:
    """Choose a stable, supported exact-deck holdout independent of input ordering."""
    counts = Counter(
        str(trajectory["deck_manifest"]["sha256"])
        for trajectory in trajectories
    )
    eligible = [
        deck_hash
        for deck_hash, episodes in counts.items()
        if minimum_episodes <= episodes <= maximum_episodes
    ]
    if len(eligible) < deck_count:
        raise ValueError(
            f"deck-OOD split needs {deck_count} eligible decks, found {len(eligible)}"
        )

    def rank(deck_hash: str) -> str:
        return hashlib.sha256(f"{seed}|{deck_hash}".encode()).hexdigest()

    return tuple(sorted(eligible, key=lambda deck_hash: (rank(deck_hash), deck_hash))[:deck_count])


def assign_corrected_split(
    row: Mapping[str, Any], ood_deck_hashes: frozenset[str]
) -> str:
    """Map the legacy Episode split into train, IID validation, or deck-OOD."""
    deck_hash = str(row["deck_manifest"]["sha256"])
    if deck_hash in ood_deck_hashes:
        return "validation_deck_ood"
    legacy = row.get("split")
    if legacy == "train":
        return "train"
    if legacy == "validation":
        return "validation_iid"
    raise ValueError(f"unknown legacy split: {legacy!r}")
        ...
        for expected in reference["shards"][split]:
            ...
            with gzip.open(path, "rt", encoding="utf-8") as handle:
                for line in handle:
                    ...
                    row["split"] = assign_corrected_split(row, ood_deck_hashes)
                    yield row
                    count += 1
...
def _compact_records(records: list[Mapping[str, Any]]) -> dict[str, Tensor]:
    ...
    option_cat, option_mask = _pad_2d(records, "option_cat", 11, dtype=torch.int16)
...
def build_materialized_dataset(
    ...
    *,
    ...
    trajectory_index = json.loads((raw_root / "trajectory_index.json").read_text())
    if trajectory_index.get("content_sha256") != raw_reference["trajectory_index_sha256"]:
        raise ValueError("trajectory index commitment mismatch")
    ood_deck_hashes = choose_deck_ood_hashes(trajectory_index["trajectories"])
    ood_deck_set = frozenset(ood_deck_hashes)
    split_episode_counts = Counter(
        (
            "validation_deck_ood"
            if trajectory["deck_manifest"]["sha256"] in ood_deck_set
            else "validation_iid"
            if trajectory["split"] == "validation"
            else "train"
        )
        for trajectory in trajectory_index["trajectories"]
    )
    ...
    split_names = ("train", "validation_iid", "validation_deck_ood")
    shards: dict[str, list[dict[str, Any]]] = {name: [] for name in split_names}
    buffers: dict[str, list[Mapping[str, Any]]] = {name: [] for name in split_names}
    counts = {name: 0 for name in split_names}
    eligible = {name: 0 for name in split_names}
    card_data_path = Path(__file__).parents[3] / "data" / "official" / "EN_Card_Data.csv"
    try:
        rows = _iter_raw_rows(
            raw_root,
            raw_reference,
            ood_deck_hashes=ood_deck_set,
        )
        ...
        expected_total = sum(
            item["count"]
            for legacy_split in ("train", "validation")
            for item in raw_reference["shards"][legacy_split]
        )
        if sum(counts.values()) != expected_total or any(not count for count in counts.values()):
        ...
        reference: dict[str, Any] = {
            ...
            "split_contract": {
                "version": DECK_OOD_SPLIT_SEED,
                "legacy_non_ood_episode_split_preserved": True,
                "deck_hash_field": "deck_manifest.sha256",
                "ood_minimum_episodes": DECK_OOD_MINIMUM_EPISODES,
                "ood_maximum_episodes": DECK_OOD_MAXIMUM_EPISODES,
                "ood_deck_count": DECK_OOD_COUNT,
                "ood_deck_hashes": list(ood_deck_hashes),
                "episode_counts": dict(sorted(split_episode_counts.items())),
            },
...
def validate_materialized_dataset(root: Path | str, *, registry: Any) -> dict[str, Any]:
    ...
    counts = {name: 0 for name in ("train", "validation_iid", "validation_deck_ood")}
    eligible = {name: 0 for name in counts}
    for split in counts:
        for expected in reference["shards"][split]:
...
def select_a0_batch(tensors: Mapping[str, Tensor], indices: Tensor) -> dict[str, Tensor]:
    """Trim Actor fields and construct termination-aware ordered targets."""
    selected = {
    ...
    termination = tensors["action_termination"].index_select(0, indices)
    optional_stop = termination.eq(TERMINATION_CODES["optional_stop"])
    forced_max = termination.eq(TERMINATION_CODES["forced_max"])
    if not torch.all(optional_stop | forced_max):
        raise ValueError("unknown action termination code")
    rows = torch.arange(batch_size)[optional_stop]
    targets[rows, lengths[optional_stop]] = option_width
    return {
...
__all__ = [
    "CACHE_FIELDS",
    "DECK_OOD_COUNT",
    "DECK_OOD_MAXIMUM_EPISODES",
    "DECK_OOD_MINIMUM_EPISODES",
    "DECK_OOD_SPLIT_SEED",
    "SCHEMA_VERSION",
    "assign_corrected_split",
    "build_materialized_dataset",
    "choose_deck_ood_hashes",
    "iter_a0_batches",
    "select_a0_batch",
