...
class IDOnlyCodec:
    ...
    def encode(
        ...
        first_player = _as_int(current.get("firstPlayer"), -1)
        if first_player == actor:
            relative_first_player = 1
        elif first_player in (0, 1):
            relative_first_player = 2
        else:
            relative_first_player = 0
        flags = 0
...
def collate_id_only(rows: list[dict[str, Any]]) -> dict[str, Tensor]:
    ...
    option_cat = torch.zeros((batch_size, option_count, 11), dtype=torch.long)
