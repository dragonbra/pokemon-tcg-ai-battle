...
def load_source_actor_critic(
    ...
    *,
    ...
    required_keys = {"model", "epoch", "global_step", "metadata"}
    missing_keys = required_keys.difference(payload)
    if missing_keys:
        raise ValueError(
            f"source checkpoint is missing required keys: {sorted(missing_keys)}"
        )
    if payload["epoch"] != 13 or payload["global_step"] != 337_194:
        raise ValueError("0020 source checkpoint must be 0019 epoch 13 step 337194")
    metadata = payload.get("metadata") or {}
    if metadata.get("schema_version") != "0019_universal_winner_training_v1":
        raise ValueError("0020 source checkpoint must use the 0019 universal schema")
    if metadata.get("version") != "V1_universal_winner_r15":
        raise ValueError("0020 source checkpoint must use the frozen 0019 V1 model")
    if metadata.get("deployment_source_id") != 0:
        raise ValueError("0020 source checkpoint must use neutral deployment source 0")
    ...
    if model.actor_parameter_count() != 17_756_162:
        raise ValueError(
