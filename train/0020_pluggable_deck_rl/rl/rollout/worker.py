    ...
    try:
        ...
        for _ in range(job.max_steps):
            ...
            if actor == candidate_index:
                ...
                if job.opponent_remote:
                    connection.send(
                        {"kind": "opponent_decision", "observation": observation}
                    )
                    response = connection.recv()
                    if not isinstance(response, dict) or response.get("kind") != "action":
                        raise RuntimeError("resident opponent returned an invalid response")
                    action = response.get("action")
                else:
                    assert opponent_agent is not None
