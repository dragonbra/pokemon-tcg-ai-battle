...
def save_checkpoint(root: Path, *, model: nn.Module, optimizer: torch.optim.Optimizer, epoch: int, global_step: int, metadata: dict[str, Any], criteria: Sequence[str]) -> dict[str, Any]:
    ...
    torch.save({"model":model.state_dict(),"epoch":epoch,"global_step":global_step,"metadata":metadata},temporary)
    ...
    manifest={"schema_version":"0019_model_only_checkpoint_v1","path":final.name,"sha256":digest,"epoch":epoch,"global_step":global_step,"metadata":metadata,"criteria":list(criteria),"optimizer_state_saved":False,"resumable_training_state_saved":False}
