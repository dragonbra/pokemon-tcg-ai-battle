...
def _run_arm(deck_id: str, arm: str, output: Path) -> subprocess.Popen[Any]:
    ...
    command = [
        sys.executable, "-m",
        "train.0045_single_deck_expert_minimal_lora.evaluation.run_benchmark_v2",
...
def _run_controller() -> int:
    ...
    for deck_id in EXECUTION_DECK_IDS:
        ...
        subprocess.run([
            sys.executable, "-m",
            "train.0045_single_deck_expert_minimal_lora.evaluation.render_benchmark_v2_g2_vs_g3_u110_full67",
            "--allow-partial",
        ], cwd=ROOT, check=True)
    state.update({
