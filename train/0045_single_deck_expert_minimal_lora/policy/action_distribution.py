...
def _decode(
    ...
    *,
    ...
    for step in range(int(maximum.max().item())):
        ...
        logits = head.logits(batch, options, state)
...
def sample_actions(
    ...
    *,
    ...
    with torch.inference_mode():
        ...
        values = model.value_from_encoded(batch, state, dual_options.value_options)
        return _decode(
            model.head,
            batch,
...
def greedy_actions(model: SemanticActorCritic, features: dict[str, Tensor]) -> list[SampledAction]:
    with torch.inference_mode():
        ...
        values = model.value_from_encoded(batch, state, dual_options.value_options)
        return _decode(
            model.head, batch, model.actor_summary(state), options, values, greedy=True
        )
...
def evaluate_actions(
    ...
    batch, state, dual_options = model.encode_dual_options(features)
    options = dual_options.policy_options
    value = model.value_from_encoded(batch, state, dual_options.value_options)
