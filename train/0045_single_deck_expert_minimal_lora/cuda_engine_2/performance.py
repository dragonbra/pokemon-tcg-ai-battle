...
def audit_source_hot_path(repository_root: Path = REPOSITORY_ROOT) -> dict[str, Any]:
    ...
    for relative in HOT_PATH_FILES:
        ...
        rows.append({"path": relative, "0042_sha256": old_hash, "0044_sha256": new_hash})
    routing_rows = []
    for relative, expected in AUDITED_ROUTING_FILES.items():
        ...
        routing_rows.append({"path": relative, "0044_sha256": actual})
    ...
    if found:
        raise RuntimeError(f"0044 CUDA inference imports CPU feature compilation: {found}")
    return {
        "status": "PASS",
        "hot_path_files": rows,
...
def main() -> int:
    ...
    payload = {
        "schema_version": "0044_cuda_engine_2_performance_compatibility_v1",
        "status": "PASS",
