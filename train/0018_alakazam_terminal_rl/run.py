...
def _configure_wandb(args: argparse.Namespace) -> None:
    ...
    os.environ.setdefault("WANDB_NAME", f"{PROJECT_ID}-{args.version}")
    os.environ.setdefault("WANDB_TAGS", "0018,alakazam,terminal_reward,official_engine")


def _load_model(device: torch.device) -> tuple[AlakazamActorCritic, dict[str, Any]]:
    model, metadata = load_source_actor_critic()
    return model.to(device), metadata


def _runtime_metrics(
    model: AlakazamActorCritic, device: torch.device
) -> dict[str, float]:
    metrics = {
...
def _collector_metrics(collector: RolloutCollector) -> dict[str, float]:
    metrics = {
        "system/inference/candidate_batch_count": float(
            collector.inference_batch_count
        ),
        "system/inference/candidate_request_count": float(
            collector.inference_request_count
        ),
        "system/inference/candidate_batch_size_max": float(
            collector.inference_batch_size_max
        ),
        "system/inference/candidate_batch_size_mean": (
            collector.inference_request_count / collector.inference_batch_count
            if collector.inference_batch_count
            else 0.0
        ),
        "system/inference/candidate_seconds": collector.inference_seconds,
    }
    if collector.resident_opponents is not None:
        audit = collector.resident_opponents.audit()
        metrics.update(
            {
                "system/inference/opponent_request_count": float(
                    audit["request_count"]
                ),
                "system/inference/opponent_batch_count": float(audit["batch_count"]),
                "system/inference/opponent_batch_size_mean": float(
                    audit["mean_batch_size"]
                ),
                "system/inference/opponent_batch_size_max": float(
                    audit["max_batch_size"]
                ),
                "system/inference/opponent_seconds": float(audit["inference_seconds"]),
            }
        )
    return metrics
...
def _resident_pool(
    ...
    packages,
    device: torch.device,
) -> ResidentOpponentPool | None:
    if not args.resident_opponents:
        return None
    if device.type != "cuda":
        raise ValueError("resident opponent inference requires --device cuda")
    return ResidentOpponentPool(packages, device=device)
...
def _base_config(
    ...
    *,
    ...
    return {
        "schema_version": "0018_terminal_rl_training_v1",
        ...
        "inference": {
            "candidate_coalesce_ms": args.coalesce_ms,
            "opponent_routing": (
                "resident_gpu" if args.resident_opponents else "isolated_worker_cpu"
            ),
        },
        "seed": args.seed,
...
def run_probe(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        model.eval()
        resident_pool = _resident_pool(args, packages, device)
        jobs = balanced_jobs(
            packages,
...
def _load_model_only(path: Path, device: torch.device) -> AlakazamActorCritic:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="0018 Alakazam terminal-reward RL")
    ...
    def common(name: str) -> argparse.ArgumentParser:
        ...
        item.add_argument("--coalesce-ms", type=float, default=2.0)
        item.add_argument("--resident-opponents", action="store_true")
    ...
    ppo.add_argument("--gae-lambda", type=float, default=0.97)
