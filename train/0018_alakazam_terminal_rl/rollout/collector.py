...
from ..opponent_inference import OpponentRequest, ResidentOpponentPool
from ..policy.action_distribution import greedy_actions, sample_actions
from ..policy.actor_critic import AlakazamActorCritic
...
def _coalesce_ready_connections(
    connections: list[Connection],
    initial_ready: list[Connection],
    window_seconds: float,
) -> list[Connection]:
    """Briefly collect staggered worker requests into a larger inference batch."""
    ready = list(initial_ready)
    if window_seconds <= 0.0 or len(ready) == len(connections):
        return ready
    ready_set = set(ready)
    deadline = time.perf_counter() + window_seconds
    while len(ready) < len(connections):
        remaining = deadline - time.perf_counter()
        if remaining <= 0.0:
            break
        later = wait(
            [connection for connection in connections if connection not in ready_set],
            timeout=remaining,
        )
        if not later:
            break
        ready.extend(later)
        ready_set.update(later)
    return ready
...
def _update_diagnostics(target: dict[str, float], selected: tuple[dict, ...]) -> None:
    ...
    for option in selected:
        ...
        if card_id in (66, 305, 741, 742, 743):
            key = f"selected_card_{card_id}"
            target[key] = target.get(key, 0.0) + 1.0
        if card_id == 743 and int(option.get("attackId", 0) or 0) > 0:
            target["alakazam_attack_submissions"] = (
                target.get("alakazam_attack_submissions", 0.0) + 1.0
            )
...
class RolloutCollector:
    ...
    def _start(self, job: RolloutJob) -> _LiveEpisode:
        if self.resident_opponents is not None and self.resident_opponents.supports(
            job.opponent.name
        ):
            job = replace(job, opponent_remote=True)
        parent, child = self._context.Pipe(duplex=True)
        process = self._context.Process(
            target=run_engine_episode,
            args=(child, job),
            name=f"0018-rollout-{job.episode_id}",
        )
        process.start()
        child.close()
        ...
        try:
            while live_by_connection:
                ...
                ready = _coalesce_ready_connections(
                    list(live_by_connection), ready, self.coalesce_seconds
                )
                decision_requests: list[tuple[_LiveEpisode, dict]] = []
                opponent_requests: list[tuple[_LiveEpisode, dict]] = []
                for connection in ready:
                    ...
                    elif message.get("kind") == "opponent_decision":
                        opponent_requests.append((live, message["observation"]))
                    elif message.get("kind") == "result":
                        episode = self._finish(live, message)
                        if self.resident_opponents is not None:
                            self.resident_opponents.close_session(live.job.episode_id)
                ...
                if opponent_requests:
                    ...
                    actions = self.resident_opponents.select(
                        [
                            OpponentRequest(
                    ...
                    for (live, _observation), action in zip(
                        opponent_requests, actions, strict=True
                    ):
                ...
                if decision_requests:
                    ...
                    self.inference_request_count += len(actions)
                    self.inference_batch_size_max = max(
                        self.inference_batch_size_max, len(actions)
                    )
                    self.inference_seconds += latency_ms / 1_000.0
...
__all__ = ["RolloutCollector", "_coalesce_ready_connections"]
