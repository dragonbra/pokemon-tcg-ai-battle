...
MAX_CARD_ATTACKS = 4
MAX_ATTACK_ENERGIES = 7
MAX_ATTACK_EFFECTS = 7
MAX_SKILL_EFFECTS = 11
MAX_SKILL_TRIGGERS = 2
MAX_TARGET_AREAS = 4
MAX_EFFECT_TARGET_CONDITIONS = 3
MAX_TRIGGER_TARGET_CONDITIONS = 2
...
def _target_cat(target: Mapping[str, Any], symbols: Mapping[str, int]) -> list[int]:
    ...
    result.extend([
        int(areas[i]) + 1 if i < len(areas) else 0
        for i in range(MAX_TARGET_AREAS)
    ])
    conditions = list(target.get("conditions", ()))
    for index in range(MAX_EFFECT_TARGET_CONDITIONS):
...
class OfficialPrototypeEncoder(nn.Module):
    ...
    def __init__(self, config: ModelConfig, prototypes: PrototypeIndex):
        super().__init__()
        self.config = config
        self._validate_collection_capacities(prototypes)
        maxima = {
        ...
        self.attack_cat_vocabs = (
            config.max_card_id + 1, *([3] * 20),
            *([513] * MAX_ATTACK_ENERGIES), 3,
        )
        # Card/skill/effect fields use independent embeddings; ordered collections use slots.
        trigger_vocabs = (
            32, 5, 3, 3, *([26] * MAX_TARGET_AREAS),
            103, 7, symbol_vocab, 103, 7, symbol_vocab,
        )
        ...
        target_vocabs = (
            5, 3, 3, *([26] * MAX_TARGET_AREAS),
            103, 7, symbol_vocab, 103, 7, symbol_vocab, 103, 7, symbol_vocab,
        )
        self.effect_cat_vocabs = (
            4, 4, 246, 34, 66, 130, 18, config.max_skill_id + 1,
        ...
        self.effect_num = NumericFields(8 + 6, d)
    ...
    @staticmethod
    def _validate_collection_capacities(prototypes: PrototypeIndex) -> None:
        failures: list[str] = []

        def check(label: str, actual: int, maximum: int) -> None:
            if actual > maximum:
                failures.append(f"{label}={actual}>{maximum}")

        for identity, card in prototypes.engine_cards.items():
            check(f"card[{identity}].attack_ids", len(card["attack_ids"]), MAX_CARD_ATTACKS)
        for identity, attack in prototypes.engine_attacks.items():
            check(f"attack[{identity}].energies", len(attack["energies"]), MAX_ATTACK_ENERGIES)
            check(
                f"attack[{identity}].effects",
                len(attack["pre_effects"]) + len(attack["post_effects"]),
                MAX_ATTACK_EFFECTS,
            )
        for identity, skill in prototypes.skills.items():
            check(f"skill[{identity}].areas", len(skill["areas"]), 2)
            check(f"skill[{identity}].triggers", len(skill["triggers"]), MAX_SKILL_TRIGGERS)
            check(f"skill[{identity}].effects", len(skill["effects"]), MAX_SKILL_EFFECTS)
            for ordinal, trigger in enumerate(skill["triggers"]):
                subject = trigger["subject"]
                check(
                    f"skill[{identity}].trigger[{ordinal}].areas",
                    len(subject["areas"]), MAX_TARGET_AREAS,
                )
                check(
                    f"skill[{identity}].trigger[{ordinal}].conditions",
                    len(subject["conditions"]), MAX_TRIGGER_TARGET_CONDITIONS,
                )
        for identity, effect in prototypes.effects.items():
            target = effect["target"]
            check(f"effect[{identity}].target.areas", len(target["areas"]), MAX_TARGET_AREAS)
            check(
                f"effect[{identity}].target.conditions",
                len(target["conditions"]), MAX_EFFECT_TARGET_CONDITIONS,
            )
        if failures:
            raise ValueError("prototype collection capacity exceeded: " + ", ".join(failures))
    ...
    def _build_tables(config: ModelConfig, prototypes: PrototypeIndex,
        ...
        card_attacks = torch.zeros(config.max_card_id + 1, MAX_CARD_ATTACKS, dtype=torch.long)
        for identity, card in prototypes.engine_cards.items():
        ...
        attack_effects = torch.zeros(config.max_attack_id + 1, MAX_ATTACK_EFFECTS, dtype=torch.long)
        for identity, attack in prototypes.engine_attacks.items():
            ...
            attack_cat_rows[identity] = [
                ...
                *[
                    int(energies[i]) + 1 if i < len(energies) else 0
                    for i in range(MAX_ATTACK_ENERGIES)
                ],
                int(bool(attack.get("last_cancel_fail_attack"))) + 1,
            ]
            attack_num[identity] = torch.tensor([float(attack["damage"])])
            refs = list(prototypes.attack_effect_refs.get(identity, ()))
            attack_effects[identity, : len(refs)] = torch.tensor(refs)
        ...
        def trigger_cat(trigger: Mapping[str, Any] | None) -> list[int]:
            ...
            result = [
                ...
                *[
                    ...
                    for index in range(MAX_TARGET_AREAS)
                ],
            ]
            for i in range(MAX_TRIGGER_TARGET_CONDITIONS):
        ...
        skill_effects = torch.zeros(config.max_skill_id + 1, MAX_SKILL_EFFECTS, dtype=torch.long)
        for identity, skill in prototypes.skills.items():
            ...
            skill_effects[identity, : len(refs)] = torch.tensor(refs)

        effect_cat_rows: dict[int, list[int]] = {}
        effect_num = torch.zeros(config.max_effect_id + 1, 14)
        for identity, effect in prototypes.effects.items():
            target = effect["target"]
            effect_cat_rows[identity] = [
                ...
                int(effect["comparator_type"]) + 1, int(effect.get("linked_skill_id", 0)),
            ...
            effect_num[identity] = torch.tensor([
                ...
                float(effect["loop_count"]), float(effect["priority"]),
                float(effect["fail_skip"]), *_target_num(target),
            ])
        ...
        return {
            ...
            "attack_cat_table": table(
                attack_cat_rows, config.max_attack_id, 22 + MAX_ATTACK_ENERGIES
            ),
            "attack_num_table": attack_num,
            "attack_effect_table": attack_effects,
            "skill_cat_table": table(
                skill_cat_rows, config.max_skill_id,
                4 + len(SKILL_BOOLEANS) + 2 * 14,
            ),
            "skill_num_table": skill_num,
            "skill_effect_table": skill_effects,
            "effect_cat_table": table(
                effect_cat_rows, config.max_effect_id,
                8 + len(EFFECT_BOOLEANS) + 16,
            ),
    ...
    def encode_all(self) -> PrototypeEmbeddings:
        ...
        effect_ids = torch.arange(self.config.max_effect_id + 1, device=device)
        effects = self.effect(effect_ids)

        skill_ids = torch.arange(self.config.max_skill_id + 1, device=device)
        skill_effects = effects[self.skill_effect_table[skill_ids]].sum(dim=-2)
        skills = self.norm(
            self.skill_identity(skill_ids)
            + self.skill_cat(self.skill_cat_table[skill_ids])
            + self.skill_num(self.skill_num_table[skill_ids])
            + skill_effects
        )
        skills = skills * skill_ids.ne(0).unsqueeze(-1)

        attack_ids = torch.arange(self.config.max_attack_id + 1, device=device)
        attack_effects = effects[self.attack_effect_table[attack_ids]].sum(dim=-2)
        attacks = self.norm(
            self.attack_identity(attack_ids)
            + self.attack_cat(self.attack_cat_table[attack_ids])
            + self.attack_num(self.attack_num_table[attack_ids])
            + attack_effects
        )
        attacks = attacks * attack_ids.ne(0).unsqueeze(-1)

        card_ids = torch.arange(self.config.max_card_id + 1, device=device)
        card_skills = skills[self.card_skill_table[card_ids]].sum(dim=-2)
        card_attacks = attacks[self.card_attack_table[card_ids]].sum(dim=-2)
        cards = self.norm(
            self.card_identity(card_ids)
            + self.card_cat(self.card_cat_table[card_ids])
            + self.card_num(self.card_num_table[card_ids])
            + card_skills
            + card_attacks
        )
        cards = cards * card_ids.ne(0).unsqueeze(-1)
        return PrototypeEmbeddings(cards, attacks, skills, effects)
