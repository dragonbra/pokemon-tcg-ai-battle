"""Materialize actor-only causal decision rows from the frozen 0019 replay catalog."""
...
def build_trajectory_index(catalog: Mapping[str, Any]) -> dict[str, Any]:
    ...
    index: dict[str, Any] = {
        "schema_version": "0019_universal_trajectory_index_v1",
...
def build_raw_dataset(
    ...
    *,
    ...
    if start_date is not None or end_date is not None:
        ...
        trajectory_index = dict(trajectory_index)
        trajectory_index["trajectories"] = [
            row
            for row in trajectory_index["trajectories"]
            if lower <= row["episode_date"] <= upper
        ]
        if not trajectory_index["trajectories"]:
            raise ValueError("date-filtered raw dataset contains no trajectories")
        trajectory_index["date_filter"] = [lower, upper]
        trajectory_index["content_sha256"] = _canonical_hash(
            {key: value for key, value in trajectory_index.items() if key != "content_sha256"}
        )
    if max_trajectories is not None:
        if max_trajectories < 1:
            raise ValueError("max_trajectories must be positive")
        per_split = max(1, max_trajectories // 2)
        train_rows = [
            row for row in trajectory_index["trajectories"] if row["split"] == "train"
        ][:per_split]
        validation_rows = [
            row for row in trajectory_index["trajectories"] if row["split"] == "validation"
        ][:per_split]
        selected = train_rows + validation_rows
        if not train_rows or not validation_rows:
            raise ValueError("smoke catalog must contain both train and validation trajectories")
        trajectory_index = dict(trajectory_index)
        trajectory_index["trajectories"] = selected
        trajectory_index["smoke_trajectory_cap"] = max_trajectories
        trajectory_index["content_sha256"] = _canonical_hash(
            {key: value for key, value in trajectory_index.items() if key != "content_sha256"}
        )
    ...
    def write(split: str, serialized: str) -> None:
        if shard_counts[split] == 0:
            ...
            compressed = gzip.GzipFile(
                filename="", mode="wb", fileobj=raw_handle, compresslevel=3, mtime=0
            )
    ...
    try:
        for trajectory, decision_rows in _parallel_trajectory_rows(
            ...
            if processed_trajectories % 100 == 0:
                print(
                    json.dumps(
                        {
                            "event": "0031_raw_decision_progress",
        ...
        reference: dict[str, Any] = {
            "schema_version": "0031_rule_faithful_raw_dataset_reference_v1",
            ...
            "trajectory_count": processed_trajectories,
            "smoke_trajectory_cap": max_trajectories,
            "date_filter": [start_date, end_date],
            "storage_guard": {
                **initial_storage,
                "check_frequency": "before_write_and_after_each_closed_shard",
            },
        }
