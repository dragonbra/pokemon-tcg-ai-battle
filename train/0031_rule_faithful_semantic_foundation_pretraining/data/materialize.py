            ...
            for item in reference["shards"][source_split]:
                ...
                with gzip.open(path, "rt", encoding="utf-8") as handle:
                    for line in handle:
                        ...
                        if catalog is None:
                            raise ValueError(f"raw Episode is absent from 0031 catalog: {episode_id}")
...
def materialize_canonical(
    ...
    *,
    ...
    if len(catalog_episodes) != len(catalog["episodes"]):
        raise ValueError("0031 catalog contains duplicate Episode IDs")
