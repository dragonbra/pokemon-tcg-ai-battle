"""Batched canonical focal inference plus batched frozen 0019 opponent inference."""
...
from ..focal.deployment.canonical_online_runtime import OnlineCausalEncoder as FocalEncoder
...
class HeterogeneousRolloutCollector:
    def __init__(
        ...
        *,
        ...
        self.opponent, _ = load_foundation(device, eval_mode=True)
    ...
    def _start(self, job: RolloutJob) -> _Live:
        ...
        process = self.context.Process(
            ...
            args=(child, job),
            name=f"0026-rollout-{job.game_id}",
        )
        process.start()
        child.close()
        ...
        return _Live(
            ...
            parent,
            process,
            FocalEncoder(focal_actor, job.focal_deck, self.model.actor.config),
            OpponentEncoder(1 - focal_actor, job.opponent_deck, self.opponent.config),
        )
    ...
    def _infer_focal(self, requests: list[tuple[_Live, dict]]) -> list[object]:
        ...
        for index, (item, _) in enumerate(requests):
            ...
            while live:
                ...
                for connection in ready:
                    ...
                    if message.get("kind") == "decision":
                        request = (item, message["observation"])
