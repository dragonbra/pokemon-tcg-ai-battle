"""Export audited 0023 decoder checkpoints as self-contained Arena candidates."""
...
def _write_entrypoint(target: Path) -> None:
    (target / "main.py").write_text(
        '''"""0023 exported Arena candidate entrypoint."""
import os
from pathlib import Path
import sys
...
''', encoding="utf-8")
    ...
    (target / "strategy/inference.py").write_text(
        '"""Shared-evaluation inference contract for exported 0023 policies."""\n'
...
def build(checkpoint: Path, target_name: str, *, overwrite: bool = False) -> Path:
    ...
    payload = {
        "schema_version": "0023_arena_actor_model_only_v1",
    ...
    manifest = {
        "schema_version": "0023_arena_candidate_v1",
