...
def sample_actions(
    ...
    with torch.no_grad():
        ...
        return sample_actions_encoded(model, batch, state, options, values)


def sample_actions_encoded(
    model: LeagueActorCritic,
    batch: dict[str, Tensor],
    state: Tensor,
    options: Tensor,
    ...
    """Sample decoder actions from an already shared encoder forward."""
    with torch.no_grad():
        actor = model.actor
...
def greedy_actions(
    ...
    with torch.no_grad():
        state, options, values = model.encode(batch)
        return greedy_actions_encoded(model, batch, state, options, values)


def greedy_actions_encoded(
    model: LeagueActorCritic,
    batch: dict[str, Tensor],
    state: Tensor,
    options: Tensor,
    ...
    """Greedy decoder actions from an already shared encoder forward."""
    with torch.no_grad():
        ...
        evaluated = evaluate_actions_encoded(
            model, batch, state, options, values, sequences, decoded.lengths, stopped
        )
        return [
            SampledAction(
                tuple(
...
def evaluate_actions(
    ...
    return evaluate_actions_encoded(
        model, batch, state, options, value, sequences, lengths, stopped
    )
...
__all__ = [
    ...
    "greedy_actions",
    "greedy_actions_encoded",
    "sample_action",
    "sample_actions",
    "sample_actions_encoded",
]
