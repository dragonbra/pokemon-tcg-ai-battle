"""The paired-0809 SemanticPolicy and Value head under the 0040 trainable boundary."""
...
def load_value_head(checkpoint: Path, actor: nn.Module) -> LatentQueryValueHead:
    ...
    if digest != EXPECTED_VALUE_SHA256:
        raise ValueError(f"0040 value-source checkpoint SHA mismatch: {digest}")
    payload = torch.load(checkpoint, map_location="cpu", weights_only=True)
    if payload.get("schema_version") != "0036_value_model_only_checkpoint_v1":
        raise ValueError("0040 critic source schema mismatch")
    metadata = payload.get("metadata") or {}
    config = metadata.get("value_config") or {}
    expected = {"architecture": "latent_queries", "queries": 8, "layers": 2, "dropout": 0.0}
    if any(config.get(key) != value for key, value in expected.items()):
        raise ValueError(f"0040 critic architecture mismatch: {config}")
    if metadata.get("source_checkpoint_sha256") != ACTOR_SHA256:
        raise ValueError("0040 Value checkpoint is not paired with the 0809 actor")
