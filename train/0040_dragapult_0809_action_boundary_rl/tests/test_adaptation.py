...
class AdaptationTest(unittest.TestCase):
    ...
    def test_adapted_checkpoint_contains_only_trainable_tensors(self) -> None:
        ...
        expected = {
            ...
            if name.startswith(("actor.action_decoder.", "value_head.", "allocation_head."))
            or (".parametrizations." in name and not name.endswith(".original"))
            or name.startswith("actor.option_encoder.cross_attention_transformer.norm.")
        }
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "update.pt"
            storage.save_model_only(model, path, update=1, metadata={})
            payload = torch.load(path, map_location="cpu", weights_only=True)
            self.assertEqual(payload["schema_version"], "0040_model_only_checkpoint_v1")
            ...
            for name, value in payload["state_dict"].items():
                self.assertTrue(torch.equal(target.state_dict()[name], value), name)
