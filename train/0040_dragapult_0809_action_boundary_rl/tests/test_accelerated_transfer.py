...
module = importlib.import_module(
    "train.0040_dragapult_0809_action_boundary_rl.training.accelerated_transfer"
)
...
class AcceleratedTransferTest(unittest.TestCase):
    ...
    def test_chance_boundary_episode_is_replaced_in_the_same_environment_slot(self):
        ...
        protocol = importlib.import_module(
            "train.0040_dragapult_0809_action_boundary_rl.rollout.protocol"
        )
        ...
        for seed in range(2048):
            ...
            episodes.append(SimpleNamespace(
                ...
                job=SimpleNamespace(
                    ...
                    focal_first=bool(seed % 2), focal_won_toss=True,
                    toss_winner=0, first_player_selected_by="focal_policy",
                ),
                diagnostics={
                    "macro_fallback": int(reason is not None),
                    "macro_fallback_reason": reason,
                    "first_player_choice": {"focal_first": bool(seed % 2)},
                },
            ))
        with tempfile.TemporaryDirectory() as temporary:
