...
series = importlib.import_module(
    "train.0040_dragapult_0809_action_boundary_rl.evaluate_seeded_checkpoint_series"
)
...
class SeededCheckpointSeriesTest(unittest.TestCase):
    ...
    def test_legacy_series_schedule_tracks_configured_game_count(self) -> None:
        ...
        self.assertEqual(len(grouped), 2048)
        self.assertTrue(all(len(seats) == 1 for seats in grouped.values()))
