    ...
    def validate(self) -> None:
        ...
        if self.version == FORMAL_VERSION:
            if self.accelerated_transfer_acceptance:
                raise ValueError("0040 V1 must use normal learning rates")
            if self.preset_name != "PRIZE":
                raise ValueError("0040 V1 preserves the verified PRIZE preset")
            if (
                ...
                raise ValueError("0040 V1 long-run scaling contract changed")
        if self.accelerated_transfer_acceptance:
...
def run(config: RunConfig) -> dict[str, Any]:
    if not config.launch_formal:
        raise RuntimeError(
            "0040 formal PPO requires explicit user approval via the "
            "--launch-formal token; "
    ...
    os.environ.update(
        {
            ...
            "WANDB_TAGS": (
                "0040,0809,action_boundary,cuda_resident,full_stack,ppo,normal_lr,"
    ...
    config_payload = {
        "schema": "0040_action_boundary_scalable_ppo_config_v1",
        "project_id": PROJECT,
        **asdict(config),
...
def main() -> int:
    ...
    parser.add_argument("--preset", choices=tuple(PRESETS), default="PRIZE")
