"""Empirical Strategy Adapter V2 pre-implementation architecture audit.
...
def _tensor_shapes(model: nn.Module, features: dict[str, Tensor]) -> dict[str, list[int]]:
    with torch.no_grad():
        ...
        value = model.value_from_encoded(validated, state, options)
        meta_logits = model.value_head.heads.archetype(queries[:, 1])
        decoder_state = model.actor.action_decoder.initialize(
            validated, model.actor_summary(state)
        )
        logits = model.actor.action_decoder.logits(validated, options, decoder_state)
    return {
...
def _gradient_case(
    ...
    validated, state, options = model.actor.encode(features)
    if case == "policy":
        decoder = model.actor.action_decoder
        logits = decoder.logits(
            validated,
            options,
            decoder.initialize(validated, model.actor_summary(state)),
        )
        ...
        queries = model.value_head.decode(memory, memory_mask)
        if case == "value":
            value_logit = model.value_head.heads.value(queries[:, 0]).squeeze(-1)
            value = 2.0 * value_logit.sigmoid() - 1.0
...
def run_audit(output: Path = DEFAULT_OUTPUT, replay: Path = DEFAULT_REPLAY) -> dict[str, object]:
    ...
    model, identity = initialization.build_preset_from_common_update0(
        deck, presets.preset("PRIZE"), device="cpu"
    )
    ...
    report = {
        "schema_version": "0040_strategy_adapter_v2_runtime_audit_v1",
        "fixture": fixture,
        "source_identity": {
