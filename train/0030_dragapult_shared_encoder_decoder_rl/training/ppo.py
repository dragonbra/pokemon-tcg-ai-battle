    ...
    def update(self, batch: PreparedBatch) -> dict[str, float]:
        ...
        for epoch in range(self.config.epochs):
            ...
            for start in range(0, batch.decisions, self.config.batch_size):
                ...
                features["summary"] = features["summary"].float()
                features["options"] = features["options"].float()
                ...
                with torch.no_grad():
                    ...
                    reference_eval = evaluate_actions_encoded(
                        self.reference, features, sequences, lengths, stopped
                    )
                evaluated = evaluate_actions_encoded(
                    self.model.head, features, sequences, lengths, stopped
                )
