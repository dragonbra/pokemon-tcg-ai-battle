"""Train the 0028 universal semantic policy on audited official winners."""
...
DATASET_ROOT = Path(
    "rl_runs/0028_universal_semantic_foundation_pretraining/dataset/"
    "V1_mid_winners_20260710_20260801"
)
PROTOTYPES = Path(
    "train/0028_universal_semantic_foundation_pretraining/assets/"
...
def _smoke_paths():
    ...
    root = Path(".tmp/0028_universal_bc_smoke") / f"run-{uuid.uuid4().hex[:10]}"
...
def main() -> None:
    ...
    if not args.smoke and not torch.cuda.is_available():
        raise RuntimeError("formal 0028 universal BC training requires CUDA")
        ...
        os.environ.update(
            {
                ...
                "WANDB_NAME": f"0028 · universal semantic · {paths.version_name}",
                "WANDB_RUN_GROUP": PROJECT_ID,
                "WANDB_RUN_ID": wandb_run_id(PROJECT_ID, paths.version_name),
                "WANDB_JOB_TYPE": "bc",
                "WANDB_TAGS": "0028,universal,mid,winner_bc",
            }
        )
    ...
    config = {
        "schema_version": "0028_universal_bc_training_v1",
    ...
    paths.model_contract.write_text(
        json.dumps(
            {
                "schema_version": "0028_universal_model_contract_v1",
                "arm": arm_contract,
    ...
    print(
        json.dumps(
            {
                "event": "0028_universal_bc_preflight",
    ...
    print(
        json.dumps(
            {
                "event": "0028_universal_bc_complete",
                "run_root": str(paths.run_root),
                "best": summary["best"],
            },
            sort_keys=True,
