"""Benchmark the 0033 actor inside the fully resident official CUDA loop."""
...
def main() -> int:
    ...
    model_module = importlib.import_module("train.0033_dragapult_third_ptcg_club_rl.model")
    ...
    with torch.inference_mode():
        ...
        torch.cuda.nvtx.range_push("0033_pod_native_resident_rollout")
        total_start.record()
    ...
    result = {
        "schema": "0033_pod_native_resident_throughput_v1",
        "passed": (
            decision_count > 0
            and illegal_count == 0
        ...
        "scope": "official CUDA engine + PolicyCodecV1 + 0033 actor fully resident rollout",
