"""Atomic model-only checkpoint storage for 0033."""
...
SCHEMA = "0033_pod_cuda_ppo_model_only_v1"
TRAINABLE_HEADS_SCHEMA = "0033_pod_cuda_ppo_trainable_heads_v1"
...
def load_model_only(path: Path) -> dict[str, Any]:
    ...
    if payload.get("schema_version") != SCHEMA:
        raise ValueError("unsupported 0033 checkpoint schema")
...
def load_trainable_heads(model: nn.Module, path: Path) -> dict[str, Any]:
    ...
    if payload.get("schema_version") != TRAINABLE_HEADS_SCHEMA:
        raise ValueError("unsupported 0033 trainable-head checkpoint schema")
