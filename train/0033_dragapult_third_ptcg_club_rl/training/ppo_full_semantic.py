"""Decoder-only PPO over the complete frozen PT0805 semantic representation."""
...
class PPOTrainer:
    ...
    def update(self, batch: PreparedBatch) -> dict[str, float]:
        ...
        if self.model.representation_sha256() != self.initial_representation_sha256:
            raise RuntimeError("frozen PT0805 representation changed before PPO")
        ...
        for epoch in range(self.config.epochs):
            ...
            for start in range(0, batch.decisions, self.config.batch_size):
                ...
                with torch.no_grad():
                    validated, summary, options, _ = self.model.encode(features)
                    behavior_eval = evaluate_actions_encoded(
                        behavior, validated, summary, options, sequences, lengths, stopped
                    )
                    reference_eval = evaluate_actions_encoded(
                        self.reference, validated, summary, options, sequences, lengths, stopped
                    )
        ...
        if self.model.representation_sha256() != self.initial_representation_sha256:
            raise RuntimeError("frozen PT0805 representation changed during PPO")
