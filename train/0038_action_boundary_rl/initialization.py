"""Strict V5-weight-free initialization contract for the common 0038 update-0."""
...
INITIALIZATION_SCHEMA = "0038_zero_shot_update0_initialization_v1"
UPDATE0_RNG_SEED = 380_038_001
...
def assert_no_rl_checkpoint_source(path: Path, expected_sha256: str) -> None:
    ...
    if "versions" in resolved.parts or any(part.startswith("update-") for part in resolved.parts):
        raise ValueError(f"RL-updated checkpoint is forbidden for 0038 initialization: {path}")
...
def build_update0_model(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("0038 common update-0 requires 0037-compatible last-block Q/V LoRA r4/a8")
...
def initialization_manifest(config: InitializationConfig = InitializationConfig()) -> dict[str, Any]:
    return {
        ...
        "base_encoder_source": "Policy-0806 / 0031 BC epoch-11; exact weights frozen",
        "option_encoder_source": "Policy-0806 / 0031 BC epoch-11",
        "action_decoder_source": "Policy-0806 / 0031 BC epoch-11; fully trainable in RL",
        "value_trunk_source": "0036 V2 latent-query Value epoch-5; pre-RL model-only",
        ...
        "lora_initialization": "fresh A=kaiming, B=zeros; exact zero-delta; no V5 tensors",
        ...
        "forbidden_initialization_sources": [
            "0037 V4/V5 checkpoints", "any checkpoint/update-*.pt", "old PPO rollout/logprob",
        ],
...
def build_preset_from_common_update0(
    ...
    """Load shared core/allocation BC and freshly initialize only enabled sidecars."""
    from .policy.actor_critic import _sha256
    if _sha256(COMMON_UPDATE0_CHECKPOINT) != COMMON_UPDATE0_SHA256:
        raise ValueError("0038 common update-0 checkpoint hash mismatch")
    ...
    if unexpected:
        raise ValueError(f"common update-0 has unexpected tensors: {unexpected[:8]}")
    missing = sorted(set(current) - set(state))
    # Frozen Zero-Shot core is intentionally not duplicated in model-only update-0.
    missing_trainable = [name for name in missing if current[name].requires_grad]
    assert_only_new_missing_keys(missing_trainable)
    with torch.no_grad():
