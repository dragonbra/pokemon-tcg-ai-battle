...
from .constants import INITIAL_TRAIN_OPPONENTS
...
def compare_reports(
    ...
    train_opponents = set(INITIAL_TRAIN_OPPONENTS)
    holdout_opponents = all_opponents - train_opponents

    groups: dict[str, Any] = {
        "overall": _group_comparison(baseline_games, candidate_games),
        "train_pool": _group_comparison(
            _selected_games(baseline_games, opponents=train_opponents),
            _selected_games(candidate_games, opponents=train_opponents),
        ),
        "holdout_pool": _group_comparison(
            _selected_games(baseline_games, opponents=holdout_opponents),
            _selected_games(candidate_games, opponents=holdout_opponents),
        ),
    ...
    return {
        "schema_version": "0017_frozen_policy_ab_v1",
...
def render_markdown(result: Mapping[str, Any], baseline_path: Path, candidate_path: Path) -> str:
    ...
    labels = {
        "overall": "Overall",
        "train_pool": "20-opponent train pool",
        "holdout_pool": "6-opponent holdout",
