...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="0017 end-to-end CUDA training smoke")
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=REPOSITORY_ROOT / ".tmp/0017_dragapult_terminal_rl/training_smoke",
    )
    ...
    ppo = PPOTrainer(
        ...
        reference,
        device=device,
        config=PPOConfig(epochs=1, batch_size=16),
    )
