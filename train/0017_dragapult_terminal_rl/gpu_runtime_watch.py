...
DEFAULT_OUTPUT = (
    REPOSITORY_ROOT
    / "rl_runs/0017_dragapult_terminal_rl/gpu_runtime_audit.jsonl"
)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Audit cumulative 0017 CUDA process time")
    parser.add_argument("--output", type=Path, default=DEFAULT_OUTPUT)
    parser.add_argument("--interval-seconds", type=float, default=60.0)
    parser.add_argument("--target-active-hours", type=float, default=12.0)
    parser.add_argument("--match", default="train.0017_dragapult_terminal_rl.run")
    ...
    while cumulative < args.target_active_hours * 3_600.0:
        ...
        record = {
            "schema_version": "0017_gpu_runtime_audit_v1",
            "timestamp": time.time(),
