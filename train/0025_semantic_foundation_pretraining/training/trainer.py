"""Matched multi-arm BC trainer for the 0025 semantic representation ablation."""
...
def _train_epoch(
    ...
    *,
    ...
    iterator = tqdm(
        dataset.iter_batches("train", batch_size, seed=seed + epoch),
        total=total,
        desc=f"0025 {arm} epoch {epoch} train",
        unit="batch",
        dynamic_ncols=True,
        mininterval=5.0,
        leave=False,
    ...
    for batch_index, source in enumerate(iterator, 1):
        ...
        iterator.set_postfix(loss=f"{float(result.loss.detach()):.4f}")
        ...
        with TrainingLogger(paths.metrics, paths.tensorboard) as logger:
            ...
            for epoch in range(1, epochs + 1):
                ...
                _atomic_json(
                    ...
                    {
                        "schema_version": "0025_checkpoint_selection_v1",
                        "retention_slots": [
                            "latest",
                ...
                print(
                    json.dumps({"event": "0025_bc_epoch", **record}, sort_keys=True),
                    flush=True,
                )
