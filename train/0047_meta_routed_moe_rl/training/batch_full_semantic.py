    ...
    *,
    ...
    if require_policy_identity:
        ...
        for episode in episodes:
            ...
            if (
                not (
                    episode.job.opponent_policy_id in {"Policy-0809", "Policy-0814"}
                ...
                or any(
                    metadata.get("opponent_policy_id") != episode.job.opponent_policy_id
        ...
        if violations:
            raise ValueError(
                "PPO rejected episodes without complete frozen-policy/exact-deck "
    ...
    own_archetype_ids: list[int] = []
    routing_meta_ids: list[int] = []
    ...
    for episode_index, episode in enumerate(valid):
        ...
        for record, record_turn, advantage, gae_return, prize_reward, prize_return, prize_advantage in zip(
            ...
            if not isinstance(raw_meta, list) or len(raw_meta) != 15:
                raise ValueError("0044 trajectory is missing 15-class pretrained Meta logits")
            meta_logits.append([float(value) for value in raw_meta])
            own_archetype_ids.append(episode.job.focal_own_archetype_id)
            routing_meta_ids.append(
                int(record.metadata.get("routing_meta_id", -1))
                if use_compound else -1
            )
    if not features:
    ...
    return PreparedBatch(
        ...
        own_archetype_id=torch.tensor(own_archetype_ids, dtype=torch.long),
        routing_meta_id=torch.tensor(routing_meta_ids, dtype=torch.long),
    )
