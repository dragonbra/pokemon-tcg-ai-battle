...
from .dual_option_adapter import DualOptionSemantic0031DeviceAdapter
...
class CudaFullSemanticRolloutCollector:
    ...
    def __init__(
        ...
        *,
        ...
        focal_allocation_planner_for_job: Any | None = None,
        ...
        # The resident bridge consumes the immutable complete Champion-G2 actor;
        # its Value/strategy adapters remain bound through the wrapper.
        ...
        self.focal_allocation_planner_for_job = focal_allocation_planner_for_job
    ...
    def collect(self, jobs: list[RolloutJob]) -> list[EpisodeTrajectory]:
        ...
        adapter = DualOptionSemantic0031DeviceAdapter(
            self.model, None, max_select=self.max_select
        )
        ...
        def focal_strategy_fn(
            ...
            value_options = adapter.take_value_options(options)
            value, auxiliary = self.model.value_and_aux_from_encoded(
                validated, state, value_options
            )
            def readout(hidden: torch.Tensor) -> torch.Tensor:
                # 0045 Actor consumes the stable pre-adapter decoder hidden.
                # Critic outputs are returned for rollout bookkeeping only.
                return hidden
        ...
        boundary = CudaActionBoundaryAdapter(
            ...
            allocation_planner_for_job=self.focal_allocation_planner_for_job,
        )
        ...
        def sink(**payload: Any) -> None:
            ...
            route_meta = values.get("routing_meta_id")
            metadata = tuple(
                {
                    **(all_metadata[int(row)] or {}),
                    **({"routing_meta_id": int(route_meta[int(row)])}
                       if route_meta is not None else {}),
                }
        ...
        for block in staged:
            ...
            for row, job_index in enumerate(job_indices):
                ...
                auxiliary: dict[str, Any] = {
                    ...
                    "routing_meta_id": int(metadata.get("routing_meta_id", -1)),
                }
            ...
            for row, decision in enumerate(decisions):
                ...
                episode.policy_transitions.append(PolicyTransition(
                    ...
                    metadata={
                        ...
                        "routing_meta_id": decision.auxiliary_values.get("routing_meta_id", -1),
                        "cuda_resident": True,
                        "opponent_policy_id": self.opponent_policy_id,
...
class ChunkedCudaRolloutCollector:
    ...
    def __init__(self, model: nn.Module, opponent: nn.Module, *, rollout_batch_size: int,
                 ...
                 before_chunk: Any | None = None,
                 after_chunk: Any | None = None,
    ...
    def collect(self, jobs: list[RolloutJob]) -> list[EpisodeTrajectory]:
        ...
        for begin in range(0, len(jobs), self.rollout_batch_size):
            ...
            if self.after_chunk is not None:
                self.after_chunk(begin, stop, collector)
            completed = min(begin + self.rollout_batch_size, len(jobs))
            print(
                f"[0047 CUDA][update {source_update:04d}] games {completed}/{len(jobs)} "
