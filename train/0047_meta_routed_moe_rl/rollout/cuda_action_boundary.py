...
class CudaActionBoundaryAdapter:
    ...
    def __init__(
        ...
        *,
        ...
        allocation_planner_for_job: Any | None = None,
    ) -> None:
        self.model = model
        ...
        self.allocation_planner_for_job = allocation_planner_for_job
    ...
    def post_route(
        ...
        *,
        ...
        for row in phantom_rows:
            ...
            root = phantom[0]
            planner_kwargs = dict(
                state_summary=routed.state.summary[row], root_index=root,
                root_option=routed.focal_options[row, root],
                target_embeddings=routed.state.cards[row, ordered],
                target_identities=identities, visible_targets=visible,
                greedy=self.greedy, generator=self.generators[job],
            )
            if self.allocation_planner_for_job is not None:
                planned = self.allocation_planner_for_job(job, **planner_kwargs)
            else:
                planner = (
                    self.planner if self.allocation_head_for_job is None
                    else MacroPlanner(self.allocation_head_for_job(job))
                )
                planner_kwargs.pop("root_index")
                planned = planner.plan(**planner_kwargs)
            macro_action = {
                "family": "phantom_dive",
