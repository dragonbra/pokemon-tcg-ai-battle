...
def aggregate(report: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "0044_benchmark_v1_published_report_v1",
...
<div class="hero"><div><p>0044 · BENCHMARK V1 · CHAMPION-G2 MIRROR LEAGUE</p><h1>{report['focal_deck_id']} · {html.escape(report['focal_deck_display_name'])}</h1><p>Focal: Champion-G2 U407 · Opponent: Meta-balanced (deck_i, Champion-G2) · CUDA-2048</p></div>{_art(payload['focal_representative_cards'])}</div>
...
def publish_index() -> Path:
    ...
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=True)
    page = f'''<!doctype html><html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>0044 Benchmark V1</title><style>body{{font:15px/1.6 system-ui;max-width:1100px;margin:auto;padding:36px;background:#f3f7f5;color:#172a24}}section{{padding:24px;background:#fff;border:1px solid #d8e4de}}table{{width:100%;border-collapse:collapse}}th,td{{padding:10px;border-bottom:1px solid #d8e4de;text-align:left}}a{{color:#186647;font-weight:700}}.art{{display:inline-flex;width:60px}}.art img{{width:36px;height:50px;object-fit:cover;border-radius:3px;margin-right:-8px}}</style></head><body><h1>0044 · Benchmark V1</h1><p>Meta-balanced 001–067 · fixed (deck_i, Champion-G2) opponents · CUDA-2048 per focal identity.</p><section><table><thead><tr><th>Deck</th><th>卡图</th><th>Focal identity</th><th>Win rate</th><th>Games</th></tr></thead><tbody>{''.join(rows)}</tbody></table></section></body></html>'''
