...
SCHEMA = "0044_pfsp_state_v1"
CURRICULUM_SCHEMA = "0044_pfsp_curriculum_v1"
...
class PFSPConfig:
    ...
    # V1 policy sampling is degenerate at singleton Champion-G2; curriculum
    # pressure is intentionally retained on the 67-deck dimension.
    maximum_probability: float = 1.0

    @classmethod
    def load(cls, path: Path) -> "PFSPConfig":
        payload = json.loads(path.read_text(encoding="utf-8"))
        if payload.get("schema_version") != "0044_league_config_v1":
            raise ValueError("unsupported 0044 league config schema")
    ...
    def validate(self) -> None:
        if self.refresh_every_updates != 10:
            raise ValueError("0044 V2.0 refresh_every_updates must be exactly 10")
