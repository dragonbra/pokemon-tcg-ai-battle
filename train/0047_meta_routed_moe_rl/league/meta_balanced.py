...
def core_deck_half_meta_balanced_schedule(
    *, quota_seed: int, shuffle_seed: int, mappings: Sequence[DeckMapping],
    core_deck_ids: Sequence[str], lanes: int = 512,
) -> tuple[tuple[MetaDeckSlot, ...], dict[str, object]]:
    """Reserve half the lanes for equal core-deck sampling.

    The other half retains the existing Meta-first then deck-balanced schedule.
    Remainder slots rotate deterministically with ``quota_seed``.
    """
    if lanes <= 0 or lanes % 2:
        raise ValueError("core-deck half schedule requires a positive even lane count")
    members = _validated_members(mappings)
    meta_by_deck = {
        deck_id: meta_id for meta_id, deck_ids in members.items() for deck_id in deck_ids
    }
    core = tuple(str(deck_id) for deck_id in core_deck_ids)
    if not core or len(set(core)) != len(core):
        raise ValueError("core-deck identities must be non-empty and unique")
    missing = sorted(set(core) - set(meta_by_deck))
    if missing:
        raise ValueError(f"core-deck identities are absent from mappings: {missing}")
    core_lanes = lanes // 2
    general_lanes = lanes - core_lanes
    base, remainder = divmod(core_lanes, len(core))
    extra_order = list(core)
    random.Random(int(quota_seed) ^ 0x47C0DE).shuffle(extra_order)
    extra = set(extra_order[:remainder])
    core_counts = {
        deck_id: base + int(deck_id in extra) for deck_id in core
    }
    rows = [
        MetaDeckSlot(-1, meta_by_deck[deck_id], deck_id, "core_deck_half")
        for deck_id in core
        for _ in range(core_counts[deck_id])
    ]
    general = balanced_meta_deck_schedule(
        quota_seed=quota_seed, shuffle_seed=shuffle_seed ^ 0x47A11,
        mappings=mappings, lanes=general_lanes,
    )
    rows.extend(
        MetaDeckSlot(-1, row.archetype_id, row.deck_id, "meta_balanced_half")
        for row in general
    )
    random.Random(int(shuffle_seed)).shuffle(rows)
    scheduled = tuple(
        MetaDeckSlot(index, row.archetype_id, row.deck_id, row.branch)
        for index, row in enumerate(rows)
    )
    audit: dict[str, object] = {
        "mode": "core_deck_half_meta_balanced_half",
        "lanes": lanes,
        "core_lanes": core_lanes,
        "general_meta_balanced_lanes": general_lanes,
        "core_deck_ids": list(core),
        "core_deck_counts": core_counts,
        "quota_seed": int(quota_seed),
...
__all__ = [
    "MetaDeckSlot", "PROJECT_ROOT", "balanced_meta_deck_schedule",
    "core_deck_half_meta_balanced_schedule",
]
