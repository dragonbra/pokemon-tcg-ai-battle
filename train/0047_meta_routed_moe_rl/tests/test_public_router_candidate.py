...
PROJECT_ROOT = ROOT / "train/0047_meta_routed_moe_rl"
V12 = (
...
def test_compact_candidate_matches_v12_and_loads_fp32(tmp_path: Path):
    candidate = importlib.import_module(
        "train.0047_meta_routed_moe_rl.evaluation.public_router_candidate"
    )
    router = importlib.import_module(
        "train.0047_meta_routed_moe_rl.semantic_runtime.deployment.public_meta_router"
    )
    assets = importlib.import_module(
        "train.0047_meta_routed_moe_rl.assets"
    )
    portables = {
...
def test_exporter_resolves_policy0809_from_identity_audit():
    exporter = importlib.import_module(
        "train.0047_meta_routed_moe_rl.evaluation.export_public_router_kaggle"
    )
    report = json.loads((V12 / "report.json").read_text())
