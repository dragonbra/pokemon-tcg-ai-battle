"""Formal resident-CUDA PPO runner for 0032."""
...
from .runtime import FOCAL_DECK_ID, RuntimeBundle, build_runtime
from .storage import save_trainable_heads


ROOT = Path(__file__).resolve().parents[3]
PROJECT = "0032_dragapult_pod_native_cuda_rl"
FORMAL_VERSION = "V4_mega_lopunny_froslass_001_cuda_ppo_200u"
...
def _opponent_snapshot(runtime: RuntimeBundle, args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        "schema": "0032_frozen_opponent_snapshot_v1",
        "focal_deck_id": FOCAL_DECK_ID,
        "focal_deck_sha256": _sha256(args.frozen_root / FOCAL_DECK_ID / "deck.csv"),
...
def run(args: argparse.Namespace) -> int:
    ...
    os.environ.update(
        {
            ...
            "WANDB_JOB_TYPE": "ppo",
            "WANDB_RUN_ID": f"0032-{args.version.lower().replace('_', '-')}",
            "WANDB_NAME": args.version,
            "WANDB_RUN_GROUP": PROJECT,
            "WANDB_TAGS": "0032,cuda,ppo,mega_lopunny,mega_froslass",
            "WANDB_DIR": str(wandb_dir),
        }
    )
    config = {
        "schema": "0032_cuda_ppo_training_config_v1",
    ...
    _atomic_json(
        ...
        {"state": "initializing", "training_started": False, "wandb_mode": args.wandb_mode},
    )
