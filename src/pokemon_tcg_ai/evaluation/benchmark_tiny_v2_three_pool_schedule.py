"""Three disjoint Meta-balanced Policy-0809 CUDA-512 evaluation pools."""
...
CONTRACT_ID = "0045_policy0809_three_meta_pools_common_seeds_cuda512_v1"
...
def materialize(
    ...
    active = tuple(class_id for class_id, decks in by_class.items() if decks)
    claimed = set(POOL_CLASS_IDS["low_score"]) | set(POOL_CLASS_IDS["priority"])
    pool_class_ids = {
        **POOL_CLASS_IDS,
        "remaining": tuple(class_id for class_id in active if class_id not in claimed),
    }
    flattened = [class_id for values in pool_class_ids.values() for class_id in values]
    if len(flattened) != len(set(flattened)) or set(flattened) != set(active):
        raise RuntimeError("three-pool Meta partition is not an exact active-class partition")

    deck_assets = {row.deck_id: row for row in registry.decks}
    pools: dict[str, dict[str, Any]] = {}
    all_jobs: list[dict[str, Any]] = []
    for pool_index, (pool_name, class_ids) in enumerate(pool_class_ids.items()):
        ...
        for class_id in class_ids:
            ...
            deck_counts = _balanced_counts(
                deck_ids, int(class_counts[class_id]), seed=pool_seed + class_id + 1
            )
    ...
    payload = {
        "schema_version": "0045_policy0809_three_pool_schedule_v1",
        "contract_id": CONTRACT_ID,
        "master_seed": MASTER_SEED,
...
__all__ = [
    ...
    "OPPONENT_POLICY_ID", "POOL_CLASS_IDS", "materialize",
]
