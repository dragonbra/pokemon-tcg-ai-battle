...
class ActionDecoder(nn.Module):
    ...
    def logits(
        ...
        state: DecoderState,
        *,
        readout_hidden: Tensor | None = None,
        option_keys: Tensor | None = None,
        option_bias: Tensor | None = None,
    ) -> Tensor:
        hidden = state.hidden if readout_hidden is None else readout_hidden
        if hidden.shape != state.hidden.shape:
            raise ValueError("decoder readout hidden shape must match recurrent hidden")
        if option_keys is None:
            option_keys = self.key(options)
        if option_bias is None:
            option_bias = self.option_bias(options).squeeze(-1)
        pointer = (
            self.query(hidden).unsqueeze(1) * option_keys
