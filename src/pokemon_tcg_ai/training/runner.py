"""Shared PPO runner for the final deck-007 public policy.
...
from ..league.focal import balanced_focal_schedule
...
FOCAL_DECK_ID = "007"
PROJECT = "pokemon_tcg_ai"
VERSION = "dragapult_policy0814_bc_ppo"
RULES = Path(os.environ.get(
    "PTCG_RULE_PACK",
    ROOT / "engine_cuda_2_0/generated/private/official_3aaeaa92/official_rules.bin",
))
U0_CHECKPOINT = (
    ROOT / "runs/versions/dragapult_policy0814_bc_ppo/checkpoint/update-000000.pt"
)
...
def _paths(version: str = VERSION) -> dict[str, Path]:
    root = ROOT / "runs" / "versions" / version
...
def _pristine_restart_allowed(
    ...
    metrics_are_pristine = not metric_rows or (
        ...
        and metric_rows[0].get("eval/policy0814_exact_deck_cuda512") == 1.0
...
def _run_id() -> str:
    return "pokemon-tcg-ai-public-dragapult-policy0814-bc-ppo"
...
def run(
    *, updates: int | None, wandb_mode: str, launch_formal: bool,
    ...
    periodic_evaluation_profile: str = "policy0814_exact_deck_cuda512",
    ...
    if periodic_evaluation_profile != "policy0814_exact_deck_cuda512":
        raise RuntimeError("the public trainer supports only Policy-0814 exact-deck evaluation")
    ...
    if baseline_evaluation_provenance is not None:
        raise RuntimeError("the public trainer does not reuse historical evaluation reports")
    ...
    _atomic_json(paths["artifact"] / "training_config.json", {
        ...
        "periodic_evaluation": {
            ...
            "contract_id": "0045_policy0814_exact_deck_common_seeds_cuda512_v1",
            "interval_updates": (
            ...
            "pool_count": int(periodic_evaluation_enabled),
