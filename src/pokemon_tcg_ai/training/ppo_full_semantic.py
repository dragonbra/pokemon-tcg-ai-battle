"""0045 PPO over a minimal specialist Actor and an isolated Critic."""
...
def set_batch_own_archetype_ids(
    model: SemanticActorCritic, batch: PreparedBatch, indices: torch.Tensor,
) -> None:
...
class PPOConfig:
    ...
    def validate(self) -> None:
        ...
        if self.protocol_version != "ppo_protocol_v2":
            raise ValueError("0045 formal PPO must use ppo_protocol_v2")
        if min(
            self.epochs,
            self.batch_size,
        ...
        if self.meta_anchor_coef <= 0:
            raise ValueError("0045 meta_anchor_coef must be explicitly positive")
        if min(
...
class PPOTrainer:
    def __init__(
        ...
        *,
        ...
        if config.meta_anchor_coef != model.integrated_flags.meta_anchor_coef:
            raise ValueError(
                "PPO meta_anchor_coef must match the selected 0045 preset: "
    ...
    def set_reference_model(self, reference_model: SemanticActorCritic) -> None:
        """Replace the immutable same-architecture specialist KL anchor."""
    ...
    def _group_update_l2(
        self, baseline: dict[str, torch.Tensor], prefix: str,
    ) -> float:
        current = dict(self.model.named_parameters())
        terms = [
            (current[name].detach() - source.to(current[name].device)).float().square().sum()
            for name, source in baseline.items() if name.startswith(prefix)
        ]
    ...
    def sparse_gradient_diagnostics(self, batch: PreparedBatch, *, samples: int = 16) -> dict[str, float]:
        ...
        set_batch_own_archetype_ids(self.model, batch, indices)
        set_batch_own_archetype_ids(self.reference_model, batch, indices)
        validated, state, dual_options = self.model.encode_dual_options(features)
        options = dual_options.policy_options
        value, auxiliary = self.model.value_and_aux_from_encoded(
            validated, state, dual_options.value_options
        )
        ...
        actor_parameters = [
            parameter
            for module in (
                self.model.actor.action_decoder,
                self.model.policy_option_lora,
                self.model.allocation_head,
            )
            for parameter in module.parameters()
            if parameter.requires_grad
        ]
        metrics = gradient_diagnostics(
            {"win": win_loss, "prize": prize_loss}, actor_parameters
        )
        ...
        *,
        ...
        with torch.no_grad():
            ...
            for start in range(0, probe_count, probe_batch_size):
                ...
                evaluated = evaluate_actions_encoded(
                    self.model.head, validated, self.model.actor_summary(state), options,
                    sequences, lengths, stopped, value,
                )
    ...
    def _backward_microbatch(
        ...
        *,
        ...
        reference_delta = evaluated.log_prob - reference_log_prob.to(self.device)
    ...
    def _reference_log_probs(self, batch: PreparedBatch) -> torch.Tensor:
        """Compute Frozen-0045-Init log-prob anchors once per on-policy batch."""
        ...
        with torch.no_grad():
            size = min(256, self.config.behavior_probe_batch_size)
            for start in range(0, batch.decisions, size):
                indices = torch.arange(start, min(start + size, batch.decisions))
                features = index_feature_batch(
                    batch.features, indices, self.device, batch.feature_widths
                )
                set_batch_own_archetype_ids(self.reference_model, batch, indices)
                validated, state, dual_options = self.reference_model.encode_dual_options(features)
                options = dual_options.policy_options
                value = self.reference_model.value_from_encoded(
                    validated, state, dual_options.value_options
                )
                evaluated = evaluate_actions_encoded(
                    self.reference, validated,
                    self.reference_model.actor_summary(state), options,
                    batch.sequences[indices].to(self.device),
                    batch.lengths[indices].to(self.device),
                    batch.stopped[indices].to(self.device), value,
                )
                output[indices] = evaluated.log_prob.detach().float().cpu()
        return output
    ...
    def update(self, batch: PreparedBatch, *, update: int = 0) -> dict[str, float]:
        ...
        preupdate_trainable = self.snapshot_trainable_state()
            ...
            for indices in epoch_minibatches(order, self.config.batch_size):
                ...
                for micro_indices in epoch_minibatches(
                    ...
                    micro_results.append(self._backward_microbatch(
                        ...
                        reference_log_prob=reference_log_probs[micro_indices],
                    ))
                ...
                print(
                    f"\r[0045 PPO][update {update:04d} epoch {epoch}/{self.config.epochs}] "
        ...
        result.update(
            {
                ...
                **{
                ...
                "ppo/action_decoder_update_norm": self._group_update_l2(
                    preupdate_trainable, "actor.action_decoder."
                ),
                "ppo/option_lora_update_norm": self._group_update_l2(
                    preupdate_trainable, "policy_option_lora."
                ),
                "ppo/allocation_head_update_norm": self._group_update_l2(
                    preupdate_trainable, "allocation_head."
                ),
                "ppo/no_option_lora": 0.0,
                "ppo/policy_only_option_lora": 1.0,
