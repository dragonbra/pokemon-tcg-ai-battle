...
class TrainingRegressionAudit:
    ...
    status: str = "PASS"
    schema_version: str = "0044_training_regression_audit_v1"
...
def audit_training_config(path: Path) -> TrainingRegressionAudit:
    ...
    if mismatches:
        raise ValueError(f"0044 PPO regression mismatch: {mismatches}")
    if (
        ...
        raise ValueError("0044 V1 focal/opponent/taxonomy launch contract changed")
