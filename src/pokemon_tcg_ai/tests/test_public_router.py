...
MODULE = "pokemon_tcg_ai.routing.rules"
...
def test_router_routes_all_eight_public_deck_families():
...
def test_router_ogerpon_is_provisional_then_hydrapple_overrides():
    rules = importlib.import_module(MODULE)
...
def test_router_hidden_unknown_and_conflict_use_injected_default():
    rules = importlib.import_module(MODULE)
...
def test_router_default_and_manifest_are_fail_closed():
    rules = importlib.import_module(MODULE)
...
def test_router_actor_audit_routes_v19_state_ffn_and_option_norm_differences():
    runtime = importlib.import_module(
        "pokemon_tcg_ai.routing.router"
    )
    states = {
        update: {
...
def test_router_cross_route_storage_alias_is_rejected():
    runtime = importlib.import_module(
        "pokemon_tcg_ai.routing.router"
    )
...
def test_router_deployment_memory_matches_evaluation_rules():
    rules = importlib.import_module(MODULE)
...
def test_router_deployment_activation_switches_complete_actor():
    deployment = importlib.import_module(
        "pokemon_tcg_ai.semantic_runtime.deployment."
...
def test_router_staryu_and_starmie_public_evidence_routes_u25():
    rules = importlib.import_module(MODULE)
    memory = rules.PublicDeckMemory(2, torch.device("cpu"), default_update=170)
    routes = memory.observe(
