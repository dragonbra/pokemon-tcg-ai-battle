...
PROJECT_ROOT = ROOT / "src/pokemon_tcg_ai"
V12 = (
    ROOT / "runs/versions/"
    "V12_dragapult_007_public_meta_router_v3_grass_fold_u200_cuda2048/"
    "artifact/public_router_evaluation"
)
requires_private_v12 = pytest.mark.skipif(
    not (V12 / "report.json").is_file(),
    reason="requires private V12 evaluation materialization from runs/",
)


@requires_private_v12
def test_compact_candidate_matches_v12_and_loads_fp32(tmp_path: Path):
    candidate = importlib.import_module(
        "pokemon_tcg_ai.evaluation.public_router_candidate"
    )
    router = importlib.import_module(
        "pokemon_tcg_ai.semantic_runtime.deployment.public_meta_router"
    )
    assets = importlib.import_module(
        "pokemon_tcg_ai.assets"
    )
    portables = {
...
def test_exporter_resolves_policy0809_from_identity_audit():
    exporter = importlib.import_module(
        "pokemon_tcg_ai.evaluation.export_public_router_kaggle"
    )
    report = json.loads((V12 / "report.json").read_text())
