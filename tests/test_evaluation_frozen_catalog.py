...
class FrozenCatalogTest(unittest.TestCase):
    def test_catalog_loads_50_lightweight_decks_and_one_policy(self) -> None:
        catalog = load_frozen_catalog(FROZEN_CATALOG, EVALUATION_ROOT)

        self.assertEqual(len(catalog.opponents), 50)
        self.assertEqual(len({item.deck_hash for item in catalog.opponents}), 50)
        self.assertEqual(catalog.pool_id, "0019_foundation_50_exact_decks_v3")
    ...
    def test_cli_defaults_to_frozen_and_preserves_explicit_legacy_pool(self) -> None:
        ...
        self.assertEqual(len(frozen.stdout.splitlines()), 50)
        self.assertIn("dragapult_ex_001", frozen.stdout.splitlines())
        self.assertIn("mega_lopunny_ex_001", frozen.stdout.splitlines())
        self.assertIn("mega_lopunny_ex_mega_froslass_ex_002", frozen.stdout.splitlines())
