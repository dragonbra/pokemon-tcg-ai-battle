...
class FrozenArenaAssetTest(unittest.TestCase):
    def test_frozen_catalog_has_one_policy_and_48_exact_decks(self) -> None:
        ...
        self.assertEqual(manifest["deck_count"], 48)
        self.assertEqual(len(entries), 48)
        self.assertEqual(len({entry["name"] for entry in entries}), 48)
