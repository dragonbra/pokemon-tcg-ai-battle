# 当前候选文档

这里记录当前候选的卡组设计、策略和本轮目标。当前代码位于 [`work/alakazam_v8_current/`](../alakazam_v8_current/)，历史结论放在 `codex/index/` 或 `reports/`，不在此复制完整 replay。

## 当前 V8 文档入口

- [`DECK_NOTES.md`](DECK_NOTES.md)：逐张卡牌的规则事实、V7 AutoIter 语义和 V8 待验证建议。
- [`STRATEGY.md`](STRATEGY.md)：当前沿用的主行动顺序、Item Lock 边界和 replay 验收约定。
- [`ANALYSIS.md`](ANALYSIS.md)：V8 与 V7 卡表差异、数据来源和构筑层面的研究假设。
- [`CURRENT_OBJECTIVES.md`](CURRENT_OBJECTIVES.md)：当前迭代目标和 promotion guardrail。
