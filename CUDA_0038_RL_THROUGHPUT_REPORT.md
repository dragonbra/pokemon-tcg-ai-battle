...
- 新训练版本为 `V8_repeat_guard_turn_limit_cuda_fresh_rl`，每 update 2,048 局（8 个随机 256-slot 环境频率单位），`fixed_optimizer_budget`，并使用 50 完整回合 terminal draw 和稳定 Ability 身份重复判负。V7 在首个 PPO update 前 fail closed，不计入正式训练吞吐或强度结论。
