...
- 评测 CLI 使用 `python3 -m evaluation list-opponents`、`validate <package>` 和 `run --candidate <package> --opponents all --output <report-path>`。每次正式“评测”都只对 `evaluation/arena/opponents/` 固定池运行；这里的 `arena/candidates/` 是候选 opponent 准入区，不是 CLI `--candidate` 所指的被评测卡组。正式实验输出必须是 `rl_runs/evaluation/<000N-project>/V<n>_<tag>.html`，项目目录直接保存版本 HTML，并维护一个 `index.html` 汇总所有 `V*.html` 的 candidate、run、局数、胜负、error、胜率和完成率；每次新增正式报告后必须自动刷新 index，版本名可点击进入完整报告。manifest、逐局轻量记录、指标和 case 摘要都内嵌在版本报告中，并继续保留真实 `run_id`。`.tmp/evaluation/<purpose>` 等临时输出仍可使用隔离的 `run-<id>/report.html`。`evaluation/arena/` 不属于 Kaggle 正式 submission 输出目录。
...
- Agent 为验证、smoke、benchmark 或临时验收生成的 Evaluation report 必须写到仓库根目录 `.tmp/evaluation/<purpose>/`，不得写到系统 `/tmp`；保留 Evaluation 自动创建的 `run_id/`，并在交付时给出仓库内可点击的 `report.html` 路径，方便直接用 VS Code 查看。`.tmp/` 只用于可删除的本机临时产物，必须保持 Git ignore，禁止提交。正式实验评测仍写入对应 `rl_runs/evaluation/<experiment>/V<n>_<tag>.html`，不得用 `.tmp/` 代替可审计的正式版本产物。
...
- 每个训练 epoch 都必须对完整 train 与 validation split 执行同口径评测，并把同名标量同时写入 `training_metrics.jsonl` 和 TensorBoard；正式在线记录还必须镜像到 W&B。不得用间隔评测、缺失值、插值或仅有 console progress 代替逐 epoch 曲线。训练成本需要优化时应优化评测实现，不能跳过 epoch 指标。
- 评测同样使用 `evaluation/V<n>_<tag>.html`，并与产生 candidate 的训练版本对应；项目级 `evaluation/index.html` 是所有版本评测的总览入口。仅评测解析器或 opponent catalog 变化时也要新建版本并在 tag/决策记录中说明变量，不得混写已有报告。正式 HTML 已存在时必须拒绝覆盖并分配新的版本或明确后缀。
...
- 自动 W&B 镜像只允许由 `TrainingLogger` 对 `rl_runs/artifact/<experiment>/V<n>_<tag>/` 正式路径启用。一个 repository version 映射到一个稳定 W&B run ID；同一版本的真实断点恢复可以 resume，新训练语义、超参数或策略更新必须分配新的 `V<n>_<tag>` 和 W&B run，禁止复用旧 run 或向旧曲线追加另一项实验。
...
- 每个正式版本的 `status.json`、`training_summary.json` 或等价版本记录必须保留 W&B project、稳定 run ID 或 URL、sync 状态以及失败原因；本地 `rl_runs/wandb/<experiment>/<version>/` 只作为可再生 staging 并保持 Git ignore。
