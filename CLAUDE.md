...
- Frozen-0806 checkpoint 强度评测固定使用 seeded official engine 和两个不可变的 256 局最小单位，即每个候选 512 局；evaluation seed 固定为 `341512806` 并与 RL 采样 seed 分离。两单位必须保持相同 exact-deck/opponent schedule，合计 256 局先手、256 局后手和 256 对共享 engine/Search seed 的换手场景；只有 512/512 finished、0 error、0 unfinished 的报告才可用于同合同强度比较。2026-08-07 的 Policy-0806 旧 256 局全池评测目前只完成 25/55，资产与中断日志必须保留，不得删除、覆盖、拼接到新 512 报告或冒充完整评测；后续默认新建独立 seeded-512 run，只有用户再次明确要求时才可继续旧合同。
