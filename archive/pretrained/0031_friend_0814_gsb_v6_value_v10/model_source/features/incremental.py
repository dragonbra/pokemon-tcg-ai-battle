"""Dependency-proven incremental canonical feature compilation.

The stateless compiler remains authoritative.  This module only reuses an
immutable layer when every actor-visible input consumed by that layer compares
equal to the previous chronological decision.  Any classification or relation
failure clears the cache and executes the full compiler for that decision.
"""
...
class IncrementalCanonicalCompiler:
    """One-session canonical layer cache with fail-closed full rebuilds."""

    def __init__(self, prototypes: PrototypeIndex):
        self.prototypes = prototypes
        self.stats = IncrementalCompileStats()
        self._dependencies: LayerDependencies | None = None
        self._layers: _LayerBundle | None = None
    ...
    def _fallback(
        ...
        if snapshot.perspective_actor in (0, 1):
            self._session_actor = snapshot.perspective_actor
        return record
    ...
    def compile(
        ...
        try:
            current = project_layer_dependencies(row, snapshot)
        ...
        if self._session_actor is None:
            self._session_actor = current.actor
        elif current.actor != self._session_actor:
            return self._fallback(row, snapshot, "actor_mismatch")
        if (
            self._last_decision_index is not None
            and current.decision_index != self._last_decision_index + 1
        ):
            return self._fallback(row, snapshot, "non_chronological_decision")

        dirty = dirty_layers(self._dependencies, current)
        if self._layers is None:
            dirty = frozenset(LAYER_NAMES)
        previous = self._layers
        try:
            cards = (
                compile_card_layer(row, snapshot, self.prototypes)
                if "cards" in dirty
                else previous.cards
            )
            resources = (
                compile_resource_layer(row, snapshot)
                if "resources" in dirty
                else previous.resources
            )
            events = (
                compile_event_layer(row, snapshot, cards)
                if "events" in dirty
                else previous.events
            )
            options = (
                compile_option_layer(row, snapshot, self.prototypes, cards)
                if "options" in dirty
                else previous.options
            )
            globals_ = (
                compile_global_layer(row, snapshot)
                if "globals" in dirty
                else previous.globals
            )
        ...
        for layer in LAYER_NAMES:
            self.stats._increment(
                f"layer/{layer}/miss" if layer in dirty else f"layer/{layer}/hit"
            )
        if len(dirty) == len(LAYER_NAMES):
            self.stats._increment("full_rebuilds")
        self._dependencies = current
        self._layers = bundle
        self._last_decision_index = current.decision_index
        return record
