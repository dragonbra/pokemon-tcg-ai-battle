...
def _load_source(
    ...
    if category_config.enabled:
        lookup = build_card_category_lookup(card_vocab_size=model_config.card_vocab_size)
        ...
        nn.init.zeros_(model.card_category_embedding.weight)
        output_metadata["card_category_embedding"] = {
            "enabled": True,
            "categories": list(CATEGORY_NAMES),
            "category_vocab_size": len(CATEGORY_NAMES),
            "scale": category_config.scale,
            "mapping": "OFFICIAL_CARD_CATEGORY_BY_ID",
            "mapping_version": STATIC_MAPPING_VERSION,
            "mapping_sha256": STATIC_MAPPING_SHA256,
            "source_csv_sha256": SOURCE_CSV_SHA256,
            "mapped_card_ids": len(OFFICIAL_CARD_CATEGORY_BY_ID),
            "card_counts": CATEGORY_COUNTS,
        }
    else:
...
def _reward_mean(
    records: Iterable[dict[str, Any]], config: ExperimentConfig
) -> float:
    total = 0.0
    count = 0
    for chunk in chunks(records, 4096):
        rewards, _ = compose_rewards(chunk, config.reward)
...
def _scan_streaming_training_data(
    dataset: Path,
    reward_sidecar: Path | None,
    config: ExperimentConfig,
) -> tuple[dict[str, int], str, float]:
    """Audit the streams and compute the train reward mean in one bounded pass."""
    counts = {"train": 0, "validation": 0, "test": 0}
    experts: set[str] = set()
    reward_total = 0.0
    reward_count = 0
    for split in counts:
        records = iter_split_records(dataset, split, reward_sidecar=reward_sidecar)
        for chunk in chunks(records, 4096):
            counts[split] += len(chunk)
            experts.update(
                str(record["expert_team_name"])
                for record in chunk
                if record.get("expert_team_name")
            )
            if split == "train":
                rewards, _ = compose_rewards(chunk, config.reward)
                reward_total += float(rewards.sum().item())
    ...
    reward_mean = reward_total / reward_count if reward_count else 0.0
    return counts, next(iter(experts)), reward_mean
...
def train(
    ...
    if config.streaming:
        split_counts, expert_team, dataset_reward_mean = _scan_streaming_training_data(
            dataset,
            reward_sidecar,
            config,
        )
    else:
        if reward_sidecar is not None:
            raise ValueError("reward sidecar currently requires streaming=true")
        ...
        expert_team = next(iter(experts))
        dataset_reward_mean = _reward_mean(splits["train"], config)
    ...
    resolved_config = {
        **config.to_dict(),
        ...
        "reward_sidecar": str(reward_sidecar.resolve()) if reward_sidecar else None,
        "reward_sidecar_sha256": _sha256(reward_sidecar) if reward_sidecar else None,
    ...
    with TrainingLogger(paths.metrics, paths.tensorboard) as logger:
        for epoch in range(1, config.epochs + 1):
            ...
            model.train()
            if config.streaming:
                train_records: Iterable[dict[str, Any]] = shuffle_buffer(
                    iter_split_records(dataset, "train", reward_sidecar=reward_sidecar),
            ...
            seen = 0
            total_batches = (
                split_counts["train"] + config.batch_size - 1
            ) // config.batch_size
            for batch_index, chunk in enumerate(
                chunks(train_records, config.batch_size), 1
            ):
                optimizer.zero_grad()
                ...
                if batch_index % config.progress_batches == 0 or batch_index == total_batches:
                    elapsed = time.monotonic() - epoch_started
                    print(
                        json.dumps(
                            {
                                "event": "reward_weighted_bc_batch",
                                "epoch": epoch,
                                "epochs": config.epochs,
                                "batch": batch_index,
                                "batches": total_batches,
                                "records": seen,
                                "records_per_second": seen / max(elapsed, 1e-9),
                            },
                            sort_keys=True,
                        ),
                        flush=True,
            ...
            validation_records = (
                iter_split_records(dataset, "validation", reward_sidecar=reward_sidecar)
                if config.streaming
            ...
            if (
                config.train_eval_interval > 0
                and epoch % config.train_eval_interval == 0
            ):
                evaluation_records = (
                    iter_split_records(dataset, "train", reward_sidecar=reward_sidecar)
                    if config.streaming
    ...
    if config.evaluate_test and split_counts["test"]:
        test_records = (
            iter_split_records(dataset, "test", reward_sidecar=reward_sidecar)
            if config.streaming
            else splits["test"]
        )
        test_metrics = _evaluate(model, test_records, config, device, dataset_reward_mean)
    else:
        test_metrics = {}
    summary = {
        "status": "completed",
...
def _main() -> None:
    ...
    parser.add_argument("--reward-sidecar", type=Path)
    ...
    summary = train(
        ...
        reward_sidecar=args.reward_sidecar,
    )
