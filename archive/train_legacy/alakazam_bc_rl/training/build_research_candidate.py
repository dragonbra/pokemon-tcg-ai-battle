...
from archive.train_legacy.alakazam_bc_rl.training.rewards import observation_potential
...
def main() -> None:
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=Path("/tmp/alakazam_bc_research"),
    )
    parser.add_argument(
        "--teacher",
        type=Path,
        default=Path(__file__).resolve().parents[3] / "work" / "alakazam_v9",
    )
