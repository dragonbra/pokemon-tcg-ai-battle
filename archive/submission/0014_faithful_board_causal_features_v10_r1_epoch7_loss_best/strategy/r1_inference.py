"""CPU inference session for the 0014 R1 structured causal policy."""
...
class R1Policy:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R1 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        ...
        model = R1StructuredCausalPolicy(config, ontology_path=ontology_path)
