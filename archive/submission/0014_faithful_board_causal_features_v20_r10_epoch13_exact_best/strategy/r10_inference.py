"""CPU inference session for the 0014 R10 bounded state-calibration policy."""
...
class R10Policy:
    def __init__(
        self,
        model: R10BoundedStateCalibrationPolicy,
        deck: Sequence[int],
        config: IDOnlyConfig,
    ) -> None:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R10 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R10 checkpoint metadata is missing model.ac.base")
        ...
        config = R10ModelConfig(
            ...
            state_maximum_scale=float(raw["state_maximum_scale"]),
            state_initial_scale=float(raw["state_initial_scale"]),
        )
        model = R10BoundedStateCalibrationPolicy(config, ontology_path=ontology_path)
