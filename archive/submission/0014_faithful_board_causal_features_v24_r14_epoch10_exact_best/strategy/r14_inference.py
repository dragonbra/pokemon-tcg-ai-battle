"""CPU inference session for the 0014 R14 policy."""
...
class R14Policy:
    def __init__(
        self,
        model: R14ScenarioModalityDropoutPolicy,
        deck: Sequence[int],
        config: IDOnlyConfig,
    ) -> None:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R14 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R14 checkpoint metadata is missing model.ac.base")
        ...
        config = R14ModelConfig(
            ...
            scenario_modality_dropout=float(raw["scenario_modality_dropout"]),
        )
        model = R14ScenarioModalityDropoutPolicy(config, ontology_path=ontology_path)
