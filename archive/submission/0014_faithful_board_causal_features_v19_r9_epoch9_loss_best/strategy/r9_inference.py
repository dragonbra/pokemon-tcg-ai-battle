"""CPU inference session for the 0014 R9 narrow strong option-query policy."""
...
class R9Policy:
    def __init__(
        self,
        model: R9NarrowStrongOptionQueryPolicy,
        deck: Sequence[int],
        config: IDOnlyConfig,
    ) -> None:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R9 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R9 checkpoint metadata is missing model.ac.base")
        ...
        config = R9ModelConfig(
            ac=ac,
            scenario_layers=int(raw["scenario_layers"]),
            scenario_ffn_multiplier=int(raw["scenario_ffn_multiplier"]),
            route_ffn_multiplier=int(raw["route_ffn_multiplier"]),
            evidence_dropout=float(raw["evidence_dropout"]),
            initial_scale=float(raw["initial_scale"]),
        )
        model = R9NarrowStrongOptionQueryPolicy(config, ontology_path=ontology_path)
