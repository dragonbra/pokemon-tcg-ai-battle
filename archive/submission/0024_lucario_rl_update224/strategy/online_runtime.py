"""Online causal feature encoder matching the frozen 0020 foundation."""
...
from .features.compiler import _event_features, _ledger_features, _zone_row
from .knowledge.state import CausalKnowledge


def _padded_2d(rows: Sequence[Sequence[int | float]], width: int, dtype: torch.dtype) -> tuple[Tensor, Tensor]:
...
class OnlineCausalEncoder:
    ...
    def encode(self, observation: Mapping[str, Any]) -> dict[str, Tensor]:
        ...
        if encoded is None:
            raise RuntimeError("observation exceeds the frozen 0020 codec contract")
        ...
        registered, registered_mask = _padded_1d(registered_ids, torch.long)
        multiplicity, multiplicity_mask = _padded_1d(
            [counts[card_id] for card_id in registered_ids], torch.float32
        )
