...
FULL_CHECKPOINT_SCHEMA = "0031_model_only_checkpoint_v1"
SLIM_CHECKPOINT_SCHEMA = "0031_model_only_checkpoint_v1_slim_prototype_dedup"
PROTOTYPE_REPLICA_PREFIXES = (
    "state_encoder.prototypes.",
    "option_encoder.prototypes.",
)


def _inflate_slim_state_dict(state_dict: dict[str, Any]) -> dict[str, Any]:
    inflated = dict(state_dict)
    for key, value in state_dict.items():
        if not key.startswith("prototype_encoder."):
            continue
        suffix = key[len("prototype_encoder.") :]
        for prefix in PROTOTYPE_REPLICA_PREFIXES:
            inflated.setdefault(prefix + suffix, value)
    return inflated
...
class PortableSemanticPolicy:
    ...
    def from_checkpoint(
        ...
        if payload["schema_version"] not in (FULL_CHECKPOINT_SCHEMA, SLIM_CHECKPOINT_SCHEMA):
        ...
        if payload["schema_version"] == SLIM_CHECKPOINT_SCHEMA:
            state_dict = _inflate_slim_state_dict(state_dict)
