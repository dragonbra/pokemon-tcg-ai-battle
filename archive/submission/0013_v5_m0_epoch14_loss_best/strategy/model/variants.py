...
class SemanticGoalPolicy(nn.Module):
    ...
    def deterministic_actions(self, batch: dict[str, Tensor]) -> BatchedActionResult:
        ...
        result = self.deterministic_action_tensors(batch, encoded=self.encode(batch))
    ...
    def deterministic_action_tensors(
        ...
        *,
        ...
        encoded = self.encode(batch) if encoded is None else encoded
    ...
    def teacher_logits(self, batch: dict[str, Tensor], targets: Tensor) -> Tensor:
        return self.teacher_logits_from_encoding(batch, targets, self.encode(batch))
