"""CPU inference session for the 0014 R12 policy."""
...
class R12Policy:
    def __init__(
        self,
        model: R12StatePreservingDropoutPolicy,
        deck: Sequence[int],
        config: IDOnlyConfig,
    ) -> None:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R12 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R12 checkpoint metadata is missing model.ac.base")
        ...
        model = R12StatePreservingDropoutPolicy(config, ontology_path=ontology_path)
