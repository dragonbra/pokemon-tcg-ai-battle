"""CPU inference session for the 0014 R15 policy."""
...
class R15Policy:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R15 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R15 checkpoint metadata is missing model.ac.base")
        ...
        model = R15DeterministicGradualOptionPolicy(config, ontology_path=ontology_path)
