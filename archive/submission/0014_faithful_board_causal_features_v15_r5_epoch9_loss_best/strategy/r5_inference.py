"""CPU inference session for the 0014 R5 option-query scenario policy."""
...
class R5Policy:
    def __init__(
        self,
        model: R5OptionQueryScenarioPolicy,
        deck: Sequence[int],
        config: IDOnlyConfig,
    ) -> None:
    ...
    def from_checkpoint(
        ...
        if not isinstance(raw, dict) or not isinstance(raw.get("ac"), dict):
            raise ValueError("R5 checkpoint metadata is missing model.ac")
        ac_raw = raw["ac"]
        if not isinstance(ac_raw.get("base"), dict):
            raise ValueError("R5 checkpoint metadata is missing model.ac.base")
        ...
        model = R5OptionQueryScenarioPolicy(config, ontology_path=ontology_path)
