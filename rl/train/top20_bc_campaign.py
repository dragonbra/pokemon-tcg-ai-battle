...
from rl.train.download_expert_replays import (
    _deck_from_replay,
    _download_one,
...
RUNS_ROOT = REPOSITORY_ROOT / "rl/_runs"
ROSTER_CSV = REPOSITORY_ROOT / "docs/reports/rl/top100_static_rank_index-20260723.csv"
RESEARCH_ROOT = REPOSITORY_ROOT / "rl/artifact/dataset/top100_research_20260723"
SHARED_CONFIG = REPOSITORY_ROOT / "rl/model/shared_model_config.json"
...
def _prepare_dataset(entry: dict[str, Any]) -> tuple[Path, dict[str, Any]]:
    ...
    dataset_root = REPOSITORY_ROOT / "rl/artifact/dataset" / str(entry["experiment_id"])
    ...
    if not summary_path.is_file():
        command = [
            sys.executable,
            "-m",
            "rl.train.build_kaggle_bc_dataset",
            str(raw_root),
            "--manifest",
            str(raw_manifest),
            "--output",
    ...
    if audit_required:
        command = [
            sys.executable,
            "-m",
            "rl.train.audit_kaggle_bc_dataset",
            str(dataset),
            "--manifest",
            str(data_manifest),
            "--output",
...
def _download_source(entry: dict[str, Any]) -> Path:
    ...
    command = [
        sys.executable,
        "-m",
        "rl.train.download_expert_replays",
        "--source-manifest",
    ...
    for entry in campaign["sources"]:
        ...
        if not package.is_dir():
            command = [
                sys.executable,
                "-m",
                "rl.train.build_full_action_submission",
                "--checkpoint",
                str(checkpoint),
                "--output",
                str(package),
...
def cleanup_datasets() -> dict[str, Any]:
    ...
    for entry in campaign["sources"]:
        ...
        if dataset.is_file():
            artifact_root = (REPOSITORY_ROOT / "rl/artifact/dataset").resolve()
