...
from rl.train.render_bc_capacity_search import render_campaign
...
def _freeze_campaign(
    ...
    metadata_path = dataset.with_suffix(dataset.suffix + ".card_metadata.json")
    summary_path = dataset.with_suffix(dataset.suffix + ".summary.json")
    data_manifest = repository_root / "rl/_runs/0003-yushin_ito_exact_bc_v2/data_manifest.json"
    data_audit = repository_root / "rl/_runs/0003-yushin_ito_exact_bc_v2/audit.json"
    ...
    source_patch = _git(
        ...
        "--",
        "rl/model",
        "rl/train/train_full_action_bc.py",
        "rl/train/build_full_action_candidate.py",
        "rl/train/bc_capacity_search.py",
        "rl/train/render_bc_capacity_search.py",
        "rl/core",
        "evaluation/cli.py",
        "evaluation/runner",
        "evaluation/metrics",
...
def _source_hash(repository_root: Path) -> tuple[str, dict[str, str]]:
    roots = [
        repository_root / "rl/model",
        repository_root / "rl/core",
        repository_root / "rl/train/train_full_action_bc.py",
        repository_root / "rl/train/build_full_action_candidate.py",
        repository_root / "rl/train/bc_capacity_search.py",
        repository_root / "rl/train/render_bc_capacity_search.py",
...
def main() -> None:
    ...
    parser.add_argument(
        "--dataset",
        type=Path,
        default=Path("rl/artifact/dataset/0003-yushin_ito_exact_bc_v2/dataset.jsonl"),
    )
